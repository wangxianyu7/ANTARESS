...
void calculate_RV_with_BOUCHY(double *wavelength,double *sig_spectrum_raw,double *spectrum,double *spectrum_master, int n1, double *RV, double *sig_RV, double detector_noise)
{
    /*
    ...
    On utilise sig_spectrum_raw pour l erreur, car le vrai signal est signal avant la correction du BLAZE, de l extinction atmospherique
    et de la distribution de l energie spectrale (Planck function)
    */
    ...
    for (i=1;i<n1;i++)
    {
        ...
        sig_A = sqrt(pow(sig_spectrum_raw[i-1],2) + pow(detector_noise,2)); // =A_rms in paper. photon noise = sqrt(flux). The flux is in electron ~ photons, we normalize here
...
void get_RV_line_by_line_c(double *vrad_ccf, double *RV_vect, double *sig_RV_vect, double *chisq_vect, int n1, double *wavelength, double *sig_spectrum_raw, double *spectrum, int n2, double *wavelength_master, double *spectrum_master, int n3, double detector_noise, double **fit_para) //n1 = len(vrad_ccf), n2 = len(avelength), n3 = len(wavelength_master)
{
    int i,j, *index_same;
...
//            gsl_vector_set (w, j, 1.0/(sig_spectrum_master_norm*sig_spectrum_master_norm));
            ...
            sig_spectrum = pow(pow(sig_spectrum_raw[j],2) + pow(detector_noise,2),1/2.);
            //printf("%.4f\n",sig_spectrum_master_norm);
        ...
        else
        {
            ...
            calculate_RV_with_BOUCHY(wavelength,sig_spectrum_raw,spectrum,spectrum_master_norm,n2,&RV,&sig_RV,detector_noise);
            RV_vect[i] = RV;
...
void get_RV_line_by_line_Bouchy_without_offset(double *normalization, double *RV, double *sig_RV, double *constant, double *chi2, double freq_line, double *wavelength, double *sig_spectrum_raw, double *spectrum, int n2, double *wavelength_master, double *spectrum_master, int n3, double detector_noise,double vrad_ccf) //n1 = len(vrad_ccf), n2 = len(avelength), n3 = len(wavelength_master)
{
    ...
    for (j=0;j<n2;j++)
    {
        ...
        {
            if (index_same[j]-1 < 0) printf("\n\n----- Be careful, want to access a value outside vector wavelength_master_shift bounds (wave=%.4f, wave_master[0,1]=%.4f,%.4f)  ------\n\n",wavelength[0],wavelength_master_shift[0],wavelength_master_shift[1]);
            //printf("diff < 0\t");
            //printf("%i\t%i\t%.8f\t%.8f\t%.8f\t%.8f\n",index_same[j], n3, wavelength_master_shift[index_same[j]-1],wavelength[j],wavelength_master_shift[index_same[j]],diff[j]);
        ...
        {
            if (index_same[j]+1 >= n3) printf("\n\n----- Be careful, want to access a value outside vector wavelength_master_shift bounds (wave=%.4f, wave_master[-2,-1]=%.4f,%.4f)  ------\n\n",wavelength[n2-1],wavelength_master_shift[n3-2],wavelength_master_shift[n3-1]);
            //printf("diff >= 0\t");
            //printf("%i\t%i\t%.8f\t%.8f\t%.8f\t%.8f\n",index_same[j], n3,wavelength_master_shift[index_same[j]],wavelength[j],wavelength_master_shift[index_same[j]+1],diff[j]);
        ...
        //printf("diff = %.4e\tw=%.4f\tw_master0=%.4f\tw_master1=%.4f\ty0=%.4f\ty1=%.4f\ty_interpol=%.4f\n",diff[j],wavelength[j],x0,x1,y0,y1,spectrum_master_same[j]);
    }

    //BOUCHY method using a linear least square.
    for (j=0;j<n2;j++)
    {
        sig_spectrum = pow(pow(sig_spectrum_raw[j],2) + pow(detector_noise,2),1/2.); //We assume that the noise on the master is negligible, therefore no noise on the derivative
        ...
        //printf("C code: %.4f\t%.4f\t%.4f\t%.4f\t%.4e\n", spectrum_master_same[j], derivative_spectrum_master_same[j], spectrum[j], sig_spectrum, 1.0/(sig_spectrum*sig_spectrum));
    }
...
static PyObject *get_RV_line_by_line(PyObject *self, PyObject *args)
{
    ...
    double *vrad_ccf,*wavelength,*sig_spectrum_raw,*spectrum,*wavelength_master,*spectrum_master,*RV_vect,*sig_RV_vect,*chisq_vect;
    ...
    sig_spectrum_raw   = (double *) (c->data + 0*c->strides[0]);
    ...
    get_RV_line_by_line_c(vrad_ccf, RV_vect, sig_RV_vect, chisq_vect, n1, wavelength, sig_spectrum_raw, spectrum, n2, wavelength_master, spectrum_master, n3, detector_noise, fit_para);
...
static PyObject *get_RV_line_by_line2(PyObject *self, PyObject *args)
{
    ...
    double *wavelength,*sig_spectrum_raw,*spectrum,*wavelength_master,*spectrum_master;
    ...
    sig_spectrum_raw   = (double *) (b->data + 0*b->strides[0]);
    ...
    get_RV_line_by_line_Bouchy_without_offset(&normalization, &RV, &sig_RV, &constant, &chi2, freq_line, wavelength, sig_spectrum_raw, spectrum, n2, wavelength_master, spectrum_master, n3, detector_noise,vrad_ccf);
...
static PyObject *get_RV_line_by_line2_with_offset(PyObject *self, PyObject *args)
{
    ...
    get_RV_line_by_line_Bouchy_with_offset(&normalization, &RV, &sig_RV, &constant, &chi2, freq_line, wavelength, sig_spectrum_raw, spectrum, n2, wavelength_master, spectrum_master, n3, detector_noise,vrad_ccf);
...
static PyObject *get_RV_line_by_line3(PyObject *self, PyObject *args)
{
    ...
    get_RV_line_by_line_Bouchy_with_offset_second_order_deriv(&para0, &para1, &para2, &RV, &sig_RV, &constant, &chi2, freq_line, wavelength, sig_spectrum_raw, spectrum, n2, wavelength_master, spectrum_master, n3, detector_noise,vrad_ccf, wavelength_master_shift, derivative_spectrum_master, derivative2_spectrum_master,spectrum_master_same,derivative_spectrum_master_same,derivative2_spectrum_master_same);
