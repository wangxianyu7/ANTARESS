...
from utils_plots import custom_axis,autom_x_tick_prop,autom_y_tick_prop,stackrel,scaled_title
...
from ANTARESS_routines import calc_pl_coord,sub_calc_plocc_prop,def_plotorbite,LD_coeff_func,orb_motion_theoRV,return_FWHM_inst,conv_inclinedStarFrame_to_StarFrame,\
                                ...
                                voigt,dgauss,corr_line_prof_gen,gain_piecewise_func,spec_dopshift,calc_Isurf_grid,calc_st_sky,def_contacts,conv_phase
...
def ANTARESS_plot_functions(system_param,plot_dic,data_dic,gen_dic,coord_dic,theo_dic,data_prop,glob_fit_dic,PropAtm_fit_dic):
    ...
    plot_settings = ANTARESS_plot_settings(plot_dic,gen_dic,data_dic)
    ...
    '''
    if (plot_dic['occulted_regions']!='') or any('map_' in key for key in list(plot_dic.keys())) or (plot_dic['prop_loc']!='') or (plot_dic['prop_raw']!='') or (plot_dic['input_LC']!='') or (plot_dic['pca_ana']!=''): 

        #Contacts
        contact_phases={}
        for pl_loc in gen_dic['studied_pl']:
            contact_phases[pl_loc]=def_contacts(data_dic['DI']['system_prop']['achrom'][pl_loc][0],system_param[pl_loc],plot_dic['stend_ph'],system_param['star'])
            T14_pl = (contact_phases[pl_loc][3]-contact_phases[pl_loc][0])*system_param[pl_loc]['period']
            print('Numerical T14['+str(pl_loc)+']='+"{0:.2f}".format(T14_pl*24.)+' h')                  
        ...
        #    - in achromatic mode
        def calc_occ_plot(inst,vis,genpar_instvis,param_loc,args,system_param_loc):
            ...
            theo_dic_loc = deepcopy(theo_dic)
            theo_dic_loc['d_oversamp']={}
            system_prop_loc = deepcopy(data_dic['DI']['system_prop'])
            par_list = ['rv','CB_RV','mu','xp_abs','r_proj','y_st','lat']
            ...
            for pl_loc in gen_dic['studied_pl']:
                ...
                xp_HR,yp_HR,_,Dprojplanet_HR,_,_,_,ecl_pl_HR = calc_pl_coord(system_param_loc[pl_loc]['ecc'],system_param_loc[pl_loc]['omega_rad'],system_param_loc[pl_loc]['aRs'],system_param_loc[pl_loc]['inclin_rad'],phase_pl,system_prop_loc['achrom'][pl_loc][0],system_param_loc[pl_loc]['lambda_rad'],system_param['star'])
    ...
    if any('map_' in key for key in list(plot_dic.keys())):
        ...
        for pl_loc in gen_dic['studied_pl']:
            ...
            if ('CCF_mask_wav' in data_dic['Atm']):
                wlines_star_HR = data_dic['Atm']['CCF_mask_wav']*np.sqrt(1. + (rv_pl_HR[pl_loc][:,None]/c_light) )/np.sqrt(1. - (rv_pl_HR[pl_loc][:,None]/c_light) ) 
        ...
        def sub_2D_map(map_mod,save_res_map,plot_options):
            if map_mod=='map_DI_prof':
                data_type_gen = 'DI'
                path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/DI_data/Maps/'
            elif map_mod=='map_DIbin':
                data_type_gen = 'DI'
                path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Binned_DI_data/Maps/'
            elif map_mod=='map_res_prof':path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Res_data/Maps/'
            elif map_mod=='map_Intr_prof':
                data_type_gen = 'Intr'
                if plot_options['aligned']:path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Aligned_Intr_data/Maps/'
                else:path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Intr_data/Maps/'
            elif map_mod in ['map_Intr_prof_est','map_Intr_prof_res']:
                data_type_gen = 'Intr'
                path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Theo_Intr_data/Maps/'                
            elif map_mod=='map_pca_prof':
                path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Res_data/PCA/Maps_theo/' 
            elif map_mod=='map_Intrbin':
                data_type_gen = 'Intr'
                path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Binned_intr_data/Maps/'
            elif map_mod=='map_Intr_1D':
                data_type_gen = 'Intr'
                path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/1D_Intr_data/Maps/'
            elif map_mod in ['map_Atm_prof']:
                data_type_gen = 'Atm'
                if plot_options['aligned']:path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Aligned_atm_data/Maps/'
                else:path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Atm_data/Maps/'                                
            elif map_mod=='map_Atmbin':
                data_type_gen = 'Atm'
                path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Binned_atm_data/Maps/'
            elif map_mod=='map_Atm_1D':
                data_type_gen = 'Atm'
                path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/1D_Atm_data/Maps/'
            ...
            for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options['visits_to_plot'].keys())):             
                ...
                for ivis,vis in enumerate(np.intersect1d(list(data_dic[inst].keys())+['binned'],plot_options['visits_to_plot'][inst])):                 
                    ...
                    #Data to be plotted 
                    if map_mod=='map_DI_prof':
                        iexp_plot = np.arange(data_dic[inst][vis]['n_in_visit'],dtype=int)
                        iexp_orig = iexp_plot
                        sp_mod = data_dic['DI']['type'][inst]
                        if plot_options['step']=='corr':
                        ...
                        data_path_all = [data_path+str(iexp)+'.npz' for iexp in iexp_plot]
                    elif map_mod=='map_DIbin':
                        sp_mod = data_dic['DI']['type'][inst]                          
                        restframe='star'
                        data_bin = np.load(gen_dic['save_data_dir']+'DIbin_data/'+inst+'_'+vis+'_'+plot_options['dim_plot']+'_add.npz',allow_pickle=True)['data'].item()
                        iexp_plot = np.arange(data_bin['n_exp'],dtype=int)     
                        data_path_all = [gen_dic['save_data_dir']+'DIbin_data/'+inst+'_'+vis+'_'+plot_options['dim_plot']+str(iexp)+'.npz' for iexp in iexp_plot]                  
                    elif map_mod=='map_res_prof': 
                        sp_mod = data_dic['Res']['type'][inst]
                        restframe='star'                        
                        if (sp_mod=='CCF') and plot_options['plot_theoRV']:
                            RV_plocc = (np.load(gen_dic['save_data_dir']+'Introrig_prop/PlOcc_Prop_'+inst+'_'+vis+'.npz',allow_pickle=True)['data'].item())['achrom']['rv'][0]
                        iexp_plot = data_dic['Res'][inst][vis]['idx_to_extract']  
                        iexp_orig = iexp_plot
                        data_path_all = [data_vis['proc_Res_data_paths']+str(iexp)+'.npz' for iexp in iexp_plot]
                    elif map_mod=='map_Intr_prof': 
                        ...
                        data_path_all = [data_path+str(iexp)+'.npz' for iexp in iexp_plot]
                    elif map_mod in ['map_Intr_prof_est','map_Intr_prof_res','map_pca_prof']: 
                        sp_mod = data_dic['Res']['type'][inst]
                        restframe='star'     
                        if map_mod in ['map_Intr_prof_est','map_Intr_prof_res']:
                            if gen_dic['loc_data_corr']:
                                iexp_plot = np.load(gen_dic['save_data_dir']+'Loc_estimates/'+plot_options['mode_loc_data_corr']+'/'+inst+'_'+vis+'_add.npz',allow_pickle=True)['data'].item()['idx_est_loc']                                        
                                iexp_orig = gen_dic[inst][vis]['idx_in2exp'][iexp_plot]
                            else:
                                iexp_plot = np.arange(data_dic[inst][vis]['n_in_tr'])
                                iexp_orig = np.array(gen_dic[inst][vis]['idx_in'])
                            data_path_all = [gen_dic['save_data_dir']+'Loc_estimates/'+plot_options['mode_loc_data_corr']+'/'+inst+'_'+vis+'_in'+str(iexp)+'.npz' for iexp in iexp_plot]
                            if (map_mod=='map_Intr_prof_res'):
                                ...
                                if plot_options['show_outres']:
                                    ...
                                    data_path_all+=[gen_dic['save_data_dir']+'Res_data/'+inst+'_'+vis+'_'+str(iexp)+'.npz' for iexp in iexp_out]
                        ...
                        iexp_plot = np.arange(data_bin['n_exp'],dtype=int) 
                        data_path_all = [gen_dic['save_data_dir']+'Intrbin_data/'+inst+'_'+vis+'_'+plot_options['dim_plot']+str(iexp)+'.npz' for iexp in iexp_plot]                   
                    elif map_mod=='map_Atm_prof': 
                        sp_mod = data_dic['Atm']['type'][inst]
                        if plot_options['aligned']:
                            suff='_pl' 
                            restframe='planet'
                        else:
                            suff=''
                            restframe='star'
                        iexp_plot = data_dic['Atm'][inst][vis]['idx_def']                       
                        if (plot_options['pl_atm_sign']=='Absorption'):iexp_orig = np.array(gen_dic[inst][vis]['idx_in'])[iexp_plot]                      
                        elif (plot_options['pl_atm_sign']=='Emission'):iexp_orig = iexp_plot
                        data_path_all = [gen_dic['save_data_dir']+'Atm_data'+suff+'/'+plot_options['pl_atm_sign']+'/'+inst+'_'+vis+'_'+str(iexp)+'.npz' for iexp in iexp_plot]
                    elif map_mod=='map_Atmbin': 
                        sp_mod = data_dic['Atm']['type'][inst]             
                        if gen_dic['align_Atm']:restframe='planet'
                        else:restframe='star'
                        data_bin = np.load(gen_dic['save_data_dir']+'Atmbin_data/'+plot_options['pl_atm_sign']+'/'+inst+'_'+vis+'_add.npz',allow_pickle=True)['data'].item()
                        iexp_plot = np.arange(data_bin['n_exp'],dtype=int) 
                    ...
                    if plot_options['theoRV_HR']:
                        theo_HR_prop_plocc = calc_occ_plot(inst,vis,{},params,{},deepcopy(system_param))
                    if plot_options['theoRV_HR_align']:
                        system_param_align = deepcopy(system_param)
                        system_param_align[pl_ref]['lambda_rad'] = 0.
                        theo_HR_prop_plocc_align = calc_occ_plot(inst,vis,{},params,{},system_param_align)
               
                    #Retrieving data
                    nexp_plot = len(iexp_plot)
                    if 'bin' in map_mod:dim_exp_plot = data_bin['dim_exp']
                    else:dim_exp_plot = data_dic[inst][vis]['dim_exp']
                    dim_all_plot = [nexp_plot]+dim_exp_plot
                    low_sp_map = np.zeros(dim_all_plot)*np.nan
                    high_sp_map = np.zeros(dim_all_plot)*np.nan
                    var_map = np.zeros(dim_all_plot)*np.nan
                    ...
                    for isub,(iexp_plot_loc,iexp_orig_loc,data_path_exp) in enumerate(zip(iexp_plot,iexp_orig,data_path_all)):
                        if map_mod in ['map_Intr_prof_est','map_Intr_prof_res','map_pca_prof']:
                            ...
                            #                  = - Pert(w,t,v)*Cref(wband,v)                     
                            elif (map_mod=='map_Intr_prof_res'):
                                ...
                                if (inout_flag[isub]=='in'):    
                                    ...
                                    if plot_options['cont_only']:
                                        loc_flux_est = data_dic['Intr']['mean_cont']*loc_flux_scaling_exp
                                        
                                    #Correct for full local stellar profile
                                    else:
                                        data_load_est = np.load(data_path_exp,allow_pickle=True)['data'].item()
                                        loc_flux_est = data_load_est['flux']
                                    
                                    loc_flux_meas = loc_flux_scaling_exp*data_load['flux']
                                    var_map[isub] =  (loc_flux_meas - loc_flux_est)/(1. - loc_flux_scaling_exp)
                                 
                                #Out-of-transit profile
                                else:
                                    data_load = np.load(data_path_exp,allow_pickle=True)['data'].item()
                                    var_map[isub]=data_load['flux']
                            ...
                            data_load = np.load(data_path_exp,allow_pickle=True)['data'].item()
                            var_map[isub] = sc_fact*data_load['flux']
                    ...
                    if map_mod in ['map_DIbin','map_Intr_prof','map_Intrbin','map_Intr_1D','map_Intr_prof_est','map_Intr_prof_res','map_pca_prof','map_Atmbin','map_Atm_1D']: 
                        ...
                        if ('bin' in map_mod):
                            ...
                            if ordi_name=='phase':     
                                ...
                                transit_prop_nom = (np.load(gen_dic['save_data_dir']+'Introrig_prop/PlOcc_Prop_'+inst+'_'+vis+'.npz',allow_pickle=True)['data'].item())['achrom'][0]                           
                    ...
                    if 'bin' not in map_mod:iexp_orig = iexp_orig[isort] 
                    ...
                    if ordi_olap:
                        ...
                        for iexp,delta_exp in zip( range(1,nexp_plot),delta_ordi_tab):
                            if delta_exp>0:
                                low_ordi_tab[iexp::]+=delta_exp
                                high_ordi_tab[iexp::]+=delta_exp
                    ...
                    for iord in order_list:
                        ...
                        #Axis                           
                        plt.ioff()  
                        if plot_options['reverse_2D']: plt.figure(figsize=(10, 6))
                        ...
                        if (inst in plot_options['v_range_all']) and (vis in plot_options['v_range_all'][inst]):v_range=np.array(plot_options['v_range_all'][inst][vis])*sc_fact
                        else:
                            v_range=np.array([np.nanmin(var_loc),np.nanmax(var_loc)])
                            if sp_mod == 'CCF':print('      range for color table:',"{0:.5f}".format(v_range[0]),' ; ',"{0:.5f}".format(v_range[1]))
                        ...
                        if plot_options['reverse_2D']:
                            ...
                            if (inst in plot_options['y_range_all']) and (vis in plot_options['y_range_all'][inst]):
                        ...
                        if False in cond_ph_in:
                            if (True not in cond_ph_in):stop('No data in selected '+ordi_name+' range')
                        ...
                        if False in cond_sp_in: 
                            if (True not in cond_sp_in):stop('No data in selected spectral range')
                        ...
                        if plot_options['plot_zermark']:
                            if plot_options['reverse_2D']:
                                ...
                                if (sp_mod == 'CCF') :plt.plot([0,0],y_range_loc,color='black',linestyle='--',lw=0.8,zorder=10)
                            else:
                                if ordi_name == 'phase':plt.plot([0,0],y_range_loc,color='black',linestyle='--',lw=0.8,zorder=10)
                                if (sp_mod == 'CCF') :plt.plot(x_range_loc,[0,0],color='black',linestyle='--',lw=0.8,zorder=10)                            
                                    
                        #Contacts for planets transiting in the visit
                        if ordi_name == 'phase':
                            lw_cont = 2.5
                            if plot_options['cmap'] in ['afmhot_r']:
                                if map_mod in ['map_Intr_prof_res','map_pca_prof']:
                                    col_loc='lime'
                        ...
                        if (restframe=='star') and (map_mod in ['map_DI_prof','map_res_prof']) and ('spec' in sp_mod) and plot_options['CCF_mask_star']:
                            col_loc='white'
                            if plot_options['reverse_2D']:
                                cond_lines_in = np_where1D((gen_dic['CCF_mask_wav'][inst]>=y_range_loc[0]) & (gen_dic['CCF_mask_wav'][inst]<=y_range_loc[1])) 
                                for iline in cond_lines_in:plt.plot(x_range_loc,np.repeat(gen_dic['CCF_mask_wav'][inst][iline],2),color=col_loc,linestyle='-',lw=1.5,zorder=10)
                            else:  
                                cond_lines_in = np_where1D((gen_dic['CCF_mask_wav'][inst]>=x_range_loc[0]) & (gen_dic['CCF_mask_wav'][inst]<=x_range_loc[1])) 
                                for iline in cond_lines_in:plt.plot(np.repeat(gen_dic['CCF_mask_wav'][inst][iline],2),y_range_loc,color=col_loc,linestyle='-',lw=1.5,zorder=10) 
                            

                        #------------------------------------  
                  
                        #Planetary orbital model
                        if (restframe in ['star','planet']) and (ordi_name == 'phase') and (map_mod in ['map_DI_prof','map_res_prof','map_Intr_prof','map_Intr_prof_est','map_Intr_prof_res','map_Atm_prof']):
                            ...
                            #Colors
                            if map_mod=='map_Atm_prof': col_loc='limegreen'
                            ...
                            if (sp_mod == 'CCF'):                            
                                ...
                                if (plot_options['theoRVpl_HR']):                                    
                                    if (restframe=='star'):rv_mod_loc = rv_pl_HR[pl_ref]
                                ...
                                if (plot_options['plot_plexc']) and (data_dic['Atm']['exc_plrange']): 
                                    if (restframe=='star'):plrange_loc = plrange_star_HR[pl_ref]
                            ...
                            elif ('spec' in sp_mod) and ('CCF_mask_wav' in data_dic['Atm']): 
                        ...
                        if (sp_mod == 'CCF'):
                            ...
                            if (map_mod in ['map_res_prof','map_Intr_prof_est','map_Intr_prof_res','map_Intrbin']) or ((map_mod in ['map_Intr_prof']) and (not plot_options['aligned'])):
                                ...
                                if plot_options['theoRV_HR'] or plot_options['theoRV_HR_align']:
                                    ...
                                    # save_path = '/Users/bourrier/Travaux/ANTARESS/En_cours/HAT_P33b_Plots/Intr_data/Maps/RMR_fits/HAT_P33_RVsurf_CB1.dat'
                            ...
                            if (plot_options['plot_measRV'] in ['det','all']):
                                ...
                                for iexp in range(data_dic[inst][vis]['n_in_visit']):
                                    ...
                                    if (map_mod=='map_res_prof') and ('prof_fit_dic' in data_dic['Res'][inst][vis]) and (iexp in gen_dic[inst][vis]['idx_in']): 
                                        CCF_fit_loc = data_dic['Res'][inst][vis]['prof_fit_dic'][i_in] 
                                        plot_rv = True
                                    if map_mod=='map_Atm_prof':
                                        ...
                                        if (restframe=='star'):RV_rest = coord_dic[inst][vis]['rv_pl'][iexp] 
                                        plot_rv = True
                        ...
                        if plot_options['reverse_2D']:
                            x_title=ordi_title                          
                            if (sp_mod == 'CCF'):
                                y_title='Velocity in '+restframe+' rest frame (km s$^{-1}$)'
                            elif ('spec' in sp_mod):
                                y_title='Wavelength in '+restframe+' frame (A)' 
                                ymajor_form='%i'  
                        else:    
                            if (sp_mod == 'CCF'):
                                x_title='Velocity in '+restframe+' rest frame (km s$^{-1}$)'
                            elif ('spec' in sp_mod):
                                x_title='Wavelength in '+restframe+' frame (A)'        
                            y_title=ordi_title                           
                        ...
                        if (map_mod in ['map_Intr_prof','map_Intr_1D','map_Intr_prof_est','map_Intr_prof_res','map_pca_prof','map_Atm_prof','map_Atm_1D']) and (ordi_name!='phase'):
                        ...
                        if map_mod in ['map_DIbin','map_DI_prof','map_res_prof','map_Intr_prof','map_Intr_prof_est','map_Intr_prof_res','map_pca_prof','map_Intrbin','map_Intr_1D']:cbar_txt='flux'
                        elif map_mod in ['map_Atm_prof','map_Atmbin','map_Atm_1D']:cbar_txt=plot_options['pl_atm_sign']
        ...
        #                        cbar.ax.set_xticklabels(labels)
                        ...
                        add_str='' if sp_mod == 'CCF' else '_iord'+str(iord)                          
                        if map_mod=='map_Intr_prof_est':add_str+='_'+plot_options['mode_loc_data_corr']
                        if map_mod=='map_Intr_prof_res':add_str+='_'+plot_options['mode_loc_data_corr']+'_res'
                        if ('bin' in map_mod):add_str+='_'+plot_options['dim_plot'] 
                        plt.savefig(path_loc+sp_mod+'_'+inst+'_'+vis+add_str+'.'+save_res_map)                        
                        plt.close() 
    ...
    '''
    Sub-functions to plot individual profiles and the residual from their fits
        - for disk-integrated, local, intrinsic, atmospheric data
    '''
    def sub_plot_CCF(plot_options,plot_mod,plot_ext):
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options['visits_to_plot'].keys())): 
            ...
            for vis in np.intersect1d(list(data_dic[inst].keys())+['binned'],plot_options['visits_to_plot'][inst]): 
                ...
                #Reference planet for the visit
                if (plot_mod in ['DI_prof','DI_prof_res','CCFloc','CCFintr','CCFintr_res','CCFatm','CCFatm_res']):
                    pl_ref=plot_options['pl_ref'][inst][vis]

                #Actions specific to each CCF mode
                txt_conv = ''
                if plot_mod in ['DI_prof','DI_prof_res']: 
                    data_type = data_dic['DI']['type'][inst]             
                    if ('spec' in gen_dic['type'][inst]) and (data_type=='CCF'):txt_conv='CCFfromSpec'         
                    if plot_mod=='DI_prof':path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/CCFraw/'+inst+'_'+vis+'_Indiv_CCFraw/'
                    if plot_mod=='DI_prof_res':path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/CCFraw/'+inst+'_'+vis+'_Indiv_CCFraw_res/'
                    ...
                    #Upload fit results
                    if gen_dic['fit_DIbin'] or gen_dic['fit_DIbinmultivis']:
                        CCF_fit_vis=np.load(gen_dic['save_data_dir']+'DIbin_prop/'+inst+'_'+vis+'.npz',allow_pickle=True)['data'].item()                
                                           
                    if plot_mod=='CCF_DIbin':suff = '_Indiv_CCF_DIbin/'
                    elif plot_mod=='CCF_DIbin_res':suff = '_Indiv_CCF_DIbin_res/'
                        
                    path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Binned_DI_data/'+inst+'_'+vis+suff+plot_options['dim_plot']+'/'
                    data_bin = np.load(gen_dic['save_data_dir']+'DIbin_data/'+inst+'_'+vis+'_'+plot_options['dim_plot']+'_add.npz',allow_pickle=True)['data'].item()
                    iexp_plot = np.arange(data_bin['n_exp'],dtype=int)                     
                    
                elif plot_mod in ['CCFloc','CCFintr','CCFintr_res']:                   

                    #Upload fit results
                    if gen_dic['fit_Intr'] and (plot_options['fit_type']=='indiv'):
                        CCF_fit_vis=np.load(gen_dic['save_data_dir']+'Introrig_prop/'+inst+'_'+vis+'.npz',allow_pickle=True)['data'].item()
                    elif (plot_options['fit_type']=='global') and (inst in glob_fit_dic['IntrProf']['idx_in_fit']) and (vis in glob_fit_dic['IntrProf']['idx_in_fit'][inst]):
                        CCF_fit_vis=(np.load(gen_dic['save_data_dir']+'Joined_fits/Intr_prof/IntrProf_fit_'+inst+'_'+vis+'.npz',allow_pickle=True)['data'].item())['prof_fit_dic']                 
                        
                    if plot_mod=='CCFloc':
                        data_type = data_dic['Res']['type'][inst]    
                        if ('spec' in gen_dic['type'][inst]) and (data_type=='CCF'):txt_conv='CCFfromSpec/' 
                    ...
                    if plot_mod in ['CCFintr','CCFintr_res']: 
                        data_type = data_dic['Intr']['type'][inst]     
                        if ('spec' in gen_dic['type'][inst]) and (data_type=='CCF'):txt_conv='CCFfromSpec/'  
                        
                        if plot_mod=='CCFintr':suff = '_Indiv_CCFintr/'
                        elif plot_mod=='CCFintr_res':suff = '_Indiv_CCFintr_res/'
                        ...
                        iexp_plot = data_dic['Intr'][inst][vis]['idx_def']
                        iexp_orig = gen_dic[inst][vis]['idx_in']
                    
                elif plot_mod in ['CCF_Intrbin','CCF_Intrbin_res']:  
                    data_type = data_dic['Intr']['type'][inst]                  
                  
                    #Upload fit results
                    if gen_dic['fit_Intrbin'] or gen_dic['fit_Intrbinmultivis']:
                        CCF_fit_vis=np.load(gen_dic['save_data_dir']+'Intrbin_prop/'+inst+'_'+vis+'.npz',allow_pickle=True)['data'].item()                
                       
                    if plot_mod=='CCF_Intrbin':suff = '_Indiv_CCF_Intrbin/'
                    elif plot_mod=='CCF_Intrbin_res':suff = '_Indiv_CCF_Intrbin_res/'
                        
                    path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Binned_intr_data/'+inst+'_'+vis+suff+plot_options['dim_plot']+'/'
                    data_bin = np.load(gen_dic['save_data_dir']+'Intrbin_data/'+inst+'_'+vis+'_'+plot_options['dim_plot']+'_add.npz',allow_pickle=True)['data'].item()
                    iexp_plot = np.arange(data_bin['n_exp'],dtype=int)                    

                elif plot_mod in ['CCFatm','CCFatm_res']:   
                    data_type = data_dic['Atm']['type'][inst] 
                    if ('spec' in gen_dic['type'][inst]) and (data_type=='CCF'):txt_conv='CCFfromSpec/' 
                    if plot_mod=='CCFatm':suff = '_Indiv_CCF/'
                    elif plot_mod=='CCFatm_res':suff = '_Indiv_CCF_res/'

                    #Upload fit results
                    if (plot_options['fit_type']=='indiv') and gen_dic['ana_atm']:
                        CCF_fit_vis=(np.load(gen_dic['save_data_dir']+'Atmorig_prop/'+plot_options['pl_atm_sign']+'/'+inst+'_'+vis+'.npz',allow_pickle=True)['data'].item())['prof_fit_dic']
                    elif (plot_options['fit_type']=='global') and (gen_dic['fit_atm_all']) and (inst in PropAtm_fit_dic['idx_in_glob_fit']) and (vis in PropAtm_fit_dic['idx_in_glob_fit'][inst]):
                        CCF_fit_vis=(np.load(gen_dic['save_data_dir']+'Atmorig_prop/'+plot_options['pl_atm_sign']+'/Fit/Atm_glob_fit_'+inst+'_'+vis+'.npz',allow_pickle=True)['data'].item())['prof_fit_dic'] 
                        
                    if plot_options['aligned']:
                        suff_frame='_pl' 
                        if ((plot_mod=='CCFatm_res') or (plot_options['plot_line_model'] and (plot_mod in ['CCFatm']))):stop('Remove "aligned" options to overplot model')
                    else:suff_frame=''  
                    iexp_plot = data_dic['Atm'][inst][vis]['idx_def']
                    if (plot_options['pl_atm_sign']=='Absorption'):iexp_orig = np.array(gen_dic[inst][vis]['idx_in'])[iexp_plot]                      
                    elif (plot_options['pl_atm_sign']=='Emission'):iexp_orig = iexp_plot
                    path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Atm_data'+suff_frame+'/'+plot_options['pl_atm_sign']+'/'+inst+'_'+vis+suff

                elif plot_mod in ['CCF_Atmbin','CCF_Atmbin_res']: 
                    data_type = data_dic['Atm']['type'][inst]                    
                    
                    #Upload fit results
                    if gen_dic['ana_Atmbin']:
                        CCF_fit_vis=np.load(gen_dic['save_data_dir']+'Atmbin_prop/'+plot_options['pl_atm_sign']+'/'+inst+'_'+vis+'.npz',allow_pickle=True)['data'].item()                
                                           
                    if plot_mod=='CCF_Atmbin':suff = '_Indiv_CCF_Atmbin/'
                    elif plot_mod=='CCF_Atmbin_res':suff = '_Indiv_CCF_Atmbin_res/'
                        
                    path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Binned_atm_data/'+inst+'_'+vis+suff+plot_options['pl_atm_sign']+'/'+plot_options['dim_plot']+'/'
                    data_bin = np.load(gen_dic['save_data_dir']+'Atmbin_data/'+plot_options['pl_atm_sign']+'/'+inst+'_'+vis+'_'+plot_options['dim_plot']+'_add.npz',allow_pickle=True)['data'].item()
                    iexp_plot = np.arange(data_bin['n_exp'],dtype=int)  

                if (plot_mod in ['CCFatm','CCF_Atmbin']) and (plot_options['print_mes']):
                    if (plot_mod=='CCFatm'):data_type='Atmorig'
                    if (plot_mod=='CCF_Atmbin'):data_type='Atmbin'
                    data_meas = np.load(gen_dic['save_data_dir']+data_type+'_prop/'+data_dic['Atm']['pl_atm_sign']+'/'+inst+'_'+vis+'.npz',allow_pickle=True)['data'].item()

                if (plot_ext in ['ps','png','pdf']) and (not os_system.path.exists(path_loc)):os_system.makedirs(path_loc) 

                #Plot profile for each exposure
                for iexp in iexp_plot:
                    plot_loc_star = False
                    if plot_mod in ['DI_prof','DI_prof_res','CCFloc','CCFintr','CCFintr_res','CCFatm','CCFatm_res']:iexp_or=iexp_orig[iexp]
                    ...
                    #Disk-integrated master CCF
                        ...
                        data_path = gen_dic['save_data_dir']+'DIbin_data/'+inst+'_'+vis+'_'+plot_options['dim_plot']+str(iexp)+'.npz' 

                    #Local CCF
                    elif plot_mod in ['CCFloc']:                      
                        data_path = gen_dic['save_data_dir']+'Res_data/'+txt_conv+inst+'_'+vis+'_'+str(iexp)+'.npz'

                        #Estimates for local stellar profile
                        if plot_options['estim_loc']:
                            i_in = gen_dic[inst][vis]['idx_exp2in'][iexp_or]
                            if (i_in>-1) and (cond_in_intr_def[i_in]):
                                data_loc_star = np.load(gen_dic['save_data_dir']+'Loc_estimates/'+plot_options['mode_loc_data_corr']+'/'+inst+'_'+vis+'_in'+str(i_in)+'.npz',allow_pickle=True)['data'].item()   
                                plot_loc_star = True
                    ...
                    #Binned intrinsic CCF
                        ...
                        data_path = gen_dic['save_data_dir']+'Intrbin_data/'+inst+'_'+vis+'_'+plot_options['dim_plot']+str(iexp)+'.npz' 

                    #Atmospheric CCF
                    elif plot_mod in ['CCFatm','CCFatm_res']:
                        if (plot_mod=='CCFatm') and (plot_options['aligned']):suff_frame='_pl'
                        else:suff_frame=''
                        data_path = gen_dic['save_data_dir']+'Atm_data'+suff_frame+'/'+plot_options['pl_atm_sign']+'/'+txt_conv+inst+'_'+vis+'_'+str(iexp)+'.npz' 

                    #Binned atmospheric CCF
                    elif plot_mod in ['CCF_Atmbin','CCF_Atmbin_res']:
                        data_path = gen_dic['save_data_dir']+'Atmbin_data/'+plot_options['pl_atm_sign']+'/'+inst+'_'+vis+'_'+plot_options['dim_plot']+str(iexp)+'.npz' 

                    data_load = np.load(data_path,allow_pickle=True)['data'].item()
                    ...
                    if (plot_mod in ['DI_prof','DI_prof_res']):     
                        ...
                        if (gen_dic['fit_DI']):
                            cond_mod=True
                            cond_fit_exp=CCF_fit_vis['cond_def_fit_all'][iexp]
                            cond_cont_exp=CCF_fit_vis['cond_def_cont_all'][iexp]
                            CCF_prop_exp=CCF_fit_vis[iexp]
                        if (plot_mod=='DI_prof_res') and (cond_mod==False):do_plot=False
                       
                    if (plot_mod in ['CCF_DIbin','CCF_DIbin_res']):

                        #Fit to individual profiles
                        if gen_dic['fit_DIbin'] or gen_dic['fit_DIbinmultivis']:    
                            cond_mod=True 
                            cond_fit_exp=CCF_fit_vis['cond_def_fit_all'][iexp] 
                            cond_cont_exp=CCF_fit_vis['cond_def_cont_all'][iexp]
                            CCF_prop_exp=CCF_fit_vis[iexp]
                        if (plot_mod=='CCF_DIbin_res') and (cond_mod==False):do_plot=False                        

                    if (plot_mod == 'CCFloc') and (gen_dic['fit_Intr']) and (iexp in gen_dic[inst][vis]['idx_in']):
                        cond_mod=True 
                        i_in = gen_dic[inst][vis]['idx_exp2in'][iexp]
                        cond_fit_exp=CCF_fit_vis['cond_def_fit_all'][i_in]   
                        cond_cont_exp=CCF_fit_vis['cond_def_cont_all'][i_in]
                        CCF_prop_exp=CCF_fit_vis[i_in]
                    ...
                    if (plot_mod in ['CCFintr','CCFintr_res']): 
                        ...
                        #Fit to global profiles
                        if (gen_dic['fit_IntrProf']) and (plot_options['fit_type']=='global') and (iexp in CCF_fit_vis):     
                            cond_mod=True  
                            CCF_prop_exp=CCF_fit_vis[iexp]
                            cond_fit_exp=CCF_prop_exp['cond_def_fit'] 
                            cond_cont_exp=CCF_prop_exp['cond_def_cont'] 
                            
                        if (plot_mod=='CCFintr_res') and (cond_mod==False):do_plot=False

                    if (plot_mod in ['CCF_Intrbin','CCF_Intrbin_res']): 
                        ...
                        if (plot_mod=='CCF_Intrbin_res') and (cond_mod==False):do_plot=False

                    if (plot_mod in ['CCFatm','CCFatm_res']): 
                        
                        #Fit to individual profiles
                        if (gen_dic['ana_Atm']) and (plot_options['fit_type']=='indiv'):    
                            cond_mod=True 
                            cond_fit_exp=CCF_fit_vis['cond_def_fit_all'][iexp]   
                            cond_cont_exp=CCF_fit_vis['cond_def_cont_all'][iexp] 
                            CCF_prop_exp=CCF_fit_vis[iexp]
                        ...
                        #Fit to global profiles
                        if (gen_dic['fit_atm_all']) and (plot_options['fit_type']=='global') and (iexp in CCF_fit_vis):   
                            cond_mod=True 
                            CCF_prop_exp=CCF_fit_vis[iexp]
                            cond_fit_exp=CCF_prop_exp['cond_def_fit'] 
                            cond_cont_exp=CCF_prop_exp['cond_def_cont'] 
                            
                        if (plot_mod=='CCFatm_res') and (cond_mod==False):do_plot=False                            

                    if (plot_mod in ['CCF_Atmbin','CCF_Atmbin_res']): 
                        
                        #Fit to individual profiles
                        if gen_dic['fit_Atmbin'] or gen_dic['fit_Atmbinmultivis']:   
                            cond_mod=True 
                            cond_fit_exp=CCF_fit_vis['cond_def_fit_all'][iexp] 
                            cond_cont_exp=CCF_fit_vis['cond_def_cont_all'][iexp] 
                            CCF_prop_exp=CCF_fit_vis[iexp]
                      
                        if (plot_mod=='CCF_Atmbin_res') and (cond_mod==False):do_plot=False
                    ...
                    if do_plot:                        
                        ...
                        x_range_loc=np.array(plot_options['x_range']) if plot_options['x_range'] is not None else np.array([vel_CCF_tab[0]-5.,vel_CCF_tab[-1]+5.])
                        dx_range=x_range_loc[1]-x_range_loc[0]
                   
                        #Plot CCF
                        if plot_mod in ['DI_prof','CCFloc','CCF_DIbin','CCFintr','CCF_Intrbin','CCFatm','CCF_Atmbin']:
                            marker=''
 
                            var_loc = sc_fact*CCF_exp/norm_exp
                            if plot_options['plot_err']:plt.errorbar(vel_CCF_tab,var_loc,yerr = sc_fact*eCCF_exp/norm_exp,color=plot_options['color_dic'][inst][vis],linestyle='',lw=plot_options['lw_plot'],marker=None,alpha=plot_options['alpha_err']) 
                            plt.plot(vel_CCF_tab, var_loc,color=plot_options['color_dic'][inst][vis],linestyle=plot_options['ls_plot'],lw=plot_options['lw_plot'],marker=marker,markersize=plot_options['markersize'],alpha=plot_options['alpha_symb'],drawstyle=plot_options['drawstyle'],zorder=10)   
                            y_range_loc=sc_fact*np.array(plot_options['y_range']) if plot_options['y_range'] is not None else np.array([0.9*np.nanmin(var_loc),1.1*np.nanmax(var_loc)])
                            dy_range=y_range_loc[1]-y_range_loc[0]

                            #Plotting estimate for local stellar profile
                            if plot_loc_star:
                                plt.plot(data_loc_star['cen_bins'][0],sc_fact*data_loc_star['flux'][0],color='red',linestyle='-',lw=plot_options['lw_plot'],rasterized=plot_options[key_plot]['rasterized'],zorder=2) 
    
                            #Level null
                            if ((plot_mod=='CCFloc') and (iexp_or in gen_dic[inst][vis]['idx_out'])) or (plot_mod=='CCFatm'):
                                plt.plot(x_range_loc,[0.,0.],linestyle='-',color='black',lw=plot_options['lw_plot'])  
                            ...
                            if plot_options['plot_line_model'] and cond_mod:      
                                ...
                                if plot_options['plot_fitpix']: 
                                    plt.plot(vel_CCF_tab[cond_fit_exp], var_loc[cond_fit_exp],color='black',linestyle='',lw=0.5,marker='o',markersize=1) 
                                    
                                #Plot continuum pixels
                                #    - specific to the exposure, used to measure dispersion
                                if plot_options['plot_cont_exp']: 
                                    plt.plot(vel_CCF_tab[cond_cont_exp], var_loc[cond_cont_exp],markeredgecolor='black',linestyle='',lw=0.5,marker='d',markersize=3,markerfacecolor='none')                  
                                
                                #Plot individual model components
                                if (plot_options['plot_line_model_compo']):
                                    if ('gauss_core' in CCF_prop_exp):plt.plot(vel_CCF_tab,sc_fact*CCF_prop_exp['gauss_core']/norm_exp,color='black',linestyle='--',lw=plot_options['lw_plot'])     
                                    if ('gauss_lobe' in CCF_prop_exp):plt.plot(vel_CCF_tab,sc_fact*CCF_prop_exp['gauss_lobe']/norm_exp,color='black',linestyle='--',lw=plot_options['lw_plot'])   
                                    if ('poly_lobe' in CCF_prop_exp): plt.plot(vel_CCF_tab,sc_fact*CCF_prop_exp['poly_lobe']/norm_exp,color='black',linestyle='--',lw=plot_options['lw_plot'])    
                                    if ('core' in CCF_prop_exp): plt.plot(vel_CCF_tab,sc_fact*CCF_prop_exp['core']/norm_exp,color='black',linestyle='--',lw=plot_options['lw_plot'])    

                                #Plot measured centroid
                                #    - corresponds to rv(star/sun) for raw CCFs, and to rv(region/star) for local CCFs
                                if plot_options['plot_line_fit_rv']:plt.plot([CCF_prop_exp['rv'],CCF_prop_exp['rv']], y_range_loc,color='black',lw=plot_options['lw_plot'],linestyle=':')   
                
                            #Plot measurements 
                            if plot_options['print_mes']:                              
                                plt.text(x_range_loc[0]+0.1*dx_range,y_range_loc[1]-0.1*dy_range,'Mean signal='+"{0:.2f}".format(1e6*data_meas['int_sign'][iexp])+'+-'+"{0:.2f}".format(1e6*data_meas['e_int_sign'][iexp])+' ppm ('+"{0:.2f}".format(data_meas['R_sign'][iexp])+'$\sigma$)' ,
                                        verticalalignment='center', horizontalalignment='left',fontsize=10.,zorder=4,color='black') 
                       
                        #----------------------------------------------------------
                         
                        #Calculate and plot residuals
                        if (plot_mod in ['DI_prof_res','CCF_DIbin_res','CCFintr_res','CCF_Intrbin_res','CCFatm_res','CCF_Atmbin_res']) and cond_mod:  
                            marker=''
                            
                            #Residuals
                            var_loc = (CCF_exp - CCF_prop_exp['flux'])/norm_exp
                            evar_loc = eCCF_exp/norm_exp
                            if plot_options['plot_err']==True:plt.errorbar(vel_CCF_tab, sc_fact*var_loc,yerr = sc_fact*evar_loc,color=plot_options['color_dic'][inst][vis],linestyle='-',lw=plot_options['lw_plot'],marker=marker,markersize=plot_options['markersize'])
                            else:plt.plot(vel_CCF_tab, sc_fact*var_loc,color=plot_options['color_dic'][inst][vis],linestyle='-',lw=plot_options['lw_plot'],marker=marker,markersize=plot_options['markersize'])                            
                            y_range_loc=sc_fact*np.array(plot_options['y_range']) if plot_options['y_range'] is not None else sc_fact*np.array([-1.1*abs(np.nanmin(var_loc)),1.1*np.nanmax(var_loc)])
                            dy_range=y_range_loc[1]-y_range_loc[0]
                            ...
                            if plot_options['plot_prop']:  
                                ...
                                #Over the fitted pixels    
                                mean_res=np.mean(var_loc[cond_fit_exp])          
                                disp_from_mean=var_loc[cond_fit_exp].std()   
                                plt.plot([vel_CCF_tab[0],vel_CCF_tab[-1]],[mean_res,mean_res],linestyle='--',color='black',lw=plot_options['lw_plot'])
                                plt.text(xtxt,y_range_loc[1]-0.1*dy_range,'Mean in fitted range ='+"{0:.2e}".format(mean_res)+'+-'+"{0:.2e}".format(disp_from_mean),verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4) 
                                 
                                #Over the continuum pixels
                                mean_res_cont=np.mean(var_loc[cond_cont_exp])           
                                disp_from_mean_cont=(var_loc[cond_cont_exp]).std()   
                                plt.plot([vel_CCF_tab[0],vel_CCF_tab[-1]],[mean_res_cont,mean_res_cont],linestyle=':',color='black',lw=plot_options['lw_plot'])                 
                                plt.text(xtxt,y_range_loc[1]-0.2*dy_range,'Mean in continuum ='+"{0:.2e}".format(mean_res_cont)+'+-'+"{0:.2e}".format(disp_from_mean_cont),verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4) 
                                   
                                #Over the fitted range outside of the continuum
                                cond_fit_nocont = cond_fit_exp & np.invert(cond_cont_exp)
                                mean_res_nocont=np.mean(var_loc[cond_fit_nocont])           
                                disp_from_mean_nocont=(var_loc[cond_fit_nocont]).std()   
                                plt.plot([vel_CCF_tab[0],vel_CCF_tab[-1]],[mean_res_nocont,mean_res_nocont],linestyle=':',color='black',lw=plot_options['lw_plot'])                 
                                plt.text(xtxt,y_range_loc[1]-0.3*dy_range,'Mean in cont - fit='+"{0:.2e}".format(mean_res_nocont)+'+-'+"{0:.2e}".format(disp_from_mean_nocont),verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4) 
                
                
                        #----------------------------------------------------------
                
                        #Print properties 
                        if (plot_mod in ['DI_prof','CCF_DIbin','CCFintr','CCF_Intrbin','CCFatm','CCF_Atmbin']) and (plot_options['plot_prop']) and (cond_mod):  
                           
                            #Intrinsic and atmospheric CCFs
                            ytxt = y_range_loc[1]
                            if (plot_mod in ['CCFintr','CCF_Intrbin','CCFatm','CCF_Atmbin']): 
                            
                                #Oplot original index
                                if (plot_mod in ['CCFintr','CCFatm']): 
                                    plt.text(x_range_loc[0]+0.1*dx_range,ytxt-0.3*dy_range,'i$_\mathrm{all}$ ='+str(iexp_or),verticalalignment='center', horizontalalignment='left',fontsize=15.,zorder=4) 
    
                                #Indicate if individual CCF is detected
                                if (plot_options['fit_type']=='indiv'):
                                    txt_detection='Forced ' if (CCF_prop_exp['forced_det']) else ''                     
                                    if CCF_prop_exp['detected']:txt_detection+='Detected' 
                                    else: txt_detection+='Undetected' 
                                    if ('crit_area' in CCF_prop_exp):txt_detection+=' (R$_\mathrm{ctrst}$='+"{0:.2f}".format(CCF_prop_exp['crit_area'])+'$\sigma$)'
                                    plt.text(x_range_loc[0]+0.1*dx_range,ytxt-0.4*dy_range,txt_detection,verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4) 
                
                            #Main fit properties
                            if (plot_mod in ['DI_prof','CCF_DIbin','CCFintr','CCF_Intrbin','CCFatm','CCF_Atmbin']):
                                xtxt = np.mean(x_range_loc)+0.05*dx_range
                                ygap = -0.1*dy_range
                                if (plot_options['fit_type']=='indiv'):    
                                    plt.text(xtxt,ytxt+ygap,'BIC = '+"{0:.5f}".format(CCF_prop_exp['BIC'])+' ($\chi^2_r$ = '+"{0:.5f}".format(CCF_prop_exp['red_chi2'])+')',verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4)  
                                    plt.text(xtxt,ytxt+2*ygap,'RV ='+stackrel(CCF_prop_exp['rv'],CCF_prop_exp['err_rv'][0],CCF_prop_exp['err_rv'][1],"0:.4f")+' km s$^{-1}$',verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4)                                                 
                                    if ('ctrst' in CCF_prop_exp):plt.text(xtxt,ytxt+3*ygap,'C ='+stackrel(CCF_prop_exp['ctrst'],CCF_prop_exp['err_ctrst'][0],CCF_prop_exp['err_ctrst'][1],"0:.4f"),verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4) 
                                    if ('FWHM' in CCF_prop_exp):plt.text(xtxt,ytxt+4*ygap,'FWHM ='+stackrel(CCF_prop_exp['FWHM'],CCF_prop_exp['err_FWHM'][0],CCF_prop_exp['err_FWHM'][1],"0:.4f")+' km s$^{-1}$',verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4) 
                                    if ('FWHM_LOR' in CCF_prop_exp):plt.text(xtxt,ytxt+5*ygap,'FWHM[Lor] ='+stackrel(CCF_prop_exp['FWHM_LOR'],CCF_prop_exp['err_FWHM_LOR'][0],CCF_prop_exp['err_FWHM_LOR'][1],"0:.4f")+' km s$^{-1}$',verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4)                                                                         
                                    if ('FWHM_voigt' in CCF_prop_exp):plt.text(xtxt,ytxt+6*ygap,'FWHM[Voigt] ='+stackrel(CCF_prop_exp['FWHM_voigt'],CCF_prop_exp['err_FWHM_voigt'][0],CCF_prop_exp['err_FWHM_voigt'][1],"0:.4f")+' km s$^{-1}$',verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4)                                     
                                    if ('cont_amp' in CCF_prop_exp):
                                        disp_from_mean_cont=((CCF_exp - CCF_prop_exp['flux'])[cond_cont_exp]).std()
                                        if (plot_mod in  ['CCFintr','CCF_Intrbin']) and (data_dic['Intr']['model'][inst]=='dgauss'):
                                            plt.text(xtxt,ytxt+6.*ygap,'cont. amp ='+stackrel(CCF_prop_exp['cont_amp'],CCF_prop_exp['err_cont_amp'][0],CCF_prop_exp['err_cont_amp'][1],"0:.2e"),verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4) 
                                            plt.text(xtxt,ytxt+6.5*ygap,'           '+"{0:.2f}".format(CCF_prop_exp['cont_amp']/np.mean(CCF_prop_exp['err_cont_amp']))+'$\sigma$ ; '+"{0:.2f}".format(CCF_prop_exp['cont_amp']/disp_from_mean_cont)+'$\sigma_\mathrm{cont}$',verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4)     
                                        plt.text(xtxt,ytxt+5.*ygap,'amp ='+stackrel(CCF_prop_exp['amp'],CCF_prop_exp['err_amp'][0],CCF_prop_exp['err_amp'][1],"0:.2e"),verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4) 
                                        plt.text(xtxt,ytxt+5.5*ygap,'           '+"{0:.2f}".format(CCF_prop_exp['amp']/np.mean(CCF_prop_exp['err_amp']))+'$\sigma$ ; '+"{0:.2f}".format(CCF_prop_exp['cont_amp']/disp_from_mean_cont)+'$\sigma_\mathrm{cont}$',verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4)                                 
                                elif (plot_options['fit_type']=='global') and (plot_mod in ['CCFintr','CCFatm']):
                                    plt.text(xtxt,ytxt+2*ygap,'RV (intr.) ='+"{0:.4f}".format(CCF_prop_exp['rv'])+' km s$^{-1}$',verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4)                                                      
                                    plt.text(xtxt,ytxt+3*ygap,'ctrst (intr.) ='+"{0:.4f}".format(CCF_prop_exp['ctrst']),verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4) 
                                    plt.text(xtxt,ytxt+4*ygap,'FWHM (intr.) ='+"{0:.4f}".format(CCF_prop_exp['FWHM'])+' km s$^{-1}$',verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4) 
                                    if ('cont_amp' in CCF_prop_exp):
                                        disp_from_mean_cont=((CCF_exp - CCF_prop_exp['flux'])[cond_cont_exp]).std()
                                        if (plot_mod == 'CCFintr') and (data_dic['Intr']['model'][inst]=='dgauss'):
                                            plt.text(xtxt,ytxt+6.*ygap,'cont. amp ='+stackrel(CCF_prop_exp['cont_amp'],CCF_prop_exp['err_cont_amp'][0],CCF_prop_exp['err_cont_amp'][1],"0:.2e"),verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4) 
                                            plt.text(xtxt,ytxt+6.5*ygap,'           '+"{0:.2f}".format(CCF_prop_exp['cont_amp']/np.mean(CCF_prop_exp['err_cont_amp']))+'$\sigma$ ; '+"{0:.2f}".format(CCF_prop_exp['cont_amp']/disp_from_mean_cont)+'$\sigma_\mathrm{cont}$',verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4)     
                                        plt.text(xtxt,ytxt+5.*ygap,'amp ='+"{0:.2e}".format(CCF_prop_exp['amp'])+'+-'+"{0:.2e}".format(CCF_prop_exp['err_amp']),verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4) 
                                        plt.text(xtxt,ytxt+5.5*ygap,'           '+"{0:.2f}".format(CCF_prop_exp['amp']/CCF_prop_exp['err_amp'])+'$\sigma$ ; '+"{0:.2f}".format(CCF_prop_exp['cont_amp']/disp_from_mean_cont)+'$\sigma_\mathrm{cont}$',verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4) 

                        #----------------------------------------------------------

                        #Oplot reference velocity
                        if plot_options['plot_refvel']:
                            if (plot_mod in ['DI_prof','DI_prof_res']):refvel=data_dic['DI']['sysvel'][inst][vis]
                            else:refvel=0.
                            plt.plot([refvel,refvel], y_range_loc,color='black',lw=plot_options['lw_plot'],linestyle=':') 
                  
                        #Plot continuum level
                        if plot_options['ref_level'] and plot_mod in ['CCFintr','CCF_Intrbin']:
                            plt.plot(x_range_loc,[1.,1.],color='black',linestyle='--',lw=plot_options['lw_plot'],zorder=-1+15)  
                                                     
                        #Oplot continuum pixels from fits
                        if plot_options['plot_cont'] and cond_mod:
                            cond_cont_com  = np.all(CCF_fit_vis['cond_def_cont_all'],axis=0)
                            plt.plot(vel_CCF_tab[cond_cont_com],var_loc[cond_cont_com],color='black',linestyle='',lw=plot_options['lw_plot'],marker='d',markersize=2,zorder=10)  

                            #Shade range requested as input
                            for i_int,bd_int in enumerate(CCF_fit_vis['cont_range']):
                                plt.axvspan(bd_int[0],bd_int[1], facecolor='dodgerblue', alpha=0.1) 
                
                        #Shade area not included within fit
                        if plot_options['shade_unfit']:
                            plot_shade_range(plt.gca(),CCF_fit_vis['fit_range'],x_range_loc,x_range_loc,mode='hatch',compl=True)

                        #Shade range of planetary signal excluded for relevant exposures
                        if (data_dic['Atm']['exc_plrange']) and (plot_options['plot_plexc']):
                            if (plot_mod in ['DI_prof','DI_prof_res']) and ('DI_CCF' in data_dic['Atm']['no_plrange']):
                                for pl_loc in data_vis['transit_pl']:
                                    bd_int=data_dic['Atm']['plrange'] + coord_vis[pl_loc]['rv_pl'][iexp_or] + coord_vis['RV_star_solCDM'][iexp_or]
                                    if (bd_int[0]<1e10):plt.axvspan(bd_int[0],bd_int[1], facecolor='red', alpha=0.1,zorder=10) 
                            elif (plot_mod=='CCFloc'):
                                for pl_loc in data_vis['transit_pl']:
                                    bd_int=data_dic['Atm']['plrange'] + coord_vis[pl_loc]['rv_pl'][iexp_or]
                                    if (bd_int[0]<1e10):plt.axvspan(bd_int[0],bd_int[1], facecolor='red', alpha=0.1,zorder=10) 
                        ...
                        if plot_mod=='DI_prof':
                            xt_str='heliocentric'
                            y_title='Flux'
                            title_name='Raw disk-integrated'
                        elif plot_mod=='DI_prof_res':
                            xt_str='heliocentric'
                            y_title='Residuals'
                            title_name='Residuals to raw disk-integrated'
                        elif plot_mod=='CCF_DIbin':
                            if (vis=='binned'):xt_str='binned'
                            elif gen_dic['align_DI'] and data_dic['DI']['sysvel'][inst][vis]>0.:xt_str='star'
                            else:xt_str='heliocentric'
                            y_title='Flux'
                            title_name='Binned disk-integrated'
                        elif plot_mod=='CCF_DIbin_res':
                            xt_str='star'
                            y_title='Residuals'
                            title_name='Residuals to binned disk-integrated'
                        elif plot_mod=='CCFloc':
                            xt_str='star'
                            y_title='Flux'
                            title_name='Local'
                        elif plot_mod=='CCFintr':   
                            title_name='Intrinsic'                             
                            if plot_options['aligned']:
                                xt_str='local'
                                title_name='Aligned '+title_name
                            else:xt_str='star' 
                            y_title='Flux'
                        elif plot_mod=='CCFintr_res':
                            title_name='Residuals to intrinsic'                      
                            if plot_options['aligned']:
                                xt_str='local'
                                title_name='Aligned '+title_name
                            else:xt_str='star' 
                            y_title='Residuals'                     
                        elif plot_mod=='CCF_Intrbin':   
                            title_name='Binned intrinsic'
                            if gen_dic['align_Intr']:
                                xt_str='local'
                                title_name='Aligned '+title_name
                            else:xt_str='star'
                            y_title='Flux'
                        elif plot_mod=='CCF_Intrbin_res':
                            title_name='Residuals to binned intrinsic'  
                            if gen_dic['align_Atm']:
                                xt_str='local'
                                title_name='Aligned '+title_name
                            else:xt_str='star'                          
                            y_title='Residuals'   
                        elif plot_mod=='CCFatm':   
                        ...
                        elif plot_mod=='CCFatm_res':
                            title_name='Residuals to atmospheric '+plot_options['pl_atm_sign']                      
                            if plot_options['aligned']:
                                xt_str='planet'
                                title_name='Aligned '+title_name
                            else:xt_str='star' 
                            y_title='Residuals'     
                        elif plot_mod=='CCF_Atmbin':   
                            title_name='Binned atmospheric '+plot_options['pl_atm_sign']  
                            if gen_dic['align_Atm']:
                                xt_str='planet'
                                title_name='Aligned '+title_name
                            else:xt_str='star'                     
                        ...
                        elif plot_mod=='CCF_Atmbin_res':
                            title_name='Residuals to binned atmospheric '+plot_options['pl_atm_sign']  
                            if gen_dic['align_Atm']:
                                xt_str='planet'
                                title_name='Aligned '+title_name
                            else:xt_str='star'                     
                        ...
                        if (plot_mod in ['DI_prof','DI_prof_res','CCF_DIbin','CCF_DIbin_res','CCFloc','CCFintr','CCFintr_res','CCF_Intrbin','CCF_Intrbin_res','CCFatm','CCFatm_res','CCF_Atmbin','CCF_Atmbin_res']):
                            if plot_mod in ['CCF_DIbin','CCF_DIbin_res','CCF_Intrbin','CCF_Intrbin_res','CCF_Atmbin','CCF_Atmbin_res']:
                                ref_name = plot_options['dim_plot']
                                ref_val=data_bin['cen_bindim'][iexp]
                            else:
                                ref_name = 'phase'
                                ref_val=coord_dic[inst][vis][pl_ref]['cen_ph'][iexp_or]                               
                            if plot_options['plot_title']:plt.title(title_name+' CCF (visit ='+vis+'; '+ref_name+' ='+"{0:.5f}".format(ref_val)+')',fontsize=plot_options['font_size']) 
                        ...
                        custom_axis(plt,position=plot_options['margins'],x_range=x_range_loc,y_range=y_range_loc,xmajor_int=xmajor_int,xminor_int=xminor_int,ymajor_int=ymajor_int,yminor_int=yminor_int,xmajor_form=xmajor_form,ymajor_form=ymajor_form,
                                    #dir_y='out',
                                    axis_thick=plot_options['axis_thick'],hide_axis = plot_options['hide_axis'],
                                    x_title=x_title,y_title=y_title,font_size=plot_options['font_size'],xfont_size=plot_options['font_size'],yfont_size=plot_options['font_size'])
                
                        if (plot_mod in ['CCF_DIbin','CCF_DIbin_res','CCFintr','CCFintr_res','CCF_Intrbin','CCF_Intrbin_res']) or ((plot_mod in ['CCFatm','CCFatm_res','CCF_Atmbin','CCF_Atmbin_res']) and (plot_options['pl_atm_sign']=='Absorption')):loc_str='_'    
                        else:                                      
                            if iexp in gen_dic[inst][vis]['idx_in']:loc_str='_in_'
                            elif iexp in gen_dic[inst][vis]['idx_out']:loc_str='_out_'
                        file_name = path_loc+'idx'+"{:d}".format(iexp)+loc_str+ref_name+"{0:.4f}".format(ref_val)
                        plt.savefig(file_name+'.'+plot_ext, transparent=plot_options['transparent']) 
                        plt.close()  
    ...
    '''
    Sub-functions to plot individual spectral profiles
        - for disk-integrated, local, intrinsic, atmospheric data
    '''
    def sub_plot_spec(plot_options,plot_mod,plot_ext):

        #Options
        sc_fact=10**plot_options['sc_fact10']
        hide_yticks=False
        
        add_txt_path ={'DI':'','Intr':'','Atm':plot_options['pl_atm_sign']+'/'}
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options['visits_to_plot'].keys())): 
            ...
            for vis in np.intersect1d(list(data_dic[inst].keys())+['binned'],plot_options['visits_to_plot'][inst]): 
                ...
                if 'DI' in plot_mod:
                    if plot_mod=='DI_raw':                       
                        path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Spec_raw/'+inst+'_'+vis+'_Indiv_sp/'
                        
                        #Exposures to plot
                        if ('iexp_plot' in plot_options) and (inst in plot_options['iexp_plot']) and (vis in plot_options['iexp_plot'][inst]) and (len(plot_options['iexp_plot'][inst][vis])>0):iexp_plot = plot_options['iexp_plot'][inst][vis]
                        ...
                        if (plot_options['plot_pre']=='permpeak') or (plot_options['plot_post']=='permpeak'):
                            data_permpeak = np.load(gen_dic['save_data_dir']+'Corr_data/Permpeak/'+inst+'_'+vis+'_add.npz', allow_pickle=True)['data'].item()
                        else:
                            data_permpeak = None
                            plot_options['det_permpeak']=False
    
                    elif plot_mod in ['DIbin','DImast']:
                        dim_plot = 'phase'
                        if plot_mod=='DIbin': 
                            path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Binned_DI_data/'+inst+'_'+vis+'_Indiv_sp/'+dim_plot+'/'   
                            data_bin = np.load(gen_dic['save_data_dir']+'DIbin_data/'+inst+'_'+vis+'_'+dim_plot+'_add.npz',allow_pickle=True)['data'].item()           
                        elif plot_mod=='DImast': 
                            path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Weighing_master/'+inst+'_'+vis+'/'
                            data_bin = np.load(gen_dic['save_data_dir']+'DI_data/Master/'+inst+'_'+vis+'_'+dim_plot+'_add.npz',allow_pickle=True)['data'].item()
                            iexp_plot = [0]

                elif plot_mod=='Res':
                    path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Res_data/'+inst+'_'+vis+'_Indiv_sp/'
                    iexp_plot = data_dic['Res'][inst][vis] ['idx_to_extract']
                    iexp_orig = iexp_plot
                    ...
                    if plot_mod=='Intr':
                        if plot_options['aligned']:
                            path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Aligned_Intr_data/'+inst+'_'+vis+'_Indiv_sp/'
                        else:
                            path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Intr_data/'+inst+'_'+vis+'_Indiv_sp/'
                        iexp_plot = data_dic['Intr'][inst][vis]['idx_def']
                        iexp_orig = np.array(gen_dic[inst][vis]['idx_in'])[iexp_plot] 
                    elif plot_mod=='Intrbin': 
                        path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Binned_intr_data/'+inst+'_'+vis+'_Indiv_sp/'+plot_options['dim_plot']+'/'
                        data_bin = np.load(gen_dic['save_data_dir']+plot_mod+'_data/'+inst+'_'+vis+'_'+plot_options['dim_plot']+'_add.npz',allow_pickle=True)['data'].item()
                
                elif 'Atm' in plot_mod:                
                    if plot_mod=='Atm':
                        if plot_options['aligned']:suff='_pl' 
                        else:suff=''
                        iexp_plot = data_dic['Atm'][inst][vis]['idx_def']
                        path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Atm_data'+suff+'/'+plot_options['pl_atm_sign']+'/'+inst+'_'+vis+'_Indiv_sp/'
                        if (plot_options['pl_atm_sign']=='Absorption'):iexp_orig = np.array(gen_dic[inst][vis]['idx_in'])[iexp_plot]                      
                        elif (plot_options['pl_atm_sign']=='Emission'):iexp_orig = iexp_plot
                    elif plot_mod=='Atmbin': 
                        path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Binned_atm_data/'+plot_options['pl_atm_sign']+'/'+inst+'_'+vis+'_Indiv_sp/'+plot_options['dim_plot']+'/'
                        data_bin = np.load(gen_dic['save_data_dir']+plot_mod+'_data/'+plot_options['pl_atm_sign']+'/'+data_dic['Atm']['type'][inst]+'_'+inst+'_'+vis+'_'+plot_options['dim_plot']+'_add.npz',allow_pickle=True)['data'].item()                

                if '_1D' in plot_mod: 
                    data_type_gen = 'DI'
                    path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/'+data_type_gen+'_data_1Dfrom2D/'+inst+'_'+vis+'_Indiv_sp/'
                ...
                if (not os_system.path.exists(path_loc)):os_system.makedirs(path_loc)                  
                n_exp_plot = len(iexp_plot)

                #Plot each order for all exposures 
                if plot_options['multi_exp']: 
                    all_figs = {}
                    all_ax= {}
                    if plot_options['y_range'] is None:y_range_loc = {}
                    
                #Colors
                if vis not in plot_options['color_dic'][inst]:plot_options['color_dic'][inst][vis] = np.repeat('dodgerblue',n_exp_plot)
                else:plot_options['color_dic'][inst][vis] = np.repeat(plot_options['color_dic'][inst][vis],n_exp_plot)
                if vis not in plot_options['color_dic_sec'][inst]:plot_options['color_dic_sec'][inst][vis] =np.repeat('red',n_exp_plot)   
                else:plot_options['color_dic_sec'][inst][vis] = np.repeat(plot_options['color_dic_sec'][inst][vis],n_exp_plot)                 
                ...
                for isub,iexp in enumerate(iexp_plot):  
                    ...
                    if 'DI' in plot_mod:
                        if plot_mod=='DI_raw':
                            ...
                            if plot_options['plot_pre'] is not None:
                                data_precorr = np.load( data_path_dic[plot_options['plot_pre']]+str(iexp)+'.npz' ,allow_pickle=True)['data'].item()
                            if plot_options['plot_post'] is not None:
                                data_exp = np.load(data_path_dic[plot_options['plot_post']]+str(iexp)+'.npz',allow_pickle=True)['data'].item()
                            else:data_exp = data_precorr
                            ...
                            data_exp = np.load(gen_dic['save_data_dir']+'DIbin_data/'+inst+'_'+vis+'_'+plot_options['dim_plot']+str(iexp)+'.npz'  ,allow_pickle=True)['data'].item() 

                        elif plot_mod=='DImast':
                            data_exp = np.load(gen_dic['save_data_dir']+'DI_data/Master/'+inst+'_'+vis+'_phase.npz'  ,allow_pickle=True)['data'].item() 

                    elif plot_mod=='Res':
                        data_exp = np.load(gen_dic['save_data_dir']+'Res_data/'+inst+'_'+vis+'_'+str(iexp)+'.npz',allow_pickle=True)['data'].item()                
                        ...
                        if plot_mod=='Intr':
                            if plot_options['aligned']:data_exp = np.load(gen_dic['save_data_dir']+'Aligned_Intr_data/'+inst+'_'+vis+'_in'+str(iexp)+'.npz' ,allow_pickle=True)['data'].item() 
                            else:data_exp = np.load(gen_dic['save_data_dir']+'Intr_data/'+inst+'_'+vis+'_in'+str(iexp)+'.npz' ,allow_pickle=True)['data'].item() 
                     
                        elif plot_mod=='Intrbin':
                            data_exp = np.load(gen_dic['save_data_dir']+'Intrbin_data/'+inst+'_'+vis+'_'+plot_options['dim_plot']+str(iexp)+'.npz'  ,allow_pickle=True)['data'].item() 

                    elif 'Atm' in plot_mod: 
                        if plot_mod=='Atm': 
                            data_exp = np.load(gen_dic['save_data_dir']+'Atm_data'+suff+'/'+plot_options['pl_atm_sign']+'/'+data_dic['Atm']['type'][inst]+'_'+inst+'_'+vis+'_'+str(iexp)+'.npz'  ,allow_pickle=True)['data'].item() 
                    ...
                    for isub_ord,iord in enumerate(order_list):
                        ...
                        plot_ord=True
                        cen_bins_loc = data_exp['cen_bins'][iord]
                        cond_def_loc = data_exp['cond_def'][iord]
                        if plot_options['x_range'] is not None:
                            if (cen_bins_loc[cond_def_loc][-1]<plot_options['x_range'][0]) or (cen_bins_loc[cond_def_loc][0]>plot_options['x_range'][1]):plot_ord&=False
                            else:cond_def_loc[ (cen_bins_loc<plot_options['x_range'][0]) | (cen_bins_loc>plot_options['x_range'][1]) ]=False
                            x_range_loc = deepcopy(plot_options['x_range'])
                        else:
                            xmin = cen_bins_loc[cond_def_loc][0]
                            xmax = cen_bins_loc[cond_def_loc][-1]
                            dx_range = xmax-xmin
                        ...
                        if plot_mod=='DI_raw':
                            ...
                            #Only plot order if cosmics were detected
                            if plot_options['det_permpeak'] and (data_permpeak['count_bad_all'][iexp,iord]==0):plot_ord &= False     
                        ...
                        if plot_ord:  
                            ...
                            if not plot_options['multi_exp']:                                          
                                plt.ioff()                    
                                all_figs = {iord:plt.figure(figsize=(10, 6))}
                                all_ax = {iord:plt.gca()}
                            ...
                            if plot_mod=='DI_raw': 
                                ...
                                if plot_options['plot_pre'] is not None:
                                    ...
                                    all_ax[iord].plot(data_precorr['cen_bins'][iord],var_loc,color=col_exp,linestyle='-',lw=plot_options['lw_plot'],rasterized=plot_options['rasterized'],zorder=1,drawstyle=plot_options['drawstyle'],figure = all_figs[iord])   
                                ...
                                if plot_options['plot_post'] is not None:        
                                    ...
                                    if plot_options['plot_err']:
                                        ...
                                        all_ax[iord].errorbar(cen_bins_loc,var_loc,yerr = sc_fact*np.sqrt(data_exp['cov'][iord][0,:])/mean_flux,color=col_exp_err,linestyle='',lw=plot_options['lw_plot'],rasterized=plot_options['rasterized'],zorder=2,alpha=plot_options['alpha_err'],figure = all_figs[iord]) 
                                    all_ax[iord].plot(cen_bins_loc,var_loc,color=col_exp_sec,linestyle='-',lw=plot_options['lw_plot'],rasterized=plot_options['rasterized'],zorder=3,drawstyle=plot_options['drawstyle'],figure = all_figs[iord])   
                                    if plot_options['y_range'] is None:y_range_loc[iord] = [min(np.nanmin(var_loc),y_range_loc[iord][0]),max(np.nanmax(var_loc),y_range_loc[iord][1])]
                                    ...
                                    if plot_options['plot_tell']:
                                        ...
                                        tell_min = np.min(tell_loc[cond_def_loc])
                                        tell_max = np.max(tell_loc[cond_def_loc])
                                        min_f = np.min(var_loc[cond_def_loc])
                                        max_f = np.max(var_loc[cond_def_loc])
                                        ...
                                        all_ax[iord].plot(cen_bins_loc,tell2flux(tell_loc),color='green',linestyle='-',lw=plot_options['lw_plot'],rasterized=plot_options['rasterized'],zorder=-10,drawstyle=plot_options['drawstyle'],figure = all_figs[iord])   
                                    ...
                                    for molec in plot_options['plot_tell_HITRANS']:
                                        ...
                                        for wave_tell_loc in wave_tellL_exp[molec][cond_in_plot]:
                                            if plot_options['plot_tell']:
                                                idx_tell = closest(cen_bins_loc,wave_tell_loc)
                                ...
                                if (gen_dic['corr_Fbal']):
                                    if (plot_options['plot_mast']): 
                                        all_ax[iord].plot(cen_bins_loc,var_loc,color='black',linestyle='-',lw=1,rasterized=plot_options['rasterized'],zorder=-1,drawstyle=plot_options['drawstyle'],figure = all_figs[iord])                                 
                            ...
                            elif plot_mod in ['sp_DI_1D','DIbin','DImast','Res','Intr','Intrbin','sp_1D_intr','atm','Atmbin','sp_1D_atm']:
                                var_loc = sc_fact*data_exp['flux'][iord]/mean_flux
                                if plot_options['plot_err']:all_ax[iord].errorbar(cen_bins_loc,var_loc,yerr = sc_fact*np.sqrt(data_exp['cov'][iord][0,:])/mean_flux,color=col_exp,linestyle='-',lw=plot_options['lw_plot'],rasterized=plot_options['rasterized'],zorder=1,alpha=plot_options['alpha_err']) 
                                else:all_ax[iord].plot(cen_bins_loc,var_loc,color=col_exp,linestyle='-',lw=plot_options['lw_plot'],rasterized=plot_options['rasterized'],zorder=1,drawstyle=plot_options['drawstyle'],figure = all_figs[iord])   
                                if plot_options['y_range'] is None:y_range_loc[iord] = [min(np.nanmin(var_loc),y_range_loc[iord][0]),max(np.nanmax(var_loc),y_range_loc[iord][1])]
        
                                #Plotting estimate for local stellar profile
                                if (plot_mod=='Res') and plot_loc_star:
                                    all_ax[iord].plot(data_loc_star['cen_bins'][iord],sc_fact*data_loc_star['flux'][iord]/mean_flux,color='red',linestyle='-',lw=plot_options['lw_plot'],rasterized=plot_options['rasterized'],zorder=2,drawstyle=plot_options['drawstyle'],figure = all_figs[iord]) 

                            #Shade range of planetary signal excluded for relevant exposures
                            if (data_dic['Atm']['exc_plrange']) and (plot_options['plot_plexc']) and (iexp in data_dic['Atm'][inst][vis]['iexp_no_plrange']):
                                if (plot_mod=='Res'): 
                                    wrange_line = data_dic['Atm'][inst][vis]['exclu_rangestar'][:,:,iexp]
                            ...
                            if (not plot_options['multi_exp']) or (isub==len(iexp_plot)-1):   
                                if plot_mod=='DI_raw':                            
                                    ...
                                    xt_str='star'
                                    title_name='Aligned '+title_name 
                                elif plot_mod=='sp_DI_1D':
                                    title_name='1D '+gen_dic['type_name'][data_type_gen]
                                    y_title='Flux'
                                    if gen_dic['align_DI']:
                                        title_name='Aligned '+title_name 
                                        xt_str='local'
                                    ...
                                    title_name='Binned intrinsic'
                                    y_title='Flux'
                                    if plot_options['dim_plot']=='phase': 
                                        xt_str='star'
                                    else:
                                        xt_str='local'
                                        title_name='Aligned '+title_name                                   
                                elif plot_mod=='sp_1D_intr':
                                    title_name='1D intrinsic'
                                    y_title='Flux'
                                    if (plot_options['dim_plot']=='orig') or ((plot_options['dim_plot']=='bin') and (dim_bin_1D=='phase')): 
                                        xt_str='star'
                                    else:
                                        xt_str='local'
                                        title_name='Aligned '+title_name                                  
                                    ...
                                    if plot_options['dim_plot']=='phase': 
                                        ...
                                        xt_str='planet'
                                        title_name='Aligned '+title_name                               
                                elif plot_mod=='sp_1D_atm':
                                    title_name='1D atmospheric '+plot_options['pl_atm_sign']
                                    if plot_options['pl_atm_sign']=='Absorption':y_title='Absorption'
                                    elif plot_options['pl_atm_sign']=='Emission':y_title='Flux'
                                    if (plot_options['dim_plot']=='orig') or ((plot_options['dim_plot']=='bin') and (dim_bin_1D=='phase')): 
                                        xt_str='star'
                                ...
                                if (plot_mod in ['DIbin','DImast']):                                        
                                    ref_name='phase'
                                    ref_val=data_bin['cen_bindim'][iexp]
                                elif (plot_mod in ['Intrbin','Atmbin']):
                                    ref_name = plot_options['dim_plot']
                                    ref_val=data_bin['cen_bindim'][iexp]
                                elif (plot_mod in ['sp_1D_intr','sp_1D_atm']) and (plot_options['dim_plot']=='bin'):
                                    ref_name = dim_bin_1D
                                    ref_val=data_bin['cen_bindim'][iexp]
                                else:
                                    ref_name = 'phase'
                                    ref_val=coord_dic[inst][vis][plot_options['pl_ref'][inst][vis]]['cen_ph'][iexp_orig[isub]] 
                                if plot_options['plot_title']:all_ax[iord].title(title_name+' spectrum (visit ='+vis+'; '+ref_name+' ='+"{0:.5f}".format(ref_val)+')',fontsize=plot_options['font_size'])                      
                                y_title = scaled_title(plot_options['sc_fact10'],y_title)  
                                ...
                                if not plot_options['multi_exp']:
                                    ...
                                    if plot_mod in ['DIbin','DImast','Intrbin','Atmbin']:str_idx = 'idx'+str(iexp)
                                    else:str_idx = 'idx'+str(iexp_orig[isub])
                                    if (plot_options['plot_pre']=='permpeak') or (plot_options['plot_post']=='permpeak'):
                                        save_path = path_loc+'iord'+str(iord)+'/'+str_idx+'_'+ref_name+"{0:.5f}".format(ref_val)
                                        if (not os_system.path.exists(path_loc+'iord'+str(iord)+'/')):os_system.makedirs(path_loc+'iord'+str(iord)+'/')  
                                    else:save_path=path_loc+str_idx+'_'+ref_name+"{0:.5f}".format(ref_val)+str_add
                                else:    
                                    save_path=path_loc+ref_name+"{0:.5f}".format(ref_val)+'_multiexp_iord'+str(iord)
                                all_figs[iord].savefig(save_path+'.'+plot_ext)                        
                                plt.close()                          
    ...
    def sub_plot_DI_trans(plot_options,plot_mod,plot_ext):
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options['visits_to_plot'].keys())): 
            ...
            for vis in np.intersect1d(list(data_dic[inst].keys()),plot_options['visits_to_plot'][inst]): 
                ...
                path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Spec_raw/DI_trans/'+inst+'_'+vis+'/'
                if not os_system.path.exists(path_loc):os_system.makedirs(path_loc)  
                ...
                data_path_dic = {
                    ...
                    'cosm':gen_dic['save_data_dir']+'Corr_data/Cosm/'+inst+'_'+vis+'_',
                    'permpeak':gen_dic['save_data_dir']+'Corr_data/Permpeak/'+inst+'_'+vis+'_',
                    'wiggle':gen_dic['save_data_dir']+'Corr_data/Wiggles/Data/'+inst+'_'+vis+'_',
                    }    
                ...
                #    - for wiggles see details in the corresponding functions
                #    - we assume here that exposures do not share a common table 
                ...
                for isub_exp,iexp in enumerate(iexp_proc_list):
                    ...
                    #Retrieve data
                    for maink in maink_list:
                        data_proc[maink][iexp] = np.load(data_path_dic[plot_options['plot_'+maink]]+str(iexp)+'.npz',allow_pickle=True)['data'].item() 
                        ...
                        if gen_dic['gain_weight']:data_proc[maink][iexp]['mean_gdet'] = dataload_npz(data_vis['mean_gdet_DI_data_paths'][iexp])['mean_gdet'] 
                        ...
                        #Exposure is resampled on common table otherwise
                            ...
                            if data_inst['gain_weight']:data_proc[maink][iexp]['mean_gdet'] = bind.resampling(edge_bins_com[isub], data_proc[maink][iexp]['edge_bins'][isub]*dopp_fact, data_proc[maink][iexp]['mean_gdet'][iord] , kind=gen_dic['resamp_mode']) 
                        ...
                        #    - so that we transmission spectra are at a comparable flux level 
                        ...
                        if iexp in iexp_mast_list:
                            ...
                            data_proc[maink][iexp]['weight'] = def_weights_spatiotemp_bin(idx_sel_ord,None,inst,vis,gen_dic['corr_Fbal'],gen_dic['corr_FbalOrd'],gen_dic['save_data_dir'],gen_dic['type'],nord_proc,iexp,'DI',data_inst['type'],dim_exp_proc,data_proc[maink][iexp]['tell'],data_proc[maink][iexp]['mean_gdet'],data_proc[maink][iexp]['cen_bins'],flux_ref,None,glob_flux_sc = 1./flux_glob)                       
                ...
                for isub_exp,iexp in enumerate(iexp_plot):
                    ...
                    if ('wiggle' in data_list) and (plot_options['shade_unfit']) and (vis in data_com_wig['wig_range_fit']):
                        plot_shade_range(plt.gca(),data_com_wig['wig_range_fit'][vis],x_range_loc,x_range_loc,mode='fill',facecolor='dodgerblue',compl=True)                  
    ...
    def sub_plot_all_prof(plot_options,plot_mod,plot_ext):
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options['visits_to_plot'].keys())):   
            ...
            for vis in np.intersect1d(list(data_dic[inst].keys()),plot_options['visits_to_plot'][inst]): 
                print('     - visit '+vis)
                data_vis = data_dic[inst][vis]
                if (plot_ext in ['ps','png','pdf','jpg']):
                    if plot_mod=='glob_mast':path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Spec_raw/FluxBalance/Global_master/'+inst+'_'+vis+'/' 
                    elif plot_mod=='DI':path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Aligned_'+plot_mod+'_data/'+inst+'_'+vis+'/'  
                    elif plot_mod=='Intr':path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Aligned_'+plot_mod+'_data/'+inst+'_'+vis+'/'
                    elif plot_mod=='Atm':path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Aligned_'+plot_mod+'_data/'+plot_options['pl_atm_sign']+'/'+inst+'_'+vis+'/'
                    if not os_system.path.exists(path_loc):os_system.makedirs(path_loc)   
                ...
                if (inst in plot_options['color_dic']) and (vis in plot_options['color_dic'][inst]) and not ((plot_mod=='glob_mast') and ('spec' in data_type)):
                    col_visit=np.repeat(plot_options['color_dic'][inst][vis],n_exp_plot)
                else:
                    cmap = plt.get_cmap('rainbow') 
                    col_visit=np.array([cmap(0)]) if n_exp_plot==1 else cmap( np.arange(n_exp_plot)/(n_exp_plot-1.))

                #------------------------------------------------------------------------------------------
                        
                #Paths
                if plot_mod=='DI':path_exp = gen_dic['save_data_dir']+'Aligned_'+plot_mod+'_data/'+inst+'_'+vis+'_'                  
                elif plot_mod=='Intr':path_exp = gen_dic['save_data_dir']+'Aligned_'+plot_mod+'_data/'+inst+'_'+vis+'_in' 
                elif plot_mod=='Atm':path_exp = gen_dic['save_data_dir']+'Aligned_'+plot_mod+'_data/'+plot_options['pl_atm_sign']+'/'+inst+'_'+vis+'_'   
                ...
                if data_type=='CCF':
                    ...
                    for isub,iexp in enumerate(iexp_plot):
                        ...
                        if plot_mod=='DI':           
                            ...
                            norm_exp=np.mean(var_loc[cond_def_scal])                         
                 
                        #Normalizing unscaled DI CCF so that they can be comparable
                        if plot_mod=='DI':var_loc/=norm_exp
                    ...
                    #Plot frame
                    if plot_options['plot_title']:plt.title('Aligned '+title_name+' CCFs in '+xt_str+' rest frame',fontsize=plot_options['font_size'])
                    y_title = scaled_title(plot_options['sc_fact10'],y_title)                       
                    dx_range=x_range_loc[1]-x_range_loc[0]
                    dy_range=y_range_loc[1]-y_range_loc[0]
                    xmajor_int,xminor_int,xmajor_form = autom_x_tick_prop(dx_range)
                    ...
                    plt.savefig(path_loc+'Aligned_'+plot_mod+'_'+data_type+'.'+plot_ext) 
                    plt.close()    
                ...
                #Plotting spectral profiles
                    ...
                    for iord in order_list:
                        ...
                        plot_ord=False
                        min_wav = np.min([wav_exp_all[iexp][iord][cond_def_exp_all[iexp][iord]][0] for iexp in iexp_plot])
                        max_wav = np.max([wav_exp_all[iexp][iord][cond_def_exp_all[iexp][iord]][-1] for iexp in iexp_plot])
                        if plot_options['x_range'] is not None:
                        ...
                        if plot_ord:                                         
                            ...
                            if plot_options['plot_input']:
                                for isub,iexp in enumerate(iexp_plot):
                                    ...
                                    plt.plot(wav_tab,var_loc,color=col_visit[isub],linestyle='-',lw=plot_options['lw_plot'],rasterized=plot_options['rasterized'],zorder=1) 
                            ...
                            if plot_options['plot_title']:
                                if plot_mod=='glob_mast':plt.title('Aligned '+title_name+' spectra in '+xt_str+' rest frame'+str_add)
                                else:plt.title('Raw '+title_name+' spectra in '+xt_str+' rest frame'+str_add)
                            y_title='Flux'
                            y_title = scaled_title(plot_options['sc_fact10'],y_title)
                            dx_range=x_range_loc[1]-x_range_loc[0]
                            dy_range=y_range_loc[1]-y_range_loc[0]
                            ...
                            plt.savefig(path_loc+'Aligned_'+plot_mod+'_'+data_type+str_add+'.'+plot_ext) 
                            plt.close() 
    ...
    def sub_plot_DI_Intr_binprof(plot_options,plot_ext):
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options['visits_to_plot'].keys())):   
            ...
            for vis in np.intersect1d(list(data_dic[inst].keys())+['binned'],plot_options['visits_to_plot'][inst]): 
                print('     - Visit : '+vis)
                path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Compa_DI_intr_series/'+inst+'_'+vis+'/' 
                if not os_system.path.exists(path_loc):os_system.makedirs(path_loc)   
                ...
                n_exp_plot_DI=len(iexp_plot_DI) 
                ...
                n_exp_plot_intr=len(iexp_plot_intr) 

                #Visit colors  
                if 'DI' in plot_options['color_dic']:col_DI=np.repeat(plot_options['color_dic']['DI'],n_exp_plot_DI)
                else:col_DI=np.array([cmap(0)]) if n_exp_plot_DI==1 else cmap( np.arange(n_exp_plot_DI)/(n_exp_plot_DI-1.))
                if 'Intr' in plot_options['color_dic']:col_intr=np.repeat(plot_options['color_dic']['Intr'],n_exp_plot_intr)
                else:col_intr=np.array([cmap(0)]) if n_exp_plot_intr==1 else cmap( np.arange(n_exp_plot_intr)/(n_exp_plot_intr-1.))
    ...
    def sub_plot_chi2_prop(plot_options,plot_ext):
        path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/CCFloc_prop/' 
        if not os_system.path.exists(path_loc):os_system.makedirs(path_loc)                                        
        plt.ioff()                    
        plt.figure(figsize=(10, 6))
        ...
        #Upload fit results
        data_upload = np.load(plot_options['IntrProp_path']+plot_options['prop']+'/Fit_results.npz', allow_pickle=True)['data'].item()
        ...
        for inst in np.intersect1d(list(data_upload['prop_fit'].keys()),list(plot_options['visits_to_plot'].keys())): 
            ...
            for vis in np.intersect1d(list(data_upload['prop_fit'][inst].keys()),plot_options['visits_to_plot'][inst]):    
                if vis not in plot_options['color_dic']:plot_options['color_dic'][inst][vis]='dodgerblue'
        ...
        #Contacts
        y_range_loc=plot_options['y_range'] if plot_options['y_range'] is not None else np.array([0.,y_max]) 

        #---------------------------------------------------------------------------------------- 

        #Plot frame 
        x_range_loc=plot_options['x_range'] if plot_options['x_range'] is not None else np.array([x_min,x_max])     
        if plot_options['plot_title']:plt.title('$\Chi^2$ per exposure',fontsize=plot_options['font_size'])                             
        custom_axis(plt,position=plot_options['margins'],x_range=x_range_loc,y_range=y_range_loc,
                    xmajor_form='%.3f',ymajor_form='%.1f',
        		    x_title=glob_fit_dic['PropLoc']['coord_line'],y_title='$\chi^2$',
                    font_size=plot_options['font_size'],xfont_size=plot_options['font_size'],yfont_size=plot_options['font_size'])
    ...
    def sub_func_bin(bin_val_loc,dic_val,x_min,x_max,y_min,y_max,plot_options):
        ...
        if (not plot_options['plot_err']) and (not plot_options['plot_HDI']):
            ...
            if plot_options['plot_HDI']:
                ...
                plt.errorbar(mid_x_bin,val_bin,yerr=eval_bin,color='black',rasterized=plot_options[key_plot]['rasterized'],markeredgecolor='black',markerfacecolor='None',marker='d',markersize=plot_options['markersize']+2,linestyle='',zorder=10,alpha=plot_options['alpha_symb']) 
    ...
    def sub_plot_CCF_prop(prop_mode,key_plot,plot_options,data_mode): 
        if data_mode=='raw':path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/DI_prop/'  
        elif data_mode=='loc': path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Loc_Prop/' 
        if not os_system.path.exists(path_loc):os_system.makedirs(path_loc)
        ...
        def fit_pol_sin(param,x_unused,args=None):
            ...
            for iprop,prop_fit in enumerate(args['prop_fit']): 
                ...
                if apply_corr:
                    if prop_mode in ['rv','RVres']:fit_tab = corr_line_prof_gen(  corr_prop, x_prop, fit_tab , 'add')  
                    else:fit_tab = corr_line_prof_gen(  corr_prop, x_prop, fit_tab , 'modul')  
        ...
        if data_mode=='loc':    
            data_fit_prop=None  
            data_fit_prof=None
            if prop_mode in ['rv','FWHM','ctrst','true_FWHM','true_FWHM']:
                if plot_options['IntrProp_path'] is not None:
                    data_fit_prop = np.load(plot_options['IntrProp_path']+prop_mode+'/Fit_results.npz',allow_pickle=True)['data'].item()          
                if plot_options['IntrProf_path'] is not None:
                    data_fit_prof = np.load(plot_options['IntrProf_path']+'Fit_results.npz',allow_pickle=True)['data'].item()       
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options['visits_to_plot'].keys())): 
            ...
            #Plot for each visit
            if data_mode=='raw':vis_list = np.intersect1d(list(data_dic['DI'][inst].keys())+['binned'],plot_options['visits_to_plot'][inst])
            elif data_mode=='loc':vis_list = plot_options['visits_to_plot'][inst]
            for vis in vis_list: 
                
                #Identifying original or binned data
                if data_mode=='raw':
                    orig_vis = vis
                    if 'bin' in vis:data_type = 'DIbin'
                    else:data_type = 'DIorig'
                    cond_vis = True
                elif data_mode=='loc':
                ...
                if (data_mode=='loc') and (prop_mode=='rv') and plot_options['theo_HR_nom'] : 
                    theo_HR_prop_plocc = calc_occ_plot(inst,vis,{},params,{},deepcopy(system_param))
                    if plot_options['prop_absc']=='phase':xvar_HR=deepcopy(theo_HR_prop_plocc[pl_ref]['phase'])  
                    elif plot_options['prop_absc'] in ['mu','lat','lon','x_st','y_st','xp_abs','r_proj']:xvar_HR=deepcopy(theo_HR_prop_plocc[pl_ref][plot_options['prop_absc']])  
                    elif plot_options['prop_absc']=='y_st2':xvar_HR=theo_HR_prop_plocc[pl_ref]['y_st']**2.  
                    elif plot_options['prop_absc']=='abs_y_st':xvar_HR=np.abs(theo_HR_prop_plocc[pl_ref]['y_st'])
                    wsort=theo_HR_prop_plocc[pl_ref]['phase'].argsort()
    
                    #Nominal high-resolution model                
                    plt.plot(xvar_HR[wsort],theo_HR_prop_plocc[pl_ref]['rv'][wsort],color='black',linestyle='-',lw=plot_options['lw_plot'],zorder=-1)   
                    
                    # #Save/replot manually 
                    # # save_path = '/Travaux/ANTARESS/En_cours/Fit/SB_aligned.dat'
                    # # save_path = '/Travaux/ANTARESS/En_cours/RVloc_model_WASP121_Vincent.dat'
                    # # save_path = '/Users/bourrier/Travaux/ANTARESS/En_cours/GJ436_b_Plots/CCFloc_prop/RRM_RV_model.dat' 
                    # # save_path = '/Users/bourrier/Travaux/ANTARESS/En_cours/TIC257527578b_Plots/CCFloc_prop/RRM_RV_model_DR0.2_lambda0.dat' 
                    # save_path = '/Users/bourrier/Travaux/ANTARESS/En_cours/TIC257527578b_Plots/CCFloc_prop/RRM_RV_model_DR0.2_lambda90.dat' 
                    ...
                    if (plot_options['calc_envMCMC_theo_HR_nom']) or (plot_options['calc_sampMCMC_theo_HR_nom']):
                        ...
                        #Load samples
                        npzfile = np.load(plot_options['theo_HR']+'/MCMC/Sample_dics.npz')
                        ...
                        if (plot_options['calc_envMCMC_theo_HR_nom'] is not None):
                        ...
                        if (plot_options['calc_sampMCMC_theo_HR_nom'] is not None):
                ...
                if cond_vis:                
                    ...
                    if data_mode=='raw':
                        ...
                        if data_type=='Introrig':
                            coord_vis = coord_dic[inst][vis][pl_ref]
                            if not plot_options['no_data']:CCF_fit_vis=np.load(gen_dic['save_data_dir']+'Introrig_prop/'+inst+'_'+vis+'.npz',allow_pickle=True)['data'].item()
                            transit_prop_nom = (np.load(gen_dic['save_data_dir']+'Introrig_prop/PlOcc_Prop_'+inst+'_'+vis+'.npz',allow_pickle=True)['data'].item())['achrom'][pl_ref]                                                      
                        elif data_type=='Intrbin': 
                            if not plot_options['no_data']:
                                CCF_fit_vis=np.load(gen_dic['save_data_dir']+'Intrbin_prop/'+inst+'_'+orig_vis+'.npz',allow_pickle=True)['data'].item()
                        ...
                        if (data_fit_prop is not None) or (data_fit_prof is not None): 
                            ...
                            if plot_options['theo_HR_prop'] or plot_options['theo_HR_prof']:  
                                def sub_plot_HR(mode_loc,input_dic,col_loc):
                                    ...
                                    if prop_mode=='rv':par_list_HR+=['rv']
                                    elif prop_mode in ['ctrst','FWHM']: 
                                        theo_dic_in['pol_mode']=data_fit_loc['pol_mode']
                                        theo_dic_in['coord_line']=data_fit_loc['coord_line']
                                        if plot_options['theo_HR_prop']:par_list_HR+=[prop_mode]
                                        elif plot_options['theo_HR_prof']:par_list_HR+=['ctrst','FWHM']
                                        theo_dic_in['coeff_line'] =  data_fit_loc['coeff_line_dic'][inst][vis]
                                        
                                    theo_HR_prop_loc = plot_plocc_prop(inst,vis,data_fit_loc['genpar_instvis'],pl_ref,deepcopy(system_param),theo_HR_prop_plocc[pl_ref]['phase'],par_list_HR,deepcopy(data_dic['DI']['system_prop']),deepcopy(data_fit_loc['p_final']),theo_dic_in)       
                                    
                                    
                                    
                                    
                                    
                                    if plot_options['prop_absc']=='phase':xvar_HR_loc=deepcopy(theo_HR_prop_loc['phase'])  
                                    elif plot_options['prop_absc'] in ['mu','lat','lon','x_st','y_st','xp_abs','r_proj']:xvar_HR_loc=deepcopy(theo_HR_prop_loc[plot_options['prop_absc']])  
                                    elif plot_options['prop_absc']=='y_st2':xvar_HR_loc=theo_HR_prop_loc['y_st']**2.  
                                    elif plot_options['prop_absc']=='abs_y_st':xvar_HR_loc=np.abs(theo_HR_prop_loc['y_st'])
                                    ...
                                    if prop_mode=='rv':
                                        ...
                                        if mode_loc =='from_prof':
                                            ...
                                            if data_fit_loc['func_prof_name'][inst]=='gauss':
                                                ctrst_mod_HR,FWHM_mod_HR = gauss_intr_prop(raw_ctrst_mod_HR,raw_FWHM_mod_HR,return_FWHM_inst(inst,c_light)) 
                                            elif data_fit_loc['func_prof_name'][inst]=='dgauss':
                                                ...
                                                for isub_HR,(raw_ctrst_loc,raw_FWHM_loc) in enumerate(zip(raw_ctrst_mod_HR,raw_FWHM_mod_HR)):
                                                    input_dic.update({'ctrst':raw_ctrst_loc,  'FWHM':raw_FWHM_loc , 'FWHM_inst':return_FWHM_inst(inst,c_light),'fit_func_gen':dgauss})
                                                ...
                                                FWHM_mod_HR = np.zeros(len(wsort),dtype=float)
                                                for isub_HR,(raw_ctrst_loc,raw_FWHM_loc) in enumerate(zip(raw_ctrst_mod_HR,raw_FWHM_mod_HR)):
                                                    input_dic.update({'ctrst':raw_ctrst_loc,  'FWHM':raw_FWHM_loc, 'FWHM_inst':return_FWHM_inst(inst,c_light),'fit_func_gen':voigt,'slope':0.})
                                                    ctrst_mod_HR[isub_HR],FWHM_mod_HR[isub_HR] = cust_mod_true_prop(input_dic,input_dic['vel'],input_dic)[0:2]                            
                                    ...
                                    #Plot
                                    plt.plot(xvar_HR_loc[wsort],yvar_HR_loc,color=col_loc,linestyle='--',lw=1,zorder=-1)   
                        ...
                        if (data_fit_prop is not None):
                            ...
                            if plot_options['theo_obs_prop']:
                                if data_fit_prop['coord_line']!=plot_options['prop_absc']:stop('Plot and fit coordinates must match')
                            ...
                            if plot_options['theo_HR_prop']:
                                sub_plot_HR('from_prop',None,'orange')
                        ...
                        if (data_fit_prof is not None):                                
                            ...
                            if ((data_fit_prof['func_prof_name'][inst]=='dgauss') & (prop_mode in ['true_FWHM','true_ctrst'])) | ((data_fit_prof['func_prof_name'][inst]=='voigt') & (prop_mode in ['FWHM_voigt','ctrst'])):                        
                                velccf_HR = gen_dic['RVstart_HR_mod'] + gen_dic['dRV_HR_mod']*np.arange(  int((gen_dic['RVend_HR_mod']-gen_dic['RVstart_HR_mod'])/gen_dic['dRV_HR_mod'])  )
                                input_dic.update({'cont':1.,'rv':0.,'vel':velccf_HR})
                                if data_fit_prof['func_prof_name'][inst]=='dgauss':
                                    ...
                                    input_dic['a_damp']=data_fit_prof['p_final'][data_fit_prof['name_prop2input']['a_damp__IS'+inst+'_VS'+vis]]
                            ...
                            if plot_options['theo_obs_prof']:                            
                                ...
                                for isub,i_in in enumerate(glob_fit_dic['IntrProf']['idx_in_fit'][inst][vis]):
                                    if data_fit_prof['func_prof_name'][inst]=='gauss':
                                        ctrst_mod_exp,FWHM_mod_exp = gauss_intr_prop(CCF_jointfit_vis[i_in]['ctrst'],CCF_jointfit_vis[i_in]['FWHM'],return_FWHM_inst(inst,c_light)) 
                                        ...
                                        ctrst_mod_exp,FWHM_mod_exp = cust_mod_true_prop(input_dic,input_dic['vel'],return_FWHM_inst(inst,c_light))[0:3]                            
                                    elif data_fit_prof['func_prof_name'][inst]=='voigt':
                                    ...
                                    if data_fit_prof['coord_line']!=plot_options['prop_absc']:stop('Plot and fit coordinates must match')
                            ...
                            if plot_options['theo_HR_prof']:  
                                sub_plot_HR('from_prof',input_dic,'black')
                    ...
                    if plot_options['prop_absc']=='phase':
                        if ((data_mode=='raw') and (vis=='binned')) or ((data_mode=='loc') and (data_type=='Intrbin')):
                    ...
                    elif plot_options['prop_absc'] in ['mu','lat','lon','x_st','y_st','xp_abs','r_proj','y_st2','abs_y_st']:  
                        if data_mode=='raw':iexp_in = gen_dic[inst][vis]['idx_in'] 
                        elif data_mode=='loc':iexp_in = range(n_exp_vis)                    
                        if plot_options['prop_absc'] in ['mu','lat','lon','x_st','y_st','xp_abs','r_proj']:  
                            x_obs[iexp_in] = transit_prop_nom[plot_options['prop_absc']][0,:]
                            st_x_obs[iexp_in],end_x_obs[iexp_in]  = transit_prop_nom[plot_options['prop_absc']+'_range'][0,:,0],transit_prop_nom[plot_options['prop_absc']+'_range'][0,:,1]
                        elif plot_options['prop_absc'] in ['y_st2','abs_y_st']:
                            if plot_options['prop_absc']=='y_st2':x_obs[iexp_in] = transit_prop_nom['y_st'][0,:]**2.
                            elif plot_options['prop_absc']=='abs_y_st':x_obs[iexp_in] = np.abs(transit_prop_nom['y_st'][0,:])
                    ...
                    elif plot_options['prop_absc'] in ['ctrst','FWHM']:
                        x_obs=np.array([CCF_fit_vis[idx_loc][plot_options['prop_absc']] for idx_loc in range(CCF_fit_vis['n_exp'])])
                        st_x_obs,end_x_obs = x_obs,x_obs 
                    elif plot_options['prop_absc'] in ['snr','snr_quad']: 
                        if plot_options['prop_absc']=='snr':x_obs =np.mean(SNR_obs[:,plot_options['idx_SNR'][inst]],axis=1)
                        elif plot_options['prop_absc']=='snr_quad':x_obs =np.sqrt(np.sum(SNR_obs[:,plot_options['idx_SNR'][inst]]**2.,axis=1))
                        st_x_obs,end_x_obs = x_obs,x_obs                                          
                    elif plot_options['prop_absc']=='snr_R':                        
                        ...
                        st_x_obs,end_x_obs = x_obs,x_obs                             
                    elif plot_options['prop_absc'] in ['AM','seeing','RVdrift','colcorrmin','colcorrmax','satur_check','alt','az']:            
                        x_obs=data_prop_vis[plot_options['prop_absc']][iexp_plot]   
                        st_x_obs,end_x_obs = x_obs,x_obs 
                    elif plot_options['prop_absc'] == 'flux_airmass':
                        #Flux decreases as 0.7^(AM^0.678), see Meinel+1976 
                        x_obs=0.7**(data_prop_vis['AM'][iexp_plot]**0.678)
                        st_x_obs,end_x_obs = x_obs,x_obs 
                    elif plot_options['prop_absc']=='colcorrR':            
                        x_obs=data_prop_vis['colcorrmax'][iexp_plot]/data_prop_vis['colcorrmin'][iexp_plot]   
                        st_x_obs,end_x_obs = x_obs,x_obs  
                    elif plot_options['prop_absc'] in ['colcorr450','colcorr550','colcorr650']:
                        idx_prop = {'colcorr450':0,'colcorr550':1,'colcorr650':2}[plot_options['prop_absc']]
                        x_obs = data_prop_vis['colcorr_vals'][iexp_plot,idx_prop]  
                        st_x_obs,end_x_obs = x_obs,x_obs    
                    elif plot_options['prop_absc'] in ['PSFx','PSFy','PSFr','PSFang']:
                        idx_prop = {'PSFx':0,'PSFy':1,'PSFr':2,'PSFang':3}[plot_options['prop_absc']]   
                        x_obs = data_prop_vis['PSF_prop'][iexp_plot,idx_prop] 
                        st_x_obs,end_x_obs = x_obs,x_obs 
                    elif plot_options['prop_absc'] in ['ha','na','ca','s','rhk']:            
                        x_obs=data_prop_vis[plot_options['prop_absc']][iexp_plot,0]   
                        x_obs_err = data_prop_vis[plot_options['prop_absc']][iexp_plot,1]   
                        st_x_obs,end_x_obs = x_obs-x_obs_err   ,x_obs+x_obs_err  
                    elif (plot_options['prop_absc'] in ['ADC1 POS','ADC1 RA','ADC1 DEC','ADC2 POS','ADC2 RA','ADC2 DEC']):    
                        if plot_options['prop_absc']=='ADC1 POS': x_obs=data_prop_vis['adc_prop'][iexp_plot,0]                                                                
                        elif plot_options['prop_absc']=='ADC1 RA': x_obs=data_prop_vis['adc_prop'][iexp_plot,1]    
                        elif plot_options['prop_absc']=='ADC1 DEC': x_obs=data_prop_vis['adc_prop'][iexp_plot,2]    
                        elif plot_options['prop_absc']=='ADC2 POS': x_obs=data_prop_vis['adc_prop'][iexp_plot,3]    
                        elif plot_options['prop_absc']=='ADC2 RA': x_obs=data_prop_vis['adc_prop'][iexp_plot,4]    
                        elif plot_options['prop_absc']=='ADC2 DEC': x_obs=data_prop_vis['adc_prop'][iexp_plot,5] 
                        st_x_obs,end_x_obs = x_obs,x_obs
                    ...
                    if not plot_options['no_data']:
                        ...
                        if prop_mode in ['rv','RVres','RVpipres','FWHM','ctrst','amp','rv_l2c','amp_l2c','FWHM_l2c','RV_lobe','amp_lobe','FWHM_lobe','true_ctrst','cont','c1_pol','c2_pol','c3_pol','c4_pol','vsini',\
                                         'ctrst_ord0__IS__VS_','FWHM_ord0__IS__VS_','FWHM_voigt','area']:
                            if (data_mode=='raw') and (vis=='binned') and (prop_mode=='RVres'):                            
                                prop_loc = 'rv'
                                err_prop_loc = 'err_rv'                            
                            else:
                                prop_loc = prop_mode
                                if prop_mode in ['area']:err_prop_loc = None  
                                else:err_prop_loc = 'err_'+prop_mode
                            val_obs=np.array([CCF_fit_vis[idx_loc][prop_loc] for idx_loc in range(CCF_fit_vis['n_exp'])])
                            if err_prop_loc is not None:eval_obs=np.vstack((  [CCF_fit_vis[idx_loc][err_prop_loc][0] for idx_loc in range(CCF_fit_vis['n_exp'])],[CCF_fit_vis[idx_loc][err_prop_loc][1] for idx_loc in range(CCF_fit_vis['n_exp'])]   ))
                            if prop_mode in ['RVres','RVpipres']:
                                val_obs*=1e3
                                eval_obs*=1e3
                        elif prop_mode in ['RVpip','FWHMpip','Ctrstpip']:
                            ...
                            val_obs = (np.load(gen_dic['save_data_dir']+'Scaled_data/'+inst+'_'+vis+'_add.npz',allow_pickle=True)['data'].item())['glob_flux_scaling'] 
                        ...
                        if (prop_mode=='RVres') and plot_options['save_RVres']:
                            np.savetxt(path_loc+inst+'_'+vis+'_'+prop_mode+'_'+plot_options['prop_absc']+'.dat', np.column_stack((x_obs,val_obs,np.mean(eval_obs,axis=0))),fmt=('%15.10f','%15.10f','%15.10f') )                                       
                     
                        #Points to plot   
                        if data_mode=='raw':
                            idx_in_plot=[iexp for iexp in range(CCF_fit_vis['n_exp']) if (iexp not in plot_options['idx_noplot'][inst][vis]) and (np.isnan(val_obs[iexp])==False)]
                        elif data_mode=='loc':
                            idx_in_plot=[i_in for i_in in range(CCF_fit_vis['n_exp']) if (i_in not in plot_options['idx_noplot'][inst][vis]) and (np.isnan(val_obs[i_in])==False) and   (     ((not plot_options['plot_det'])) or (plot_options['plot_det']  and (CCF_fit_vis[i_in]['detected']))        ) ]
                        ...
                        idx_in_plot_det=CCF_fit_vis['cond_detected'][idx_in_plot]
                    ...
                    #Colors
                    if vis not in plot_options['color_dic'][inst]:plot_options['color_dic'][inst][vis]='dodgerblue'                
                    if plot_options['color_dic'][inst][vis]=='rainbow':
                        cmap = plt.get_cmap('rainbow') 
                    ...
                    if data_mode=='raw':
                        ...
                        #Normalise values
                        if ('RV' not in prop_mode) and (plot_options['norm_out']) and (not np.isnan(val_out)):
                            val_obs/=val_out
                            eval_obs/=val_out            
                    ...
                    #-------------------------------------------------------
                    #Plot value
                    if (not plot_options['no_orig']) and (not plot_options['no_data']):
                        HDIval_obs = np.empty(0,dtype=float)
                        for i_loc in range(len(x_obs)): 
                            if plot_options['plot_err']:
                            ...
                            if plot_options['plot_HDI']:                            
                                data_exp =np.load(gen_dic['save_data_dir']+data_type+'_prop/'+inst+'_'+vis+'_mcmc/iexp'+str(i_loc)+'/merged_deriv_chains_walk'+np.str(plot_options[key_plot]['nwalkers'])+'_steps'+np.str(plot_options[key_plot]['nsteps'])+'.npz',allow_pickle=True)['data'].item() 
                                if prop_mode=='RVres':ipar = np_where1D(data_exp['var_par_list']=='rv')[0] 
                                ...
                                for HDI_sub in data_exp['HDI_interv'][ipar]:
                                    ...
                                    if prop_mode=='RVres':yerr_sub = (np.array(yerr_sub)-CCF_fit_vis[i_loc]['RVmod'])*1e3
                                ...
                                HDIval_obs=np.append(HDIval_obs,[[val_obs[i_loc]-yerr_sub_minmax[0]],[yerr_sub_minmax[1]-val_obs[i_loc]]])
                    ...
                    if (plot_options['print_disp'] or plot_options['plot_disp']) and (not plot_options['no_orig']):                        
                        ...
                        if (plot_options['print_disp']):
                            plt.text(0.2,1.1+dytxt,'w. mean['+prop_mode+'] ='+"{0:.5e}".format(mean_val_plot)+' +-'+"{0:.2e}".format(disp_from_mean),verticalalignment='center', horizontalalignment='center',fontsize=10.,zorder=10,color=col_loc,transform=plt.gca().transAxes) 
                        
                        if plot_options['plot_disp'] and ((data_mode=='raw') and (prop_mode not in ['rv','RVpip'])) or (data_mode=='loc'):
                        ...
                        if (prop_mode not in ['rv','RVpip']):
                            ...
                            if (prop_mode not in ['RVres','RVpipres']):
                        ...
                        if ('plot_fit_comm' in plot_options) and (inst in plot_options['plot_fit_comm']) and (vis in plot_options['plot_fit_comm'][inst]):
                            ...
                            ytxt_ref=2.
                            ygap=0.4
                            plt.text(xtxt_left,ytxt_ref-0.*ygap,'v$_\mathrm{eq}$ = '+"{0:.4f}".format(fit_allCCF_results['veq'])+'+-'+"{0:.4f}".format(fit_allCCF_results['err_veq']),verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4) 
                            plt.text(xtxt_left,ytxt_ref-1.*ygap,'$\lambda$ = '+"{0:.4f}".format(fit_allCCF_results['lamb'])+'+-'+"{0:.4f}".format(fit_allCCF_results['err_lamb']),verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4) 
                            plt.text(xtxt_left,ytxt_ref-2.*ygap,'i$_{*}$ = '+"{0:.4f}".format(fit_allCCF_results['istar'])+'+-'+"{0:.4f}".format(fit_allCCF_results['err_istar']),verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4) 
                            plt.text(xtxt_left,ytxt_ref-3.*ygap, r'$\alpha_{DR}$ = '+"{0:.4f}".format(fit_allCCF_results['alpha'])+'+-'+"{0:.4f}".format(fit_allCCF_results['err_alpha']),verticalalignment='center', horizontalalignment='left',fontsize=fontsize_info,zorder=4) 
                    ...
                    if (prop_mode in ['rv','RVpip','RV_lobe']) and (plot_options['prop_absc']=='phase') and plot_options['theoRV']:   
                        phase_RV_star,RV_star_solCDM = orb_motion_theoRV(pl_ref,system_param,gen_dic['kepl_pl'],coord_dic,inst,vis,data_dic)
                        shift_RV=np.nanmean([CCF_fit_vis[idx_loc]['rv_l2c'] for idx_loc in idx_out]) if prop_mode=='RV_lobe'  else 0. 
                    ...
                    if (((inst in plot_options['deg_prop_fit']) and (vis in plot_options['deg_prop_fit'][inst])) or (((inst in plot_options['fit_sin']) and (vis in plot_options['fit_sin'][inst])))) and cond_plot_fit:                      
                        print('       Fit versus')
    
                        #Absolute index for fit, in the plot-reduced table
                        #    - by default, out-of-transit points
                        if len(plot_options['idx_fit'][inst][vis])==0:plot_options['idx_fit'][inst][vis] = isub_out_plot
                        ...
                        p_guess = Parameters()
                        if prop_mode in ['rv','RVres']:p_guess.add_many(('a0', 0., True,None,None,  None)) 
                        ...
                        for iprop,prop_fit in enumerate(fixed_args['prop_fit']):
                            if prop_fit=='phase':var_prop = coord_vis['cen_ph']
                        ...
                        if len(fixed_args['prop_fit'])==1:
                            ...
                            plt.plot(fixed_args['var_fit'][plot_options['prop_absc']],p_mod,color='darkgrey',linestyle='-',lw=1,zorder=50)                            
                        else:
                            for prop_fit in fixed_args['prop_fit']:
                            ...
                            wsort = np.argsort(fixed_args['var_fit'][plot_options['prop_absc']])
                            plt.plot(fixed_args['var_fit'][plot_options['prop_absc']][wsort],p_mod[wsort],color='darkgrey',linestyle='-',lw=1,zorder=50)
                    ...
                    #-------------------------------------------------------           
                    #Master property 
                    if (prop_mode in ['RVres','RVpipres','FWHM','ctrst','amp','rv_l2c','amp_l2c','FWHM_l2c','amp_lobe','FWHM_lobe','area']) and (not plot_options['no_orig']) and \
                        ( (plot_options['plot_Mout'] and (data_dic['DI']['type'][inst]=='CCF')) or ( (plot_options['plot_Mloc'] and (data_dic['Res']['type'][inst]=='CCF')))):                                                
                        
                        if (data_mode=='raw') and (plot_options['norm_out'] or ('RV' in prop_mode)):
                            val_ref = val_out
                        else:
                            if plot_options['plot_Mout']:
                                Bin_CCF_fit_vis=(np.load(gen_dic['save_data_dir']+'DIbin_prop/'+inst+'_'+vis+'.npz',allow_pickle=True)['data'].item())[0]
                            if plot_options['plot_Mloc']:
                                Bin_CCF_fit_vis=(np.load(gen_dic['save_data_dir']+'Intrbin_prop/'+inst+'_'+vis+'.npz',allow_pickle=True)['data'].item())[0] 
                            val_ref=Bin_CCF_fit_vis[prop_loc]
                    ...
                    if (data_mode=='loc') and ('predic' in plot_options) and (len(plot_options['predic'])>0) and (prop_mode=='rv') and (plot_options['prop_absc']=='phase'):                 
                        ...
                        # CCFloc(rv,t) = CCFloc_cont(t)*(1 - CT_loc*exp( -(2*sqrt(ln2)*(rv-rv0)/FWHM_loc)^2 )) 
                        #      and its slope for rv = rv0+FWHM_loc is proportional to
                        # dCCFdv(t) ~ CCFloc_cont(t)*CT_loc/FWHM_loc = CCFref_cont*(1 - LC(t))*CT_loc/FWHM_loc  
                        ...
                        #      see def_weights_spatiotemp_bin(), fluxes are defined as F = g_inst*N for a given instrument, and the error on the local CCF flux approximates as:
                        ...
                        # SNR(t) = C*CCFloc_cont(t)/sigCCF(t)
                        ...
                        def sub_def_err_RVloc(const_fact,FWHM_loc,CT_loc,LC_exp,C_inst,texp):
                            return const_fact*( LC_exp/(1. - LC_exp) )*sqrt(1./C_inst)*(FWHM_loc/CT_loc)*10.**(star_params['mag']/5.)*sqrt(1./texp)
                    
                        #High-resolution light curve
                        data_upload = np.load(gen_dic['save_data_dir']+'Scaled_data/'+inst+'_'+vis+'_add.npz',allow_pickle=True)['data'].item() 
                        ...
                        for low_ph,high_ph in zip(st_x_obs[idx_in_plot],end_x_obs[idx_in_plot]):
                            ...
                            if (True in cond_in) and (True in cond_in_LC):
                                ...
                                if (~np.isnan(RV_exp)) and (LC_exp<1.):
                                    ...
                                    SNRpred_tab=np.append(SNRpred_tab,plot_options['C_RMR']*sqrt(plot_options['predic']['Cg_inst'][inst])*plot_options['predic']['ctrst']*((1 - LC_exp)/LC_exp)*10.**(-star_params['mag']/5.) *np.sqrt(texp))
                ...
                if (inst in plot_options['bin_val']) and (not plot_options['no_data']): 
        ...
        if ('all' in plot_options['bin_val']) and (not plot_options['no_data']): 
        ...
        if plot_options['plot_ref']:
           if (prop_mode in ['RVres','RVpipres']):val_ref = 0.
           elif plot_options['norm_out']:val_ref = 1.
           else:val_ref = None
        ...
        if plot_options['prop_absc'] not in x_title_dic:x_title_dic[plot_options['prop_absc']]=plot_options['prop_absc']
            
        y_title_dic={
                'RVpip':'RV pip. (km s$^{-1}$)',
                'RVpipres':'RV pip. res (m s$^{-1}$)',
                'FWHM':'FWHM (km s$^{-1}$)',
                'FWHM_voigt':'FWHM(Voigt) (km s$^{-1}$)',
                'FWHM_ord0__IS__VS_':'Local FWHM (deg 0) (km s$^{-1}$)',
                'FWHMpip':'FWHM pip. (km s$^{-1}$)',
        ...
        if data_mode=='raw':
            ...
            y_title_dic['RVres'] = 'RV res. (m s$^{-1}$)'
        elif data_mode=='loc':
            y_title_dic['rv'] = 'Surface RV (km s$^{-1}$)'
            y_title_dic['RVres'] =  'Surface RV residuals (m s$^{-1}$)'           
        ...
        if plot_options['plot_title']:
            ...
            elif data_mode=='loc':sub_key = 'intrinsic'
            plot_title_dic['rv'] = 'RV of '+sub_key+' CCFs'
            plot_title_dic['RVpip'] = 'RV pip. of '+sub_key+' CCFs'
            plot_title_dic['RVres'] = 'Residuals from RV of '+sub_key+' CCFs'
            plot_title_dic['RVpipres'] = 'RV pip. residuals of '+sub_key+' CCFs'
            plot_title_dic['FWHM'] = 'FWHM of '+sub_key+' CCFs'
            plot_title_dic['FWHMpip'] = 'FWHM pip. of '+sub_key+' CCFs'
            plot_title_dic['amp'] = 'Amplitude of '+sub_key+' CCFs'
            plot_title_dic['ctrst'] = 'Contrast of '+sub_key+' CCFs'
            plot_title_dic['Ctrstpip'] = 'Contrast pip. of '+sub_key+' CCFs'
        ...
        custom_axis(plt,position=plot_options['margins'],x_range=x_range_loc,y_range=y_range_loc,
                    ...
                    x_title=x_title_dic[plot_options['prop_absc']],y_title=y_title_dic[prop_mode],
                    font_size=plot_options['font_size'],xfont_size=plot_options['font_size'],yfont_size=plot_options['font_size'])
        if data_mode=='raw':sub_key = 'raw'
        elif data_mode=='loc':sub_key = 'loc'
        plt.savefig(path_loc+prop_mode+'_'+plot_options['prop_absc']+'.'+plot_dic['prop_'+sub_key]) 
        plt.close()
        print('   ---------------')
    ...
    def sub_plot_propCCF_mcmc_PDFs(plot_options,plot_prop,plot_type):    
        path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/CCF_'+plot_options['data_mode']+'_prop/MCMC/'
        if not os_system.path.exists(path_loc):os_system.makedirs(path_loc)  
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options['visits_to_plot'].keys())):                            
            ...
            for ivis,vis in enumerate(vis_list): 
                ...
                if orig_vis in list(data_dic[plot_options['data_dic_idx']][inst].keys()):
                    ...
                    #Common data
                    data_vis  = np.load(gen_dic['save_data_dir']+data_type+'_prop/'+inst+'_'+vis+'.npz',allow_pickle=True)['data'].item() 
                    ...
                    for plot_prop in plot_options['plot_prop_list']:
                        ...
                        for isub,iexp in enumerate(iexp_eff):
                            ...
                            data_exp =np.load(gen_dic['save_data_dir']+data_type+'_prop/'+inst+'_'+vis+'_mcmc/iexp'+str(iexp)+'/merged_deriv_chains_walk'+np.str(plot_options['nwalkers'])+'_steps'+np.str(plot_options['nsteps'])+'.npz',allow_pickle=True)['data'].item() 
                            ...
                            #Horzontal range
                            if plot_prop in plot_options['xrange_all']:x_range_loc = plot_options['xrange_all'][plot_prop]
                            ...
                            #Vertical range
                            if plot_options[key_plot]['yrange'] is None:y_range_loc = [0,1.1 * np.max(n)]
                            else:y_range_loc = plot_options[key_plot]['yrange']                        
                            ...
                            #Exposures index
                            if plot_options['plot_expid']:ax.text(x_range_loc[0]+0.07*(x_range_loc[1]-x_range_loc[0]),y_range_loc[0]+0.8*(y_range_loc[1]-y_range_loc[0]),str(iexp),verticalalignment='bottom', horizontalalignment='center',fontsize=0.8*plot_options['font_size'],zorder=4,color='black') 
                            ...
                            if (plot_prop in plot_options['xrange_all']):
                                  if (irow==nsub_rows-1) or ((irow==nsub_rows-2) & (comp_ax and (icol>((nexp_eff-1)%plot_options[key_plot]['nsub_col'] )))):ax.tick_params('x',labelsize=plot_options['font_size'])
                            ...
                            if (irow==nsub_rows-1) or ((irow==nsub_rows-2) & (comp_ax and (icol>((nexp_eff-1)%plot_options[key_plot]['nsub_col'] )))):
                        ...
                        for isub_comp in range(isub+1,nall):
                            irow = int(isub_comp/plot_options[key_plot]['nsub_col'])
                            icol = isub_comp%plot_options[key_plot]['nsub_col'] 
    ...
    def dist1D_stlines_CCFmasks(plot_options,key_plot,plot_ext):
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options['visits_to_plot'].keys())): 
            ...
            #Create directory if required
            path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/'+data_paths
            if not os_system.path.exists(path_loc):os_system.makedirs(path_loc)                 
            plt.ioff()        
            fig = plt.figure(figsize=plot_options['fig_size'])
    ...
    def gen_plot_default(plot_options,key_plot):
        ...
        #--------------------------------------   
        if (key_plot in ['DI_prof','DI_prof_res','CCF_DIbin','CCF_DIbin_res','CCFloc','CCFintr','sp_intr','CCFintr_res','CCF_Intrbin','CCF_Intrbin_res','CCFatm','CCFatm_res','CCF_Atmbin','CCF_Atmbin_res']):        
            for key in ['aligned','print_mes','ref_level']:
                plot_options[key_plot][key]=False
        ...
        #--------------------------------------              
        if (key_plot in ['Fbal_corr','input_LC','plocc_ranges','propCCF_DI_mcmc_PDFs','propCCFloc_mcmc_PDFs']):
        ...
        if ('prop_' in key_plot):
            ...
            #Reference level
            plot_options[key_plot]['plot_ref'] = False            
        ...
        if 'map_' in key_plot:
            ...
            #Scaling factor
            if ('map_DI' in key_plot) or (key_plot=='map_Intr_prof'):plot_options[key_plot]['sc_fact10']=0
            else:plot_options[key_plot]['sc_fact10']=3

            #Font size
            plot_options[key_plot]['font_size']=20  
            ...
            if 'map_res' in key_plot:plot_options[key_plot]['cmap']="rainbow"             
    ...
    '''
    Plotting detector gain estimates
    '''
    if (plot_dic['det_gain']!='') or (plot_dic['det_gain_ord']!=''):
        ...
        plot_options[key_plot]['plot_best_exp'] = True

        #Normalize gain profiles over all exposures
        plot_options[key_plot]['norm_exp'] = False
        
        #Plot mean gain if calculated
        plot_options[key_plot]['mean_gdet']  = True

        #Range per order
        #    - unfitted points are not shown in automatic ranges
        plot_options[key_plot]['x_range_ord'] = None
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options[key_plot]['visits_to_plot'].keys())): 
            ...
            if plot_options[key_plot]['mean_gdet'] and (not plot_options[key_plot]['norm_exp']):
                mean_gdet_func = (np.load(gen_dic['save_data_dir']+'Processed_data/Gains/'+inst+'_mean_gdet.npz', allow_pickle=True)['data'].item())['func']  

            for vis in np.intersect1d(list(data_dic[inst].keys()),plot_options[key_plot]['visits_to_plot'][inst]): 
                data_vis = data_dic[inst][vis]

                #Create directory if required
                path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/General/Det_gain/'+inst+'/'+vis+'/'
                if not os_system.path.exists(path_loc):os_system.makedirs(path_loc)  

                #Orders to process
                if ('orders_to_plot' in plot_options[key_plot]) and (len(plot_options[key_plot]['orders_to_plot'])>0):order_list = plot_options[key_plot]['orders_to_plot']
                else:order_list=range(data_dic[inst]['nord']) 
                nord_list = len(order_list)
            
                #Exposures to plot
                if ('iexp_plot' in plot_options[key_plot]) and (inst in plot_options[key_plot]['iexp_plot']) and (len(plot_options[key_plot]['iexp_plot'][inst][vis])>0):iexp_plot = plot_options[key_plot]['iexp_plot'][inst][vis]
                else:iexp_plot=range(data_dic[inst][vis]['n_in_visit'])
                ...
                for isub_iexp,iexp in enumerate(iexp_plot):
                    data_load = np.load(data_vis['gain_data_paths']+str(iexp)+'.npz', allow_pickle=True)['data'].item()  
                    ...
                    for isub_ord,iord in enumerate(order_list):
                        ...
                        if len(cond_def_exp[iord]):
                            ...
                            gdet_bins_all[isub_ord,isub_iexp] = gain_piecewise_func(data_load['gdet_inputs'][iord]['par'],cen_bins_all[isub_ord,isub_iexp],args=data_load['gdet_inputs'][iord]['args'])    

                        #Mean gain per order (from measurements)
                        if (plot_dic[key_plot]!=''):
                            ...
                            gdet_bins_mean[isub_ord,isub_iexp] = np.mean(gain_piecewise_func(data_load['gdet_inputs'][iord]['par'],data_load['wav_bin_all'][iord],args=data_load['gdet_inputs'][iord]['args']))   
                                
                #Mean gain per order (from model, for normalization)
                if plot_options[key_plot]['norm_exp']:norm_gdet = gdet_bins_mean
                ...
                if (plot_dic[key_plot]!=''):
                    ...
                    #Mean flux gain over each order, for all orders and all exposures 
                    for isub_iexp in range(len(iexp_plot)):
                        if ('spec' in data_dic['DI']['type'][inst]):plt.plot(gdet_cen_binned_mean[:,isub_iexp],gdet_meas_binned_mean[:,isub_iexp],linestyle='',marker='o',markerfacecolor=col_visit[isub_iexp],markeredgecolor=col_visit[isub_iexp],markersize=plot_options[key_plot]['markersize'],rasterized=plot_options[key_plot]['rasterized'],color = col_visit[isub_iexp])
                        plt.plot(cen_bins_mean[:,isub_iexp],gdet_bins_mean[:,isub_iexp],linestyle='-',marker='.',markerfacecolor=col_visit[isub_iexp],markeredgecolor=col_visit[isub_iexp],markersize=plot_options[key_plot]['markersize'],rasterized=plot_options[key_plot]['rasterized'],color = col_visit[isub_iexp])            
                    x_range_loc = plot_options[key_plot]['x_range'] if plot_options[key_plot]['x_range'] is not None else [np.nanmin(cen_bins_mean),np.nanmax(cen_bins_mean)] 
                    ...
                    #Plot frame  
                    if plot_options[key_plot]['plot_title']:plt.title('Flux gain for visit '+vis+' in '+inst)
                    dx_range=x_range_loc[1]-x_range_loc[0]
                    dy_range=y_range_loc[1]-y_range_loc[0]
                    xmajor_int,xminor_int,xmajor_form = autom_x_tick_prop(dx_range)
                    ymajor_int,yminor_int,ymajor_form = autom_y_tick_prop(dy_range)                    
                    custom_axis(plt,position=plot_options[key_plot]['margins'],x_range=x_range_loc,y_range=y_range_loc,dir_y='out', 
                                xmajor_int=xmajor_int,xminor_int=xminor_int,ymajor_int=ymajor_int,yminor_int=yminor_int,
                                xmajor_form=xmajor_form,ymajor_form=ymajor_form,hide_axis=plot_options[key_plot]['hide_axis'],
                                x_title='Wavelength (A)',y_title=r'10$^{-5}$ Gain',
                    ...
                    plt.savefig(path_loc+'Global'+'.'+plot_dic[key_plot],transparent=plot_options[key_plot]['transparent']) 
                    plt.close()                     

                #---------------------------------------------------------------------------------------------------------------

                #Plot current order for all exposures
                if (plot_dic['det_gain_ord']!='') and ('spec' in data_dic['DI']['type'][inst]):
                    for isub_ord,iord in enumerate(order_list):
                        plt.ioff()        
                        fig = plt.figure(figsize=(10, 6)) #,dpi=10)
    
                        #Flux gain estimate (measured and modelled)
                        y_min=1e100
                        y_max=-1e100
                        for isub_iexp in range(len(iexp_plot)):
                            x_range_loc = plot_options[key_plot]['x_range_ord'] if plot_options[key_plot]['x_range_ord'] is not None else [np.nanmin(cen_bins_all[isub_ord,isub_iexp]),np.nanmax(cen_bins_all[isub_ord,isub_iexp])] 
                                
                            #Mesured gain estimate
                            ...
                            if len(gdet_meas_binned_all[isub_ord,isub_iexp])>0:
                                ...
                                plt.plot(gdet_cen_binned_all[isub_ord,isub_iexp][cond_fit_ord],gdet_meas_binned_all[isub_ord,isub_iexp][cond_fit_ord]/norm_gdet[isub_ord,isub_iexp],linestyle='',marker='o',markerfacecolor=col_visit[isub_iexp],markeredgecolor=col_visit[isub_iexp],markersize=plot_options[key_plot]['markersize'],zorder=0,rasterized=plot_options[key_plot]['rasterized'])
                                plt.plot(gdet_cen_binned_all[isub_ord,isub_iexp][~cond_fit_ord],gdet_meas_binned_all[isub_ord,isub_iexp][~cond_fit_ord]/norm_gdet[isub_ord,isub_iexp],linestyle='',marker='o',markerfacecolor='None',markeredgecolor=col_visit[isub_iexp],markersize=plot_options[key_plot]['markersize'],zorder=0,rasterized=plot_options[key_plot]['rasterized'])
                            ...
                            if plot_options[key_plot]['plot_best_exp'] and (gdet_bins_all[isub_ord,isub_iexp] is not None):
                                plt.plot(cen_bins_all[isub_ord,isub_iexp],gdet_bins_all[isub_ord,isub_iexp]/norm_gdet[isub_ord,isub_iexp],linestyle='-',color=col_visit[isub_iexp],lw=0.5,rasterized=plot_options[key_plot]['rasterized'],zorder=0,alpha=0.5) 
                        ...
                        #Mean gain profile over all exposures
                        ...
                        if plot_options[key_plot]['mean_gdet'] and (not plot_options[key_plot]['norm_exp']):
                            plt.plot(cen_bins_all[isub_ord,0],mean_gdet_func[iord](cen_bins_all[isub_ord,0])*1e-3,linestyle='--',color='black',lw=1,rasterized=plot_options[key_plot]['rasterized'],zorder=10) 

                        #Transitions of polynomials
                        y_range_loc=plot_options[key_plot]['y_range_ord'] if plot_options[key_plot]['y_range_ord'] is not None else np.array([y_min,y_max])
                        for isub_iexp in range(len(iexp_plot)):
                            plt.plot([wav_trans_all[0,isub_ord,isub_iexp],wav_trans_all[0,isub_ord,isub_iexp]],y_range_loc,linestyle='--',color=col_visit[isub_iexp],lw=0.5,rasterized=plot_options[key_plot]['rasterized'])
                        ...
                        #Plot frame                          
                        if plot_options[key_plot]['plot_title']:plt.title('Flux gain for visit '+vis+' in '+inst+' (order '+str(iord)+')')
                        dx_range=x_range_loc[1]-x_range_loc[0]
                        dy_range=y_range_loc[1]-y_range_loc[0]
                        xmajor_int,xminor_int,xmajor_form = autom_x_tick_prop(dx_range)
                        ymajor_int,yminor_int,ymajor_form = autom_y_tick_prop(dy_range)
                        custom_axis(plt,position=plot_options[key_plot]['margins'],x_range=x_range_loc,y_range=y_range_loc,dir_y='out', 
                                    xmajor_int=xmajor_int,xminor_int=xminor_int,ymajor_int=ymajor_int,yminor_int=yminor_int,
                                    xmajor_form=xmajor_form,ymajor_form=ymajor_form,
                                    x_title='Wavelength (A)',y_title=r'10$^{-3}$ Gain',hide_axis=plot_options[key_plot]['hide_axis'],
                                    font_size=plot_options[key_plot]['font_size'],xfont_size=plot_options[key_plot]['font_size'],yfont_size=plot_options[key_plot]['font_size'])
                        plt.savefig(path_loc+'ord'+str(iord)+'.'+plot_dic['det_gain_ord'],transparent=plot_options[key_plot]['transparent']) 
                        plt.close() 
    ...
    if gen_dic['specINtype'] and (plot_dic['glob_mast']!=''):
        ...
        plot_options[key_plot]['glob_mast_all']=None 
        ...
        sub_plot_all_prof(plot_options[key_plot],key_plot,plot_dic[key_plot])   
    ...
    '''
    Plotting master spectrum used for weighing
    '''
    if gen_dic['specINtype'] and (plot_dic['DImast']!=''):
        ...
        sub_plot_spec(plot_options[key_plot],key_plot,plot_dic[key_plot])   
    ...
    if gen_dic['specINtype'] and (plot_dic['tell_CCF']!=''):
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options[key_plot]['visits_to_plot'].keys())): 
            print('      - Instrument :',inst)
            if inst not in plot_options[key_plot]['color_dic']:plot_options[key_plot]['color_dic'][inst]={}
            if inst not in plot_options[key_plot]['color_dic_sec']:plot_options[key_plot]['color_dic_sec'][inst]={}
            for vis in np.intersect1d(list(data_dic[inst].keys()),plot_options[key_plot]['visits_to_plot'][inst]): 
                print('        - Visit :',vis)
                if vis not in plot_options[key_plot]['color_dic'][inst]:plot_options[key_plot]['color_dic'][inst][vis]='red'
                if vis not in plot_options[key_plot]['color_dic_sec'][inst]:plot_options[key_plot]['color_dic_sec'][inst][vis]='dodgerblue'
                
                #Create directory if required
                path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Spec_raw/Tell_corr/'+inst+'_'+vis+'/Tell_CCF/'
                if not os_system.path.exists(path_loc):os_system.makedirs(path_loc)  

                #Exposures to plot
                if (inst in plot_options[key_plot]['iexp_plot']) and (vis in plot_options[key_plot]['iexp_plot'][inst]) and (len(plot_options[key_plot]['iexp_plot'][inst][vis])>0):iexp_plot_vis = deepcopy(plot_options[key_plot]['iexp_plot'][inst][vis])
                else:iexp_plot_vis=np.arange(data_dic[inst][vis]['n_in_visit'])
                if plot_options[key_plot]['print_disp'] is not None:
                    disp_dic = {}
                    for molec in plot_options[key_plot]['tell_species']: disp_dic[molec] = np.zeros(len(iexp_plot_vis),dtype=float)*np.nan
                    ...
                    if len(data_path)>0:
                        ...
                        for molec in plot_options[key_plot]['tell_species']:                           
                            if molec in data_exp:
                                ...
                                plt.plot(data_exp['velccf'],data_exp[molec]['ccf_uncorr_master'],color=plot_options[key_plot]['color_dic'][inst][vis],linestyle='-',lw=plot_options[key_plot]['lw_plot'],zorder=0)   
                                plt.plot(data_exp['velccf'],data_exp[molec]['ccf_model_conv_master'],color='green',linestyle='-',lw=plot_options[key_plot]['lw_plot'],zorder=1)   
                                plt.plot(data_exp['velccf'],data_exp[molec]['ccf_corr_master'],color=plot_options[key_plot]['color_dic_sec'][inst][vis],linestyle='-',lw=plot_options[key_plot]['lw_plot'],zorder=1)   
                    
                                if plot_options[key_plot]['x_range'] is None:
                                    x_min=np.min([np.min(data_exp['velccf']),x_min])
                                    x_max=np.max([np.max(data_exp['velccf']),x_max]) 
                                if plot_options[key_plot]['y_range'] is None:
                                ...
                                #Plot dispersion  
                                if plot_options[key_plot]['print_disp'] is not None:
                                    cond_disp = (~np.isnan(data_exp[molec]['ccf_corr_master'])) & (data_exp['velccf']>=plot_options[key_plot]['print_disp'][0]) & (data_exp['velccf']<=plot_options[key_plot]['print_disp'][1])
                                ...
                                #Frame
                                x_range_loc=plot_options[key_plot]['x_range'] if plot_options[key_plot]['x_range'] is not None else np.array([x_min,x_max])
                                if (plot_options[key_plot]['y_range'] is not None) and (inst in plot_options[key_plot]['y_range']) and (vis in plot_options[key_plot]['y_range'][inst]) and (molec in plot_options[key_plot]['y_range'][inst][vis]):
                                    y_range_loc = plot_options[key_plot]['y_range'][inst][vis][molec]
                                else:y_range_loc=np.array([y_min,y_max])                            
                                dx_range=x_range_loc[1]-x_range_loc[0]        
                                dy_range=y_range_loc[1]-y_range_loc[0]                         
                                xmajor_int,xminor_int,xmajor_form = autom_x_tick_prop(dx_range)
                                ymajor_int,yminor_int,ymajor_form = autom_y_tick_prop(dy_range)
                                custom_axis(plt,position=plot_options[key_plot]['margins'],x_range=x_range_loc,y_range=y_range_loc,xmajor_int=xmajor_int,xminor_int=xminor_int,ymajor_int=ymajor_int,yminor_int=yminor_int,xmajor_form=xmajor_form,ymajor_form=ymajor_form,
                                            x_title=r'Velocity in telluric rest frame (km s$^{-1}$)',y_title='Flux',font_size=plot_options[key_plot]['font_size'],xfont_size=plot_options[key_plot]['font_size'],yfont_size=plot_options[key_plot]['font_size'])
                ...
                if plot_options[key_plot]['print_disp'] is not None:
                    for molec in plot_options[key_plot]['tell_species']:  
                        ...
                        cen_ph_vis = coord_dic[inst][vis][plot_options[key_plot]['pl_ref'][inst][vis]]['cen_ph'][cond_def]
                        ...
                        ymajor_int,yminor_int,ymajor_form = autom_y_tick_prop(dy_range)
                        custom_axis(plt,position=plot_options[key_plot]['margins'],x_range=x_range_loc,y_range=y_range_loc,xmajor_int=xmajor_int,xminor_int=xminor_int,ymajor_int=ymajor_int,yminor_int=yminor_int,xmajor_form=xmajor_form,ymajor_form=ymajor_form,
                                    x_title=r'Orbital Phase',y_title='Dispersion',font_size=plot_options[key_plot]['font_size'],xfont_size=plot_options[key_plot]['font_size'],yfont_size=plot_options[key_plot]['font_size'])
    ...
    if gen_dic['specINtype'] and (plot_dic['tell_prop']!=''):
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options[key_plot]['visits_to_plot'].keys())): 
            ...
            for vis in np.intersect1d(list(data_dic[inst].keys()),plot_options[key_plot]['visits_to_plot'][inst]): 
                print('        - Visit :',vis)
                if vis not in plot_options[key_plot]['color_dic'][inst]:plot_options[key_plot]['color_dic'][inst][vis]='dodgerblue'
                data_vis = data_dic[inst][vis]
                cen_ph_vis = coord_dic[inst][vis][plot_options[key_plot]['pl_ref'][inst][vis]]['cen_ph']
                
                #Create directory if required
                path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Spec_raw/Tell_corr/'+inst+'_'+vis+'/Tell_prop/'
                if not os_system.path.exists(path_loc):os_system.makedirs(path_loc)  
                    ...
                    for key in ['Temperature','IWV_LOS','Pressure_ground']:
                        ...
                        if True in cond_def:
                            ...
                            if plot_settings[key_plot]['plot_err']:
                                ...
                                plt.errorbar(cen_ph_vis[cond_def][sub_cond_def],tell_prop[molec][key][0][cond_def][sub_cond_def],yerr=tell_prop[molec][key][1][cond_def][sub_cond_def],linestyle='',color=plot_options[key_plot]['color_dic'][inst][vis],rasterized=plot_options[key_plot]['rasterized'],zorder=0,alpha=plot_options[key_plot]['alpha_err'])                       
                            else:sub_cond_def = np.repeat(True,np.sum(cond_def))
                            plt.plot(cen_ph_vis[cond_def],tell_prop[molec][key][0][cond_def],color=plot_options[key_plot]['color_dic'][inst][vis],linestyle='',zorder=1,marker='o',markersize=1.5)
                     
                            #Frame
                            if plot_options[key_plot]['x_range'] is not None:
                                x_range_loc=plot_options[key_plot]['x_range']
                            else:
                                xmin = cen_ph_vis[cond_def][0]
                                xmax = cen_ph_vis[cond_def][-1]
                                dx_range = xmax-xmin
                                x_range_loc = [xmin-0.05*dx_range,xmax+0.05*dx_range]  
                            if (plot_options[key_plot]['y_range'] is not None) and (inst in plot_options[key_plot]['y_range']) and (vis in plot_options[key_plot]['y_range'][inst]) and (molec in plot_options[key_plot]['y_range'][inst][vis]) and (key in plot_options[key_plot]['y_range'][inst][vis][molec]):
                                y_range_loc = plot_options[key_plot]['y_range'][inst][vis][molec][key]
                            else:
                            ...
                            custom_axis(plt,position=plot_options[key_plot]['margins'],x_range=x_range_loc,y_range=y_range_loc,xmajor_int=xmajor_int,xminor_int=xminor_int,ymajor_int=ymajor_int,yminor_int=yminor_int,xmajor_form=xmajor_form,ymajor_form=ymajor_form,
                                        x_title='Orbital phase',y_title=y_title,font_size=plot_options[key_plot]['font_size'],xfont_size=plot_options[key_plot]['font_size'],yfont_size=plot_options[key_plot]['font_size'])
    ...
    if gen_dic['specINtype'] and (plot_dic['Fbal_corr']!=''):
        ...
        #Plot order indexes
        plot_options[key_plot]['plot_idx_ord'] = True
        
        #Indexes of exposures and bins to be plotted 
        plot_options[key_plot]['ibin_plot'] = {}
          
        #Overplot all exposures or offset them
        plot_options[key_plot]['gap_exp']=0.        

        #Strip range used for correction
        plot_options[key_plot]['strip_corr'] = False
        ...
        #Create directory if required
        path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Spec_raw/FluxBalance/'
        if not os_system.path.exists(path_loc):os_system.makedirs(path_loc)  
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options[key_plot]['visits_to_plot'].keys())): 
            ...
            for vis in np.intersect1d(list(data_dic[inst].keys()),plot_options[key_plot]['visits_to_plot'][inst]): 
                ...
                if plot_settings[key_plot]['gap_exp']==0.:plot_settings[key_plot]['margins']=[0.15,0.15,0.95,0.7] 
                else:plot_settings[key_plot]['margins']=[0.1,0.02,0.95,0.95] 
                    
                #Visit color
                n_in_visit=data_vis['n_in_visit']
                if (inst in plot_options[key_plot]['color_dic']) and (vis in plot_options[key_plot]['color_dic'][inst]):
                    col_visit=np.repeat(plot_options[key_plot]['color_dic'][inst][vis],n_in_visit)
                else:
                    cmap = plt.get_cmap('rainbow') 
                    col_visit=np.array([cmap(0)]) if n_in_visit==1 else cmap( np.arange(n_in_visit)/(n_in_visit-1.))

                plt.ioff()        
                if plot_settings[key_plot]['gap_exp']==0.:fig = plt.figure(figsize=(10, 6))
                ...
                #Horizontal range
                if plot_options[key_plot]['x_range'] is not None:x_range_loc = deepcopy(plot_options[key_plot]['x_range'])
                else:
                    data_com = np.load(data_vis['proc_com_data_paths']+'.npz',allow_pickle=True)['data'].item() 
                    if plot_options[key_plot]['sp_var'] == 'nu' :x_range_loc = [c_light/data_com['edge_bins'][-1,-1],c_light/data_com['edge_bins'][0,0]]
                    elif plot_options[key_plot]['sp_var'] == 'wav' :x_range_loc = [data_com['edge_bins'][0,0],data_com['edge_bins'][-1,-1]]
                if plot_options[key_plot]['plot_model']:
                    if plot_options[key_plot]['sp_var'] == 'wav' :min_wav,max_wav = x_range_loc[0],x_range_loc[1]
                    elif plot_options[key_plot]['sp_var'] == 'nu' :min_wav,max_wav = c_light/x_range_loc[1],c_light/x_range_loc[0] 
                    nspec_plot = int( np.floor(   np.log(max_wav/min_wav)/np.log( plot_options[key_plot]['dlnw_plot'] + 1. ) )  ) 
                    cen_bin_plot = min_wav*( plot_options[key_plot]['dlnw_plot'] + 1. )**(0.5+np.arange(nspec_plot))  
                ...
                for isub,iexp in enumerate(iexp_plot_vis):
                    lev_exp = isub*plot_settings[key_plot]['gap_exp'] 
                    ...
                    if (inst in plot_options[key_plot]['ibin_plot']) and (vis in plot_options[key_plot]['ibin_plot'][inst]) and (len(plot_options[key_plot]['ibin_plot'][inst][vis])>0):ibin_exp = list(plot_options[key_plot]['ibin_plot'][inst][vis])
                    else:ibin_exp=range(len(data_Fbal['Fbal_wav_bin_all'][1])) 

                    #Mesured ratio binned exposure/binned master
                    if plot_options[key_plot]['plot_data']:
                        ...
                        if plot_options[key_plot]['sp_var'] == 'nu' :Fbal_cen_bin_plot = c_light/data_Fbal['Fbal_wav_bin_all'][1,ibin_exp]
                        elif plot_options[key_plot]['sp_var'] == 'wav' :Fbal_cen_bin_plot = data_Fbal['Fbal_wav_bin_all'][1,ibin_exp]
                        ...
                        for cen_bin,val_bin,cond_bin in zip(Fbal_cen_bin_plot,Fbal_T_binned_plot,cond_fit_plot):
                            ...
                            plt.plot(cen_bin,lev_exp+val_bin,linestyle='',marker='o',markerfacecolor=markerfacecolor  ,markeredgecolor=col_visit[iexp],markersize=plot_options[key_plot]['markersize'],rasterized=plot_options[key_plot]['rasterized'])

                    #Best-fit model
                    if plot_options[key_plot]['plot_model']:
                        y_mod = data_Fbal['corr_func'](nu_bin_plot)
                        if plot_options[key_plot]['sp_var'] == 'nu' :mod_cen_bin_plot = nu_bin_plot
                        elif plot_options[key_plot]['sp_var'] == 'wav' :mod_cen_bin_plot = cen_bin_plot
                        plt.plot(mod_cen_bin_plot,lev_exp+y_mod,linestyle='-',color=col_visit[iexp],lw=1) 

                    #Plot exposure indexes
                    if plot_options[key_plot]['plot_expid']:
                        if plot_options[key_plot]['plot_data']:ytxt = np.median(Fbal_T_binned_plot)
                        if plot_options[key_plot]['plot_model']: ytxt = np.median(y_mod)
                ...
                #Shade area not included within fit
                if plot_options[key_plot]['shade_unfit']:
                    if (inst not in gen_dic['Fbal_range_fit']) or ((inst in gen_dic['Fbal_range_fit']) & (vis not in gen_dic['Fbal_range_fit'][inst])):fit_range=[x_range_loc]
                        ...
                        if plot_options[key_plot]['sp_var'] == 'nu' :fit_range=c_light/gen_dic['Fbal_range_fit'][inst][vis]  
                        elif plot_options[key_plot]['sp_var'] == 'wav' :fit_range=gen_dic['Fbal_range_fit'][inst][vis]       
                ...
                if plot_options[key_plot]['strip_corr']:
                    if len(gen_dic['Fbal_range_corr'])==0:Fbal_range_corr=[x_range_loc]
                    else:
                        if plot_options[key_plot]['sp_var'] == 'nu' :Fbal_range_corr=c_light/gen_dic['Fbal_range_corr']
                        elif plot_options[key_plot]['sp_var'] == 'wav' :Fbal_range_corr=gen_dic['Fbal_range_corr']                    
                ...
                dy_range=y_range_loc[1]-y_range_loc[0]
                if plot_options[key_plot]['plot_idx_ord']:
                    if plot_options[key_plot]['sp_var'] == 'nu' :cen_ord = c_light/gen_dic['wav_ord_inst'][inst]
                    elif plot_options[key_plot]['sp_var'] == 'wav' :cen_ord = gen_dic['wav_ord_inst'][inst]                
                    delt_txt = 0.03 if plot_settings[key_plot]['gap_exp'] == 0. else 0.005
                    # ord_freq = 4
                    # fontsize_ord = 6.
                    ord_freq = 8
                    fontsize_ord = 11.
                ...
                xmajor_int,xminor_int,xmajor_form = autom_x_tick_prop(dx_range)
                ymajor_int,yminor_int,ymajor_form = autom_y_tick_prop(dy_range)     
                if plot_options[key_plot]['sp_var'] == 'nu' :x_title=r'$\nu$ (10$^{-10}$s$^{-1}$)'
                elif plot_options[key_plot]['sp_var'] == 'wav' :x_title='Wavelength (A)'
                custom_axis(plt,position=plot_settings[key_plot]['margins'],x_range=x_range_loc,y_range=y_range_loc,dir_y='out', 
                            xmajor_int=xmajor_int,xminor_int=xminor_int,xmajor_form=xmajor_form,
    ...
    '''
    DRS global flux balance (exposures)
    '''
    if gen_dic['specINtype'] and (plot_dic['Fbal_corr_DRS']!=''):
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options[key_plot]['visits_to_plot'].keys())): 
            ...
            wav_ord_inst = gen_dic['wav_ord_inst'][inst] 
            ...
            for vis in np.intersect1d(list(data_dic[inst].keys()),plot_options[key_plot]['visits_to_plot'][inst]):
                ...
                for iexp in range(n_in_visit):
                    ...
                    plt.plot(wav_ord_inst,Fbal_T_exp,linestyle='-',marker='s',markerfacecolor=col_visit[iexp],markeredgecolor=col_visit[iexp],markersize=plot_options[key_plot]['markersize'] )
    ...
    if gen_dic['specINtype'] and (plot_dic['Fbal_corr_vis']!=''):
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options[key_plot]['visits_to_plot'].keys())): 
            ...
            plt.ioff()        
            if plot_settings[key_plot]['gap_exp']==0.:fig = plt.figure(figsize=(10, 4))
            ...
            #Process each visit
            for isub,vis in enumerate(np.intersect1d(list(data_dic[inst].keys()),plot_options[key_plot]['visits_to_plot'][inst])): 
                data_vis = data_dic[inst][vis]
  
                #Visit color
                if (inst in plot_options[key_plot]['color_dic']) and (vis in plot_options[key_plot]['color_dic'][inst]):
                    col_visit=plot_options[key_plot]['color_dic'][inst][vis]
                else:
                ...
                if plot_options[key_plot]['plot_data']:
                    ...
                    if plot_options[key_plot]['sp_var'] == 'nu' :Fbal_cen_bin_plot = c_light/data_Fbal['Fbal_wav_bin_vis'][ibin_exp]
                    elif plot_options[key_plot]['sp_var'] == 'wav' :Fbal_cen_bin_plot = data_Fbal['Fbal_wav_bin_vis'][ibin_exp]
                    y_min=np.min([np.min(lev_exp+Fbal_T_binned_plot),y_min])
                    y_max=np.max([np.max(lev_exp+Fbal_T_binned_plot),y_max]) 
                    for cen_bin,val_bin,cond_bin in zip(Fbal_cen_bin_plot,Fbal_T_binned_plot,cond_fit_plot):
                        markerfacecolor = col_visit if cond_bin else 'None'
                        plt.plot(cen_bin,lev_exp+val_bin,linestyle='',marker='o',markerfacecolor=markerfacecolor  ,markeredgecolor=col_visit,markersize=plot_options[key_plot]['markersize'],rasterized=plot_options[key_plot]['rasterized'])
    ...
    '''
    Intra-order flux balance correction
    '''
    if gen_dic['specINtype'] and (plot_dic['Fbal_corr_ord']!=''):
        ...
        #Create directory if required
        path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Spec_raw/FluxBalance/Orders/'
        if not os_system.path.exists(path_loc):os_system.makedirs(path_loc)  
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options[key_plot]['visits_to_plot'].keys())): 
            ...
            for vis in np.intersect1d(list(data_dic[inst].keys()),plot_options[key_plot]['visits_to_plot'][inst]): 
                ...
                if len(plot_options[key_plot]['orders_to_plot'])>0:order_list = np.intersect1d(order_list,data_Fbal_gen['iord_corr_list'])
                nord_list = len(order_list)
                ...
                for isub_ord,iord in enumerate(order_list):
                    ...
                    for iexp in range(n_in_visit):
                        ...
                        plt.plot(Fbal_wav_bin_all[isub_ord][iexp],var_plot,linestyle='',marker='o',markerfacecolor=col_visit[iexp],markeredgecolor=col_visit[iexp],markersize=plot_options[key_plot]['markersize'])
    ...
    if gen_dic['specINtype'] and (plot_dic['Ftemp_corr']!=''):
        ...
        #Create directory if required
        path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Spec_raw/Ftemp_corr/'
        if not os_system.path.exists(path_loc):os_system.makedirs(path_loc)  
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options[key_plot]['visits_to_plot'].keys())): 
            ...
            for vis in np.intersect1d(list(data_dic[inst].keys()),plot_options[key_plot]['visits_to_plot'][inst]): 
                ...
                for iexp in range(data_vis['n_in_visit']):
                    ...
                    if plot_options[key_plot]['plot_err']:plt.errorbar(data_Ftemp['bjd_all'],data_Ftemp['Tflux_all'],yerr=data_Ftemp['Tsig_all'],linestyle='',color=col_visit[iexp],rasterized=plot_options[key_plot]['rasterized'],zorder=0,alpha=plot_options[key_plot]['alpha_err'])                       
                    plt.plot(data_Ftemp['bjd_all'],data_Ftemp['Tflux_all'],linestyle='',marker='o',markerfacecolor=col_visit[iexp]  ,markeredgecolor=col_visit[iexp],markersize=plot_options[key_plot]['markersize'])

                #Best-fit 
                plt.plot(data_Ftemp['bjd_all'],data_Ftemp['corr_func'](data_Ftemp['bjd_all']),linestyle='-',color=col_visit[iexp],lw=1,rasterized=plot_options[key_plot]['rasterized']) 
    ...
    if gen_dic['specINtype'] and (plot_dic['cosm_corr']!=''):
        ...
        #Cosmic marker
        plot_options[key_plot]['markcosm'] = True  
        plot_options[key_plot]['ncosm'] = True  

        #Plot orders with detected cosmics only  
        plot_options[key_plot]['detcosm'] = True
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options[key_plot]['visits_to_plot'].keys())): 
            ...
            for vis in np.intersect1d(list(data_dic[inst].keys()),plot_options[key_plot]['visits_to_plot'][inst]): 
                print('     - Visit : '+vis)
                path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Spec_raw/Cosmics/'+inst+'_'+vis+'/'
                if not os_system.path.exists(path_loc):os_system.makedirs(path_loc)                 
                data_vis=data_dic[inst][vis]
                ...
                #Orders to plot
                order_list = plot_options[key_plot]['orders_to_plot'] if len(plot_options[key_plot]['orders_to_plot'])>0 else range(data_dic[inst]['nord_ref']) 
                ...
                for isub,iexp in enumerate(iexp_plot_vis):                
                    ...
                    for isub_ord,iord in enumerate(order_list):
                        ...
                        if (not plot_options[key_plot]['detcosm']) or (n_cosmics_vis[isub,isub_ord]>0):                         
                            wav_tab=data_exp['cen_bins'][iord]     
                            plt.ioff()        
                            plt.figure(figsize=(10, 6))
                                
                            #Plot boundaries
                            x_range_loc = plot_options[key_plot]['x_range'] if plot_options[key_plot]['x_range'] is not None else [wav_tab[0],wav_tab[-1]]
                            y_range_loc = plot_options[key_plot]['y_range'] if plot_options[key_plot]['y_range'] is not None else [0.,np.nanmax(data_cosm['SNR_diff_exp'][:,iord])]
                            dy_range=y_range_loc[1]-y_range_loc[0]
                            ...
                            plt.plot(wav_tab,data_cosm['SNR_diff_exp'][0,iord],linestyle='-',color='black',rasterized=plot_options[key_plot]['rasterized'],zorder=0,drawstyle='steps-mid',lw=plot_options[key_plot]['lw_plot'])
                            plt.plot(wav_tab,data_cosm['SNR_diff_exp'][1,iord],linestyle='-',color='limegreen',rasterized=plot_options[key_plot]['rasterized'],zorder=1,drawstyle='steps-mid',lw=plot_options[key_plot]['lw_plot'])
                 
                            #Cosmics
                            if n_cosmics_vis[isub,isub_ord]>0:
                                if plot_options[key_plot]['markcosm']:plt.plot(wav_tab[idx_cosm_tab],data_cosm['SNR_diff_exp'][0,iord][idx_cosm_tab],linestyle='',color='red',marker='o',markersize=plot_options[key_plot]['markersize'],zorder=2)
                                if plot_options[key_plot]['ncosm']:plt.text(x_range_loc[0]+0.1*(x_range_loc[1]-x_range_loc[0]),y_range_loc[0]+0.9*dy_range,'N(cosmics) ='+"{:d}".format(n_cosmics_vis[isub,isub_ord]),verticalalignment='center', horizontalalignment='left',fontsize=15.,zorder=4) 
                ...
                if plot_options[key_plot]['cosm_vs_ord']:
                    plt.ioff()        
                    plt.figure(figsize=(10, 6))
                    cmap = plt.get_cmap('rainbow') 
                    ...
                    #Plot boundaries
                    x_range_loc = plot_options[key_plot]['x_range'] if plot_options[key_plot]['x_range'] is not None else [gen_dic['wav_ord_inst'][inst][order_list][0],gen_dic['wav_ord_inst'][inst][order_list][-1]]
                    y_range_loc = plot_options[key_plot]['y_range'] if plot_options[key_plot]['y_range'] is not None else [0.,np.max(n_cosmics_vis)]
                            
                    #Plot number of cosmics for each exposure and the total count (scaled to the axis range)
                    for isub,iexp in enumerate(iexp_plot_vis): 
                        plt.plot(gen_dic['wav_ord_inst'][inst][order_list],n_cosmics_vis[isub],linestyle='-',color=col_tab[isub],rasterized=plot_options[key_plot]['rasterized'],zorder=0,drawstyle='steps-mid',lw=plot_options[key_plot]['lw_plot'])
    ...
    if gen_dic['specINtype'] and (plot_dic['permpeak_corr']!=''):
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options[key_plot]['visits_to_plot'].keys())): 
            ...
            for vis in np.intersect1d(list(data_dic[inst].keys()),plot_options[key_plot]['visits_to_plot'][inst]): 
                ...
                #Create directory if required
                path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Spec_raw/'+inst+'_'+vis+'_Permpeaks/'
                if not os_system.path.exists(path_loc):os_system.makedirs(path_loc)                 
                data_vis=data_dic[inst][vis]
                ...
                #Orders to plot
                order_list = plot_options[key_plot]['orders_to_plot'] if len(plot_options[key_plot]['orders_to_plot'])>0 else range(data_dic[inst]['nord']) 
                ...
                for isub_ord,iord in enumerate(order_list):
                    ...
                    #Master spectrum
                    plt.plot(data_ord['wav_mast'],data_ord['flux_mast'],linestyle='-',color='dodgerblue',rasterized=plot_options[key_plot]['rasterized'],zorder=0,lw=plot_options[key_plot]['lw_plot'] )

                    #Anchor points 
                    plt.plot(data_ord['wav_max'],data_ord['flux_max'],linestyle='',color='black',rasterized=plot_options[key_plot]['rasterized'],zorder=1,marker='o',markersize=plot_options[key_plot]['markersize'] )
                    
                    #Continuum
                    plt.plot(data_ord['wav_mast'],data_corr['cont_func_dic'][iord](data_ord['wav_mast']),linestyle='-',color='red',rasterized=plot_options[key_plot]['rasterized'],zorder=1,lw=plot_options[key_plot]['lw_plot'] )
                    ...
                    if plot_options[key_plot]['x_range'] is not None:x_range_loc = plot_options[key_plot]['x_range']
                        ...
                        dx_range = xmax-xmin
                        x_range_loc = [xmin-0.05*dx_range,xmax+0.05*dx_range]  
                    if plot_options[key_plot]['y_range'] is not None:y_range_loc = plot_options[key_plot]['y_range'] 
                    else:
                ...
                for iexp in range(data_vis['n_in_visit']):
                    ...
                    for iord in range(data_dic[inst]['nord_ref']):
                        if data_corr['exp_ord_defring'][iord]==True:
                            ...
                            #Plot boundaries
                            x_range_loc = plot_options[key_plot]['x_range'] if plot_options[key_plot]['x_range'] is not None else [low_wav_ov[0],high_wav_ov[-1]]
                                
                            #Ratio between two successive orders over their overlapping range
                            if plot_options['plot_err']:plt.errorbar(wav_ov,data_corr['data_fit'][iord],yerr=data_corr['data_fit'][iord],linestyle='-',color='dodgerblue',rasterized=plot_options[key_plot]['rasterized'],zorder=0)
                            else:plt.plot(wav_ov,data_corr['data_fit'][iord],linestyle='-',color='dodgerblue',rasterized=plot_options[key_plot]['rasterized'],zorder=0)
                            
                            #Plot binned data
                            if (inst in plot_options[key_plot]['bin_data']):
                                nw_bin=int((high_wav_ov[-1]-low_wav_ov[0])/plot_options[key_plot]['bin_data'][inst])+1
                                ...
                                if plot_options['plot_err']:plt.errorbar(w_bin,ratio_ov_bin,yerr=eratio_ov_bin,linestyle='',color='black',rasterized=plot_options[key_plot]['rasterized'],zorder=1,marker='o',markersize=2)
                                else:plt.plot(w_bin,ratio_ov_bin,linestyle='',color='black',rasterized=plot_options[key_plot]['rasterized'],zorder=1,marker='o',markersize=2)
    ...
    '''
    Plotting individual disk-integrated spectra in their input rest frame (typically heliocentric)
    ...
    if gen_dic['specINtype'] and (plot_dic['sp_raw']!=''):
        ...
        sub_plot_spec(plot_options[key_plot],key_plot,plot_dic['sp_raw'])               
    ...
    '''
    Plotting individual disk-integrated profiles in their original rest frame, with their model fit
    '''
    if ((plot_dic['DI_prof']!='') or (plot_dic['DI_prof_res']!='')):
        ...
        ##############################################################################
        #Plot residuals between the raw CCFs and their fit
        ...
        #Plot        
        sub_plot_CCF(plot_options[key_plot],key_plot,plot_dic[key_plot])
    ...
    '''
    Plotting individual disk-integrated transmission spectra in the star rest frame 
        - before and after spectral corrections 
    '''
    if gen_dic['specINtype'] and (plot_dic['trans_sp']!=''):
        ...
        #Plot spectra at two chosen steps of the correction process
        plot_options[key_plot]['plot_pre']='raw'
        plot_options[key_plot]['plot_post']='all'
        
        #Exposures used in master out
        plot_options[key_plot]['iexp_mast_list']={}
        
        #Plot binned data and errors
        plot_options[key_plot]['plot_bin'] = True
        plot_options[key_plot]['plot_bin_err'] = True

        #Data bin size (in A)
        plot_options[key_plot]['bin_width'] = 10.

        #Force order level to unity
        plot_options[key_plot]['force_unity'] = False
        
        #Offset between uncorrected / corrected data
        plot_options[key_plot]['gap_exp'] = 0.03
        
        #Path to correction
        #    - leave empty to use last result from 'wig_vis_fit'
        plot_options[key_plot]['wig_path_corr'] = {}  
        ...
        sub_plot_DI_trans(plot_options[key_plot],key_plot,plot_dic[key_plot])               
    ...
    '''
    Plotting individual binned disk-integrated spectral profiles
    '''
    if (any('spec' in s for s in data_dic['DI']['type'].values())) and (plot_dic['sp_DIbin']!=''):
        ...
        print('-----------------------------------')
        print('+ Individual binned disk-integrated spectra')
        sub_plot_spec(plot_options[key_plot],'DIbin',plot_dic[key_plot])             
    ...
    '''
    Plotting individual binned disk-integrated CCF profiles
    '''
    if ('CCF' in data_dic['DI']['type'].values()) and ((plot_dic['CCF_DIbin']!='') or (plot_dic['CCF_DIbin_res']!='')):
        ...
        if (plot_dic['CCF_DIbin']!=''):
            ...
            print('-----------------------------------')
            print('+ Individual binned disk-integrated CCFs')
            
        ##############################################################################
        #Plot residuals between the binned disk-integrated CCFs and their fit
        if (plot_dic['CCF_DIbin_res']!='') and (gen_dic['fit_DIbin']):
            key_plot = 'CCF_DIbin_res'

            print('-----------------------------------')
            print('+ Individual residuals from binned disk-integrated CCFs')
    ...
    '''
    Plotting individual 1D disk-integrated stellar profiles
    '''
    ...
    if ((plot_dic['DImask_spectra']!='') or (plot_dic['Intrmask_spectra']!='')):
        for key_plot in ['DImask_spectra','Intrmask_spectra']:
            if plot_dic[key_plot]!='':
                ...
                plot_options[key_plot]['tell_depth_min'] = False
        ...
        #Plot
        data_type_gen=key_plot.split('mask')[0]
        key_step = plot_options[key_plot]['step']
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options[key_plot]['visits_to_plot'].keys())): 
            vis_det=list(data_dic[inst].keys()) if data_dic[inst]['n_visits_inst']==1 else 'binned'
            data_paths = 'CCF_masks_'+data_type_gen+'/'+gen_dic['add_txt_path'][data_type_gen]+'/'+inst+'_'+vis_det+'/'
            ...
            if plot_options[key_plot]['y_range'] is not None:
                y_range_loc=plot_options[key_plot]['y_range'] 
            else:
            ...
            #--------------------------------------------------------
                ...
                if plot_options[key_plot]['print_nl']:
                ...
                if key_step=='sel1':
                    ...
                    ax.plot(x_range_loc,np.repeat(1.-plot_info_step['linedepth_cont_min'],2),color='magenta',linestyle='--',lw=plot_options[key_plot]['lw_plot']+1,zorder=10,rasterized=plot_options[key_plot]['rasterized'])               
                    ax.plot(x_range_loc,np.repeat(1.-plot_info_step['linedepth_cont_max'],2),color='magenta',linestyle='--',lw=plot_options[key_plot]['lw_plot']+1,zorder=10,rasterized=plot_options[key_plot]['rasterized'])               

                #Line selection with telluric contamination
                elif key_step=='sel3':

                    #Plot minimum telluric depth to be considered
                    if plot_options[key_plot]['tell_depth_min']:
                        ax.plot(x_range_loc,np.repeat(1.-plot_info_step['tell_depth_min'],2),color='magenta',linestyle='--',lw=plot_options[key_plot]['lw_plot']+1,zorder=10,rasterized=plot_options[key_plot]['rasterized'])               
                    
                    #Telluric spectrum at minimum / maximum positions
                    ax.plot(plot_info['cen_bins_reg']*plot_info_step['min_dopp_shift'],plot_info_step['spectre_t'],color='dodgerblue',lw=plot_options[key_plot]['lw_plot'],zorder=10,rasterized=plot_options[key_plot]['rasterized'])    
                    ax.plot(plot_info['cen_bins_reg']*plot_info_step['max_dopp_shift'],plot_info_step['spectre_t'],color='red',lw=plot_options[key_plot]['lw_plot'],zorder=10,rasterized=plot_options[key_plot]['rasterized'])    

                    #Contaminating telluric lines
                    for wave_t,flux_t in zip(plot_info_step['w_tell_contam'],plot_info_step['f_tell_contam']):
                        ax.plot(wave_t,flux_t,marker='o',markersize=plot_options[key_plot]['markersize'],color='grey',markeredgecolor='white',markeredgewidth=0.5,rasterized=plot_options[key_plot]['rasterized'])
            ...
            custom_axis(plt,ax=ax,position=plot_options[key_plot]['margins'] ,x_range=x_range_loc,y_range=y_range_loc,dir_y='out', 
                     ...
                     font_size=plot_options[key_plot]['font_size'],xfont_size=plot_options[key_plot]['font_size'],yfont_size=plot_options[key_plot]['font_size'])
            plt.savefig(path_loc+'Spectrum_'+plot_options[key_plot]['step']+'.'+plot_dic[data_type_gen+'mask_spectra'])                        
            plt.close()   
      

    '''
    Plotting line depth range selection for CCF mask generation
    '''
    if ((plot_dic['DImask_ld']!='') or (plot_dic['Intrmask_ld']!='')):
        for key_plot in ['DImask_ld','Intrmask_ld']:
            if plot_dic[key_plot]!='':
                ...
                plot_options[key_plot]['y_log_hist'] = False
                plot_options[key_plot]['x_range_hist'] = None
                plot_options[key_plot]['y_range_hist'] = None
    ...
    '''
    Plotting line depth and width selection for CCF mask generation
    '''
    ...
    '''
    Plotting delta-position of lines for CCF mask generation
    '''
    ...
    '''
    Plotting depth correction of VALD lines for CCF mask generation
    '''
    if ((plot_dic['DImask_vald_depthcorr']!='') or (plot_dic['Intrmask_vald_depthcorr']!='')):
        for key_plot in ['DImask_vald_depthcorr','Intrmask_vald_depthcorr']:
            if plot_dic[key_plot]!='':
                ...
                #Figure size
                plot_options[key_plot]['fig_size'] = (12,15)
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options[key_plot]['visits_to_plot'].keys())): 
            ...
            if len(plot_options[key_plot]['spec_to_plot'])>0:vald_spec = np.intersect1d(plot_options[key_plot]['spec_to_plot'],vald_spec) 
            ...
            fig, axes = plt.subplots(nsub_rows,nsub_col, figsize=plot_options[key_plot]['fig_size'])
            fig.subplots_adjust(top=plot_options[key_plot]['margins'][3],left=plot_options[key_plot]['margins'][0],right=plot_options[key_plot]['margins'][2],bottom=plot_options[key_plot]['margins'][1],wspace=0.45,hspace=0.55)
            for idx_plot,elem in enumerate(vald_spec):
                ...
                ax_loc.errorbar(plot_info_elem['vald_depth_fit'],plot_info_elem['line_minus_vald_depth_fit'],yerr=plot_info_elem['line_minus_vald_depth_err_fit'],linestyle='',color='black',rasterized=plot_options[key_plot]['rasterized'],zorder=1,marker='o',markersize=plot_options[key_plot]['rasterized'])      
    ...
    '''
    Plotting morphological (asymmetry) properties for CCF mask generation
    '''
    ...
    '''
    Plotting morphological (shape) properties for CCF mask generation
    '''
    ...
    '''
    Plotting RV properties for CCF mask generation
    '''
    ...
    '''
    Plotting properties of raw data and disk-integrated CCFs
    ...
    if (plot_dic['prop_raw']!=''):
        ...
        for plot_prop in plot_settings['prop_ordin']:
            ...
            #Abscissa
            plot_options[key_plot]['prop_absc']='phase'
            ...
            plot_options[key_plot]['norm_out']=False

            #Orders for SNR
            plot_options[key_plot]['idx_SNR']={'HARPS':[49],'HARPN':[46],'CORALIE':[46],'ESPRESSO':[102,103],'ESPRESSO_MR':[39],'CARMENES_VIS':[40],'NIRPS_HE':[57],'NIRPS_HA':[57]}
            ...
            if (plot_prop=='rv'):
                ...
                plot_options[key_plot]['theoRV'] = True
            ...
            #-----------------------------------------------------
            #FWHM voigt plot
            if (plot_prop=='FWHM_voigt'):
                print('   > FWHM of Voigt profile')
            ...
            if (plot_prop=='flux_airmass'):
                print('   > Airmass absorption')
            ...
            #----------------------------------------
            if (plot_prop=='glob_flux_sc'):
                print('   > Global flux scaling') 

            #-----------------------------------------------------
            #Saturation flag
            if (plot_prop=='satur_check'):
                print('   > Saturation check')   
    
            #-----------------------------------------------------
            #Telescope altitude
            if (plot_prop=='alt'):
                print('   > Telescope altitude')   
        
            #-----------------------------------------------------
            #ESPRESSO ADC intel
            for key in ['ADC1 POS','ADC1 RA','ADC1 DEC','ADC2 POS','ADC2 RA','ADC2 DEC']:
                if (plot_prop==key):
                    print('   > ESPRESSO ',key)
                    
            #-----------------------------------------------------
            #ESPRESSO piezo intel
            for key in ['TILT1 VAL1','TILT1 VAL2','TILT2 VAL1','TILT2 VAL2']:
                if (plot_prop==key):
                    print('   > ESPRESSO ',key)
            ...
            #-----------------------------------------------------
            #Plot of polynomial continuum
            if (plot_prop=='cont'):
                print('   > Continuum level')     
    
            if (plot_prop=='c1_pol'):
                print('   > Polynomial continuum (deg. 1)')     
    
            if (plot_prop=='c2_pol'):
                print('   > Polynomial continuum (deg. 2)')     
    
            if (plot_prop=='c3_pol'):
                print('   > Polynomial continuum (deg. 3)')     
                
            if (plot_prop=='c4_pol'):
                print('   > Polynomial continuum (deg. 4)')                  
            ...
            if (plot_prop=='vsini'):
                print('   > Projected rotational velocity')

            #-----------------------------------------------------
            #Local line contrast 
            if (plot_prop=='ctrst_ord0__IS__VS_'):
                print('   > Local contrast (deg. 0)')
    
            #-----------------------------------------------------
            #Local line FWHM 
            if (plot_prop=='FWHM_ord0__IS__VS_'):
                print('   > Local FWHM (deg. 0)')
            ...
            #Plot routine   
            sub_plot_CCF_prop(plot_prop,key_plot,plot_options[key_plot],'raw')  
    ...
    if (plot_dic['all_DI_data']!=''):  
        ...
        sub_plot_all_prof(plot_options[key_plot],'DI',plot_dic['all_DI_data'])                 
    ...
    '''
    Plotting input light curves used to rescale the flux
    '''
    if (plot_dic['input_LC']!=''):
        ...
        plot_options[key_plot]['plot_LC_exp'] = True
        
        #Plot HR input light curves
        plot_options[key_plot]['plot_LC_HR'] = True
        
        #Plot raw imported light curve, if available
        plot_options[key_plot]['plot_LC_imp'] = True

        #Print visit names
        plot_options[key_plot]['plot_vis']=True

        #Indexes of bands to plot
        #    - default is all
        plot_options[key_plot]['idx_bands']=[]
        ...
        path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Light_curves/Input/'
        if not os_system.path.exists(path_loc):os_system.makedirs(path_loc)  
        ...
        idx_bands_plot = plot_options[key_plot]['idx_bands'] if len(plot_options[key_plot]['idx_bands'])>0 else range(system_prop['nw'])    
        for iband in idx_bands_plot:
            print('   > Band w = '+str(system_prop['w'][iband]))
            ...
            for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options[key_plot]['visits_to_plot'].keys())):                           
                ...
                y_max=-1e100
                if (inst not in plot_options[key_plot]['color_dic'] ):plot_options[key_plot]['color_dic'][inst]={}
                for ivis,vis in enumerate(np.intersect1d(list(data_dic[inst].keys()),plot_options[key_plot]['visits_to_plot'][inst])): 

                    #Visit color
                    if (inst!='binned') and (vis in plot_options[key_plot]['color_dic'][inst]):col_vis = plot_options[key_plot]['color_dic'][inst][vis]
                    else:col_vis = 'black'
                    ...
                    if plot_options[key_plot]['lc_gap'] is None:vis_shift = ivis*0.3*Tdepth
                    else:vis_shift = ivis*plot_options[key_plot]['lc_gap']
                    ...
                    if plot_options[key_plot]['plot_LC_HR']:
                        ...
                        plt.plot(ph_plot,var_plot,color=col_vis,linestyle='-',lw=plot_options[key_plot]['lw_plot'])  
                    ...
                    if plot_options[key_plot]['plot_LC_exp']: 
                        ...
                        for iexp,(ph_loc,ph_dur_loc,flux_loc) in enumerate(zip(coord_vis['cen_ph'],coord_vis['ph_dur'],LC_flux_band_all[:,iband])):
                            ...
                            if iexp in gen_dic[inst][vis]['idx_in']:
                                ...
                                if plot_options[key_plot]['plot_expid']:plt.text(ph_loc,flux_loc-vis_shift-0.1*Tdepth,str(i_in),verticalalignment='bottom', horizontalalignment='center',fontsize=4.,zorder=4,color=col_vis) 
                                i_in+=1                               
                            plt.errorbar(ph_loc,flux_loc-vis_shift,xerr=0.5*ph_dur_loc,color=col_vis,marker=marker,markersize=plot_options[key_plot]['markersize'],linestyle='',markerfacecolor=markerfacecolor)                

                #Axis ranges
                x_range_loc=plot_options[key_plot]['x_range'] if plot_options[key_plot]['x_range'] is not None else np.array([x_min-0.005,x_max+0.005])
                if plot_options[key_plot]['y_range'] is not None:y_range_loc=plot_options[key_plot]['y_range'] 
                ...
                custom_axis(plt,position=plot_options[key_plot]['margins'] ,x_range=x_range_loc,y_range=y_range_loc,dir_y='out', 
                         ...
                         font_size=plot_options[key_plot]['font_size'],xfont_size=plot_options[key_plot]['font_size'],yfont_size=plot_options[key_plot]['font_size'])
                plt.savefig(path_loc+inst+'_idxband'+str(iband)+'.'+plot_dic['input_LC'])                        
                plt.close()  
    ...
    '''
    Plot effective light curves used to rescale the flux
    '''
    if (plot_dic['spectral_LC']!=''):
        ...
        #Visit color
        n_wLC=len(plot_options[key_plot]['wav_LC'])
        if (len(plot_options[key_plot]['color_dic'])==0):
            cmap = plt.get_cmap('rainbow') 
            if n_wLC==1:color_dic={0:cmap(0)} 
            else:
                col_tab = cmap( np.arange(n_wLC)/(n_wLC-1.))
                color_dic={iw:col_tab[iw] for iw in range(n_wLC)}
        else:color_dic = plot_options[key_plot]['color_dic']
    ...
    if (plot_dic['map_DI_1D']!=''):
        ...
        print('-----------------------------------')
        print('+ Plotting 2D maps of 1D disk-integrated stellar profiles') 
    ...
    '''
    2D maps of residual profiles in stellar rest frame
    '''
    if (plot_dic['map_res_prof']!=''):
        ...
        print('-----------------------------------')
        print('+ Plotting 2D maps of residual profiles')    
    ...
    if any('spec' in s for s in data_dic['Res']['type'].values()) and (plot_dic['sp_loc']!=''):
        ...
        plot_options[key_plot]['estim_loc']= False 
        ...
        sub_plot_spec(plot_options[key_plot],'Res',plot_dic[key_plot])
    ...
    '''
    Plot standard deviation with bin size for out-of-transit residual CCFs
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options[key_plot]['visits_to_plot'].keys())): 
            ...
            for vis in np.intersect1d(list(data_dic[inst].keys()),plot_options[key_plot]['visits_to_plot'][inst]): 
                ...
                for isub,iexp in enumerate(gen_dic[inst][vis]['idx_out']):
                    ...
                    y_range_loc=plot_options[key_plot]['y_range'] if plot_options[key_plot]['y_range'] is not None else np.array([0.9*y_min,1.1*y_max])
                    custom_axis(plt,position=plot_options[key_plot]['margins'],x_range=[1,gen_dic['scr_srch_nperbins'][-1]],y_range=y_range_loc,
    ...
    '''
    Plotting 2D maps of intrinsic stellar profiles
    '''
    ...
    '''
    2D maps of 1D intrinsic stellar profiles
    '''
    if (plot_dic['map_Intr_1D']!=''):
        ...
        print('-----------------------------------')
        print('+ Plotting 2D maps of 1D intrinsic stellar profiles') 
    ...
    if (plot_dic['pca_ana']!=''):
        ...
        #Subplots
        plot_options[key_plot]['pc_var'] = True 
        for key in ['pc_rms','pc_bic','pc_hist','pc_prof','fft_prof']:plot_options[key_plot][key] = False

        #PC variances to overplot
        plot_options[key_plot]['var_list'] = ['pre','post','out']

        #Principal components profiles to plot
        plot_options[key_plot]['pc_list'] = [0]
        plot_options[key_plot]['pc_col'] = ['dodgerblue']
        ...
        plot_options[key_plot]['fft_list'] = ['res','corr','boot']
        ...
        #Create directory if required
        path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/Res_data/PCA/'
        if not os_system.path.exists(path_loc):os_system.makedirs(path_loc)  
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options[key_plot]['visits_to_plot'].keys())): 
            ...
            for vis in np.intersect1d(list(data_dic[inst].keys()),plot_options[key_plot]['visits_to_plot'][inst]):
                ...
                #------------------------------------------------                         
                #PC variances
                if plot_options[key_plot]['pc_var']:
                    plt.ioff()        
                    fig = plt.figure(figsize=(10, 6))
                    y_min=1e100
                    y_max=-1e100
                    ...
                    for key in plot_options[key_plot]['var_list']:
                        if key in data_upload['eig_val_res']:
                            ...
                            plt.plot(np.arange(n_pc), var_res_explained,marker = 'o',color=col_type[key],lw=plot_options[key_plot]['lw_plot'],markersize=plot_options[key_plot]['markersize'],linestyle='-')
                            ...
                            plt.plot(np.arange(n_pc), var_noise_explained,color=col_type[key],lw=plot_options[key_plot]['lw_plot'],linestyle='--')
                ...
                if plot_options[key_plot]['pc_rms']:
                    plt.ioff()        
                    fig = plt.figure(figsize=(10, 6))
                    y_min=1e100
                    y_max=-1e100
                    ...
                    #Measured RMS
                    plt.plot(cen_ph[data_upload['idx_corr']][idx_corr_out], data_upload['rms_full_res_matr'][0][idx_corr_out],markerfacecolor='dodgerblue',marker = 's',color='dodgerblue',ls='',markersize=plot_options[key_plot]['markersize'])
                    plt.plot(cen_ph[data_upload['idx_corr']][idx_corr_out], data_upload['rms_full_res_matr'][1][idx_corr_out],markerfacecolor='red',marker = 's',color='red',ls='',markersize=plot_options[key_plot]['markersize'])
                    plt.plot(cen_ph[idx_corr_in], data_upload['rms_full_res_matr'][0][idx_corr_in],markerfacecolor='dodgerblue',marker = 'o',color='dodgerblue',ls='',markersize=plot_options[key_plot]['markersize'])
                    plt.plot(cen_ph[idx_corr_in], data_upload['rms_full_res_matr'][1][idx_corr_in],markerfacecolor='red',marker = 'o',color='red',ls='',markersize=plot_options[key_plot]['markersize'])
                    y_min=np.min([np.nanmin(data_upload['rms_full_res_matr']),y_min])
                    y_max=np.max([np.nanmax(data_upload['rms_full_res_matr']),y_max]) 
                    

                    #Contacts
                    y_range_loc=plot_options[key_plot]['y_range_rms'] if plot_options[key_plot]['y_range_rms'] is not None else np.array([y_min,y_max])
                    for cont_ph in contact_phases[pl_ref]:plt.plot([cont_ph,cont_ph],y_range_loc,color='black',linestyle=':',lw=plot_options[key_plot]['lw_plot'],zorder=0)
                ...
                if plot_options[key_plot]['pc_bic']:
                    plt.ioff()        
                    fig = plt.figure(figsize=(10, 6))
                    y_min=1e100
                    y_max=-1e100
                    ...
                    for isub,iexp in enumerate(data_upload['idx_corr']):
                        ...
                        plt.plot(cen_ph[iexp], dBIC[iexp],markerfacecolor=col_loc,marker = marker_loc,color=col_loc,ls='',markersize=plot_options[key_plot]['markersize'])
                    y_min=np.min([np.nanmin(dBIC),y_min])
                    y_max=np.max([np.nanmax(dBIC),y_max]) 
                    

                    #Contacts
                    x_range_loc=plot_options[key_plot]['x_range_bic'] if plot_options[key_plot]['x_range_bic'] is not None else np.array([np.min(cen_ph),np.max(cen_ph)])
                    y_range_loc=plot_options[key_plot]['y_range_bic'] if plot_options[key_plot]['y_range_bic'] is not None else np.array([y_min,y_max])
                    for cont_ph in contact_phases[pl_ref]:plt.plot([cont_ph,cont_ph],y_range_loc,color='black',linestyle=':',lw=plot_options[key_plot]['lw_plot'],zorder=0)
                ...
                if plot_options[key_plot]['pc_hist']:
                    y_min=1e100
                    y_max=-1e100
                    ...
                    if not np.isnan(hist_data[0][0]):
                        ...
                        if plot_options[key_plot]['x_range_hist'] is not None:x_range_loc = plot_options[key_plot]['x_range_hist']
                        else:x_range_loc = [np.min(cen_bins),np.max(cen_bins)]    
                        if plot_options[key_plot]['y_range_hist'] is not None:y_range_loc = plot_options[key_plot]['y_range_hist']
                        else:y_range_loc = [0,1.1 * np.max(gauss_norm)]                      
                        ax.text(x_range_loc[0]+0.5*(x_range_loc[1]-x_range_loc[0]),y_range_loc[1]+0.01*(y_range_loc[1]-y_range_loc[0]),'Pre-transit',verticalalignment='bottom', horizontalalignment='center',fontsize=0.8*plot_options[key_plot]['font_size'],zorder=4,color='black') 
                        ax.set_xlim(x_range_loc)
                        ax.set_ylim(y_range_loc)
                        ax.set_yticklabels([])
                        ax.yaxis.set_ticks_position('none')          
                        ax.set_xlabel(x_title,fontsize=plot_options[key_plot]['font_size'])
                    ...
                    if not np.isnan(hist_data[0][0]):
                        ...
                        ax.text(x_range_loc[0]+0.5*(x_range_loc[1]-x_range_loc[0]),y_range_loc[1]+0.01*(y_range_loc[1]-y_range_loc[0]),'Post-transit',verticalalignment='bottom', horizontalalignment='center',fontsize=0.8*plot_options[key_plot]['font_size'],zorder=4,color='black') 
                        ax.set_xlim(x_range_loc)
                        ax.set_ylim(y_range_loc)
                        ax.set_yticklabels([])
                        ax.yaxis.set_ticks_position('none')          
                    ...
                    ax.text(x_range_loc[0]+0.5*(x_range_loc[1]-x_range_loc[0]),y_range_loc[1]+0.01*(y_range_loc[1]-y_range_loc[0]),'Out-transit',verticalalignment='bottom', horizontalalignment='center',fontsize=0.8*plot_options[key_plot]['font_size'],zorder=4,color='black') 
                    ax.set_xlim(x_range_loc)
                    ax.set_ylim(y_range_loc)
                    ax.set_yticklabels([])
                    ax.yaxis.set_ticks_position('none')                     
                ...
                #------------------------------------------------                
                #PC profiles
                if plot_options[key_plot]['pc_prof']:
                    plt.ioff()        
                    fig = plt.figure(figsize=(10, 6))
                    y_min=1e100
                    y_max=-1e100
                    ...
                    #PC 
                    for isubpc,ipc in enumerate(plot_options[key_plot]['pc_list']):
                        plt.plot(data_upload['cen_bins'], data_upload['eig_res_matr'][ipc],color=plot_options[key_plot]['pc_col'][isubpc],lw=plot_options[key_plot]['lw_plot'])
                ...
                #------------------------------------------------                
                #FFT analysis
                if plot_options[key_plot]['fft_prof']:
                    x_range_loc=plot_options[key_plot]['x_range_fft'] if plot_options[key_plot]['x_range_fft'] is not None else np.array([np.min(data_upload['cen_bins_res_mat']),np.max(data_upload['cen_bins_res_mat'])])
                    for fft_loc in plot_options[key_plot]['fft_list']:
                        plt.ioff()        
                        fig = plt.figure(figsize=(10, 6))
                        y_min=1e100
                        y_max=-1e100
                        ...
                        plt.plot(data_upload['cen_bins_res_mat'], var_plot[0],lw=plot_options[key_plot]['lw_plot'],marker='o',markersize=1.5,color='dodgerblue')
                        plt.plot(data_upload['cen_bins_res_mat'], var_plot[1],lw=plot_options[key_plot]['lw_plot'],marker='o',markersize=1.5,color='red')
    ...
    '''
    Plotting all individual intrinsic profiles from a given visit together
    '''
    if (plot_dic['all_intr_data']!=''):
        ...
        print('-----------------------------------')
        print('+ Plotting all intrinsic profiles in each visit')
        sub_plot_all_prof(plot_options[key_plot],'Intr',plot_dic[key_plot])             
    ...
    if any('spec' in s for s in data_dic['Res']['type'].values()) and (plot_dic['sp_intr']!=''):
        ...
        sub_plot_spec(plot_options[key_plot],'Intr',plot_dic[key_plot])             
    ...
    '''
    if ('CCF' in data_dic['Res']['type'].values()) and ((plot_dic['CCFintr']!='') or (plot_dic['CCFintr_res']!='')):
    ...
    '''
    Plotting individual binned intrinsic spectral profiles
    '''
    if any('spec' in s for s in data_dic['Res']['type'].values()) and (plot_dic['sp_Intrbin']!=''):
        ...
        print('-----------------------------------')
        print('+ Plotting individual binned intrinsic spectra for each visit')
        sub_plot_spec(plot_options[key_plot],'Intrbin',plot_dic['sp_Intrbin'])             
    ...
    if ('CCF' in data_dic['Res']['type'].values()) and ((plot_dic['CCF_Intrbin']!='') or (plot_dic['CCF_Intrbin_res']!='')):
        ...
        if (plot_dic['CCF_Intrbin']!=''):
            ...
            print('-----------------------------------')
            print('   > Plotting individual binned intrinsic CCFs for each visit')        
        ...
        if (plot_dic['CCF_Intrbin_res']!=''):
            ...
            print('-----------------------------------')
            print('   > Plot individual residuals of fit to binned intrinsic CCFs')
    ...
    '''
    Plotting individual 1D intrinsic stellar profiles
    '''
    if ('spec' in data_dic['Res']['type'].values()) and (plot_dic['sp_1D_intr']!=''):
        key_plot = 'sp_1D_intr'
        ...
        print('-----------------------------------')
        print('   > Plotting individual 1D intrinsic spectra for each visit')
        sub_plot_spec(plot_options,'sp_1D_intr',plot_dic['sp_1D_intr'])             
    ...
    if (plot_dic['chi2_fit_loc_prop']!=''):
        ...
        plot_options[key_plot]['chi2_thresh']=3.  

        #Property
        plot_options[key_plot]['prop'] = 'rv'
        ...
        print('-----------------------------------')
        print('+ Plotting chi2 per fitted local property value')

        sub_plot_chi2_prop(plot_options[key_plot],plot_dic['chi2_fit_loc_prop'])             
    ...
    if (plot_dic['plocc_ranges']!=''):
        ...
        plot_options[key_plot]['x_prop']='r_proj' 
        
        #Gap between exposures
        plot_options[key_plot]['y_gap'] = 0.2
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options[key_plot]['visits_to_plot'].keys())):                            
            ...
            for ivis,vis in enumerate(vis_list): 
                ...
                if (inst in color_dic) and (vis in color_dic[inst]):
                    col_visit=np.repeat(color_dic[inst][vis],n_in_tr)
                else:
                    cmap = plt.get_cmap('rainbow') 
                    col_visit=np.array([cmap(0)]) if n_in_tr==1 else cmap( np.arange(n_in_tr)/(n_in_tr-1.))
                    
                #Plot chosen property
                transit_prop_nom = (np.load(gen_dic['save_data_dir']+'Introrig_prop/PlOcc_Prop_'+inst+'_'+vis+'.npz',allow_pickle=True)['data'].item())['achrom']                           
                x_obs = transit_prop_nom[plot_options[key_plot]['x_prop']][0,:]
                st_x_obs,end_x_obs = transit_prop_nom[plot_options[key_plot]['x_prop']+'_range'][0,:,0],transit_prop_nom[plot_options[key_plot]['x_prop']+'_range'][0,:,1]
                for i_in,(x_obs_loc,st_x_obs_loc,end_x_obs_loc,col_exp) in enumerate(zip(x_obs,st_x_obs,end_x_obs,col_visit)):
                    
                    plt.errorbar(x_obs_loc, y_max+ plot_options[key_plot]['y_gap']*i_in,xerr=[[x_obs_loc-st_x_obs_loc],[end_x_obs_loc-x_obs_loc]],color=col_exp,marker='o',markersize=plot_options['markersize'],linestyle='',markerfacecolor=col_exp,lw=plot_options['lw_plot'])                

                    #Exposures indexes
                    if plot_options[key_plot]['plot_expid']:plt.text(x_obs_loc,y_max+ plot_options[key_plot]['y_gap']*(i_in+0.2),str(i_in),verticalalignment='bottom', horizontalalignment='center',fontsize=plot_options[key_plot]['font_size']-2.,zorder=4,color='black') 

                #Visit name
                plt.text(0.5*(plot_options[key_plot]['x_range'][0]+plot_options[key_plot]['x_range'][1]),y_max+ plot_options[key_plot]['y_gap']*0.5*data_dic[inst][vis]['n_in_tr'],str(vis),verticalalignment='bottom', horizontalalignment='center',fontsize=plot_options[key_plot]['font_size']-2.,zorder=4,color='black') 
                y_max+=n_in_tr*plot_options[key_plot]['y_gap'] + 5.*plot_options[key_plot]['y_gap'] 

            #Plot frame  
            x_title={'mu':'$\mu$','xp_abs':'|x$_p$|','r_proj':'r$_{proj}$'}[plot_options[key_plot]['x_prop']]
            if plot_options[key_plot]['plot_title']:plt.title('Planet-occulted range for '+x_title,fontsize=plot_options[key_plot]['font_size'])
            y_range_loc=np.array([y_min,y_max])              
            xmajor_int,xminor_int,xmajor_form=autom_x_tick_prop(plot_options[key_plot]['x_range'][1]-plot_options[key_plot]['x_range'][0])
            ymajor_int,yminor_int,ymajor_form=autom_y_tick_prop(y_range_loc[1]-y_range_loc[0]) 
            custom_axis(plt,position=plot_options[key_plot]['margins'],x_range=x_range_loc,y_range=y_range_loc,dir_y='out', 
            		     xmajor_int=xmajor_int,xminor_int=xminor_int,xmajor_form=xmajor_form,
            		     ymajor_int=ymajor_int,yminor_int=yminor_int,ymajor_form=ymajor_form,
                         ...
                         y_title='Visits',
                         font_size=plot_options[key_plot]['font_size'],xfont_size=plot_options[key_plot]['font_size'],yfont_size=plot_options[key_plot]['font_size'])
            plt.savefig(path_loc+inst+'_'+plot_options[key_plot]['x_prop']+'.'+plot_dic['plocc_ranges'])                        
            plt.close()  
    ...
    if (plot_dic['propCCF_DI_mcmc_PDFs']!='') or (plot_dic['propCCFloc_mcmc_PDFs']!=''):
        for key_plot in ['propCCF_DI_mcmc_PDFs','propCCFloc_mcmc_PDFs']:
            if plot_dic[key_plot]!='':
                ...
                #Figure size
                plot_options[key_plot]['fig_size']=(10,5)

                #Choose property to plot
                plot_options[key_plot]['plot_prop_list']=['rv']
        
                #Number of subplots per row (>=1)
                plot_options[key_plot]['nsub_col'] = 5

                #Spacing between subplots
                plot_options[key_plot]['wspace'] = 0.05
                plot_options[key_plot]['hspace'] = 0.05

                #Plot 1 sigma HDI or confidence intervals
                plot_options[key_plot]['plot_conf_mode']=['HDI']

                #Tick intervals
                plot_options[key_plot]['xmajor_int_all'] = {}
                plot_options[key_plot]['xminor_int_all'] = {}

                #Common ranges
                plot_options[key_plot]['xrange_all'] = {}

        ##############################################################################
        if (plot_dic['propCCF_DI_mcmc_PDFs']!=''):
            key_plot='propCCF_DI_mcmc_PDFs'
            plot_options[key_plot]['data_mode'] = 'DI'
            plot_options[key_plot]['data_dic_idx'] = 'DI'
            
            print('-----------------------------------')
            print('+ Plotting 1D PDF of disk-integrated stellar profiles properties')

        ##############################################################################
        if (plot_dic['propCCFloc_mcmc_PDFs']!=''):
            key_plot='propCCFloc_mcmc_PDFs'
            plot_options[key_plot]['data_mode'] = 'Intr'
            plot_options[key_plot]['data_dic_idx'] = 'Res'
            
            print('-----------------------------------')
            print('+ Plotting 1D PDF of intrinsic stellar profiles properties')

        #Overwrite default settings
        plot_options[key_plot].update(plot_settings[key_plot]) 
        
        #Plot
        sub_plot_propCCF_mcmc_PDFs(plot_options,key_plot,plot_dic[key_plot])
    ...
    if (plot_dic['prop_loc']!=''):
        print('-----------------------------------')
        print('+ Plotting properties of planet-occulted stellar CCFs')
        for plot_prop in plot_settings['prop_ordin']:
            key_plot = 'prop_'+plot_prop 
            ...
            #Orders for SNR
            plot_options[key_plot]['idx_SNR']={'HARPS':[49],'HARPN':[46],'CORALIE':[46],'ESPRESSO':[102,103],'ESPRESSO_MR':[39],'CARMENES_VIS':[40]}
            plot_options[key_plot]['idx_num_SNR']={'HARPN':[46]}
            plot_options[key_plot]['idx_den_SNR']={'HARPN':[46]}   
            ...
            #Plot routine   
            sub_plot_CCF_prop(plot_prop,key_plot,plot_options[key_plot],'loc')                  
    ...
    if (plot_dic['occulted_regions']!=''):
        ...
        #Plot full orbit
        plot_options[key_plot]['plot_orb']=True        

        #Ranges
        plot_options[key_plot]['x_range'] = np.array([-1.15,1.15])
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options['visits_to_plot'].keys())): 
            for vis in np.intersect1d(list(data_dic[inst].keys())+['binned'],plot_options['visits_to_plot'][inst]): 
                ...
                if orig_vis in list(data_dic['Res'][inst].keys()):
                    ...
                    #Plot frame  
                    custom_axis(plt,position=[0.15,0.15,0.95,0.95],x_range=plot_options[key_plot]['x_range'],y_range=plot_options[key_plot]['y_range'],
    ...
    if (plot_dic['system_view']!=''):
        ...
        plot_options[key_plot]['xorp_pl'] = np.tile([[-2.],[2.]],len(plot_options[key_plot]['pl_to_plot'])).T
        ...
        plot_options[key_plot]['xorb_dir'] = np.tile([[-2.],[2.]],len(plot_options[key_plot]['pl_to_plot'])).T        
        plot_options[key_plot]['yorb_dir'] = np.repeat(-0.5,len(plot_options[key_plot]['pl_to_plot'])) 

        #Apparent size of the planet
        plot_options[key_plot]['RpRs_pl'] = [data_dic['DI']['system_prop']['RpRs_max'][pl_loc] for pl_loc in plot_options[key_plot]['pl_to_plot']]

        #Orbit colors
        plot_options[key_plot]['col_orb'] = np.repeat('forestgreen',len(plot_options[key_plot]['pl_to_plot']))
        plot_options[key_plot]['col_orb_samp'] = np.repeat('forestgreen',len(plot_options[key_plot]['pl_to_plot']))
            
        #Number of orbits drawn randomly
        plot_options[key_plot]['norb']=2500

        #Ranges of orbital parameters
        plot_options[key_plot]['lambdeg_err']={}
        plot_options[key_plot]['aRs_err']={}
        plot_options[key_plot]['ip_err']={}
        plot_options[key_plot]['b_range_all']={}            

        #Plot width
        plot_options[key_plot]['width']=10.         

        #Choose view
        plot_options[key_plot]['conf_system']='sky_ste'

        #Reference planet for the 'sky_orb' configuration
        plot_options[key_plot]['pl_ref'] = plot_options[key_plot]['pl_to_plot'][0]
        
        #Overlaying grid cell boundaries
        plot_options[key_plot]['st_grid_overlay']=False
        plot_options[key_plot]['pl_grid_overlay']=False
        
        #Number of cells on a diameter of the star and planets (must be odd)
        plot_options[key_plot]['n_stcell']=theo_dic['nsub_Dstar']
        plot_options[key_plot]['n_plcell']={}
        for pl_loc in plot_options[key_plot]['pl_to_plot']:plot_options[key_plot]['n_plcell'][pl_loc] = theo_dic['nsub_Dpl'][pl_loc]

        #Color stellar disk with RV, with limb-darkened specific intensity, with gravity-darkened specific intensity, or total flux
        plot_options[key_plot]['disk_color']='RV'

        #Plot colorbar
        plot_options[key_plot]['plot_colbar']=True 

        #Plot visible stellar equator
        plot_options[key_plot]['plot_equ_vis']=True  
    
        #Plot hidden equator
        plot_options[key_plot]['plot_equ_hid']=  False
        
        #Plot stellar spin
        plot_options[key_plot]['plot_stspin']=True 
        
        #Plot stellar poles
        plot_options[key_plot]['plot_poles']=True  
        ...
        plot_options[key_plot]['margins']=[0.12,0.15,0.85,0.88]    
    
        #Plot boundaries
        plot_options[key_plot]['x_range'] = np.array([-1.5,1.5])   
        plot_options[key_plot]['y_range'] = np.array([-1.5,1.5]) 
        ...
        path_loc = gen_dic['save_dir']+gen_dic['main_pl_text']+'_Plots/System_view/' 
        if not os_system.path.exists(path_loc):os_system.makedirs(path_loc)  
        ...
        mRp2 = (1. - Rpole**2.)
        lref=system_param[plot_options[key_plot]['pl_ref']]['lambda_rad']
        istar_rad=star_params['istar_rad']
        ci = cos(istar_rad)
        si = sin(istar_rad)  
        iband = plot_options[key_plot]['iband']
        ...
        if star_params['f_GD']>0.:
            ...
            if plot_options[key_plot]['conf_system']=='sky_ste':
        ...
        #Stellar equator:
        #    - theta set negative to go along equator clockwise, in the direction of the star motion, from 0 to -2pi 
        #      theta starts at 0 on the eastern node, then goes beyond the star, then through the western node at -pi, then on the visible hemisphere.
        if plot_options[key_plot]['plot_equ_vis'] or plot_options[key_plot]['plot_equ_hid']: 
            n_pts_eq=1000.
            th_eq=-2.*pi*np.arange(n_pts_eq+1.)/n_pts_eq      #in between 0 to -2pi                            
            x_eqst_st = np.cos(th_eq)
            y_eqst_st = 0.
            z_eqst_st = np.sin(th_eq)            
            x_eqst_sky_st,y_eqst_sky_st,z_eqst_sky_st=conv_StarFrame_to_inclinedStarFrame(x_eqst_st,y_eqst_st,z_eqst_st,istar_rad)
            if plot_options[key_plot]['conf_system']=='sky_orb': 
                x_eqst_view,y_eqst_view,z_eqst_view=conv_inclinedStarFrame_to_Losframe(lref,x_eqst_sky_st,y_eqst_sky_st,z_eqst_sky_st) 
        ...
        #----------------------   
        if plot_options[key_plot]['plot_stspin']:
...
#            st_spin_norm=1.1  #1.33   #1.08   
            ...
            if plot_options[key_plot]['conf_system']=='sky_orb':st_spin_up=st_spin_norm*Rpole*np.array([sin(lref)*sin(istar_rad),cos(lref)*sin(istar_rad),cos(istar_rad)])
            if plot_options[key_plot]['conf_system']=='sky_ste':st_spin_up=st_spin_norm*Rpole*np.array([0.,sin(istar_rad) ,cos(istar_rad)])
            ...
            if plot_options[key_plot]['conf_system']=='sky_orb':st_spin_x_st,st_spin_y_st,st_spin_z_st=conv_Losframe_to_inclinedStarFrame(lref,st_spin_x,st_spin_y,st_spin_z) 
            ...
            headwidth_spin=10*2  
            headlength_spin=10
            if plot_options[key_plot]['conf_system']=='sky_ste':x_shaft_spin=0.
            ...
            if (nst_spin-1) in w_vis_close:
                ...
                if plot_options[key_plot]['conf_system']=='sky_orb':x_shaft_spin = st_spin_x[w_vis_last]-st_spin_x[w_vis_first]
            ...
            #Last point of stellar spin is pointing away from us and is visible or not outside of the projected photosphere              
            elif ((nst_spin-1) in w_vis_far) or (plot_options[key_plot]['plot_hidden_pole'] and ((nst_spin-1) in w_unvis_far)):
                if ((nst_spin-1) in w_vis_far): 
                    ...
                    ls_spin = '--'
                    alph_spin = 0.3
                if plot_options[key_plot]['conf_system']=='sky_orb':x_shaft = st_spin_x[w_vis_last]-st_spin_x[w_vis_first]
                y_shaft_spin = st_spin_y[w_vis_last]-st_spin_y[w_vis_first]
        ...
        #Stellar poles
        #-------------------------------------------------------   
        if plot_options[key_plot]['plot_poles']:  

            #North and south poles coordinates
            if plot_options[key_plot]['conf_system']=='sky_orb':coord_Npole=Rpole*np.array([sin(lref)*sin(istar_rad),cos(lref)*sin(istar_rad),cos(istar_rad)])
            if plot_options[key_plot]['conf_system']=='sky_ste':coord_Npole=Rpole*np.array([0.,sin(istar_rad) ,cos(istar_rad)])
        ...
        d_stcell,Ssub_Sstar,coord_grid['x_st_sky'],coord_grid['y_st_sky'],_=occ_region_grid(1.,plot_options[key_plot]['n_stcell'])

        #Grid coordinates in the sky-projected stellar frame      
        if plot_options[key_plot]['conf_system']=='sky_orb':  
            coord_grid['x_st_sky'],coord_grid['y_st_sky'],_=conv_Losframe_to_inclinedStarFrame(lref,coord_grid['x_st_sky'],coord_grid['y_st_sky'],None)          
        ...
        ld_grid_star,gd_grid_star,mu_grid_star,_,Fsurf_grid_star,_,_ = calc_Isurf_grid(range(system_prop['nw']),nsub_star,system_prop,coord_grid,star_params,Ssub_Sstar)
        ...
        #Orbits
        #--------------------------------------------
        def plot_orb_func(ax_plot,stend_ph,npts_orbits_plot,pl_params_plot,pl_ref,col_orb_plot,alph_front,alph_back,zord_plot,lw):
            
            #Orbit coordinates
            coord_orbit_loc=def_plotorbite(npts_orbits_plot,pl_params_plot)
            ...
            x_orbit_plot,y_orbit_plot,z_orbit_plot = conv_Losframe_to_inclinedStarFrame(ang_orb,x_orbit_sky,y_orbit_sky,z_orbit_sky)
            ...
            dx_range = plot_options[key_plot]['x_range'][1]-plot_options[key_plot]['x_range'][0]
            dy_range = plot_options[key_plot]['y_range'][1]-plot_options[key_plot]['y_range'][0]
            cond_in_plot = (x_orbit_plot>=plot_options[key_plot]['x_range'][0]-0.1*dx_range) & (x_orbit_plot<=plot_options[key_plot]['x_range'][1]+0.1*dx_range) &  (y_orbit_plot>=plot_options[key_plot]['y_range'][0]-0.1*dy_range) & (y_orbit_plot<=plot_options[key_plot]['y_range'][1]+0.1*dy_range)
            w_front = (z_orbit_plot >= 0.) & cond_in_plot
            isort=np.argsort(x_orbit_plot[w_front])
            ax_plot.plot(x_orbit_plot[w_front][isort],y_orbit_plot[w_front][isort],zorder=zord_plot, color=col_orb_plot,lw=plot_options[key_plot]['lw_plot'],alpha=alph_front)
            ...
            ax_plot.plot(x_orbit_back,y_orbit_back,zorder=zord_plot, color=col_orb_plot,lw=plot_options[key_plot]['lw_plot'],alpha=alph_back)
        ...
        for idx_pl,plot_t in enumerate(plot_series):
            ...
            n_pl = len(plot_options[key_plot]['pl_to_plot'])
            for ipl,pl_loc in enumerate(plot_options[key_plot]['pl_to_plot']):
          
                #Planet orbit	
                #    - 'coord_orbit' is defined in the Sky-projected orbital frame: Xsky,Ysky,Zsky	
                x_orbit_view,y_orbit_view,z_orbit_view  = plot_orb_func(ax1,plot_dic['stend_ph'],plot_options[key_plot]['npts_orbits'][ipl],system_param[pl_loc],pl_loc,plot_settings[key_plot]['col_orb'][ipl],1.,0.4,40.+2*ipl+1,2)
                
                #Uncertainty of planet orbit
                if (pl_loc in plot_options[key_plot]['lambdeg_err']) or (pl_loc in plot_options[key_plot]['aRs_err']) or (pl_loc in plot_options[key_plot]['ip_err']):
                    if (pl_loc in plot_options[key_plot]['lambdeg_err']):
                        grid_val,cdf_val = sem_norm_dist(system_param[pl_loc]['lambda_rad'],plot_options[key_plot]['lambdeg_err'][pl_loc][0]*np.pi/180.,plot_options[key_plot]['lambdeg_err'][pl_loc][1]*np.pi/180.)
                        rand_draw = np.random.uniform(low=0.0, high=1.0, size=plot_options[key_plot]['norb'])    
                        lambdarad_tab = np_interp(rand_draw,cdf_val,grid_val)
                        lambdeg_range = [system_param[pl_loc]['lambda_rad']-plot_options[key_plot]['lambdeg_err'][pl_loc][0]*np.pi/180.,system_param[pl_loc]['lambda_rad']+plot_options[key_plot]['lambdeg_err'][pl_loc][1]*np.pi/180.]
                    else:
                        lambdarad_tab = np.repeat(system_param[pl_loc]['lambda_rad'],plot_options[key_plot]['norb'])
                        lambdeg_range=[-1e10,1e10]
                    if (pl_loc in plot_options[key_plot]['aRs_err']):
                        grid_val,cdf_val = sem_norm_dist(system_param[pl_loc]['aRs'],plot_options[key_plot]['aRs_err'][pl_loc][0],plot_options[key_plot]['aRs_err'][pl_loc][1])
                        rand_draw = np.random.uniform(low=0.0, high=1.0, size=plot_options[key_plot]['norb'])    
                        aRs_tab = np_interp(rand_draw,cdf_val,grid_val)    
                        aRs_range = [system_param[pl_loc]['aRs']-plot_options[key_plot]['aRs_err'][pl_loc][0],system_param[pl_loc]['aRs']+plot_options[key_plot]['aRs_err'][pl_loc][1]]               
                    else:
                        aRs_tab = np.repeat(system_param[pl_loc]['aRs'],plot_options[key_plot]['norb'])
                        aRs_range=[0.,1e10]
                    if (pl_loc in plot_options[key_plot]['ip_err']):
                        grid_val,cdf_val = sem_norm_dist(system_param[pl_loc]['inclin_rad'],plot_options[key_plot]['ip_err'][pl_loc][0]*np.pi/180.,plot_options[key_plot]['ip_err'][pl_loc][1]*np.pi/180.)
                        rand_draw = np.random.uniform(low=0.0, high=1.0, size=plot_options[key_plot]['norb'])    
                        ip_tab = np_interp(rand_draw,cdf_val,grid_val)
                        ip_range = [system_param[pl_loc]['inclin_rad']-plot_options[key_plot]['ip_err'][pl_loc][0]*np.pi/180.,system_param[pl_loc]['inclin_rad']+plot_options[key_plot]['ip_err'][pl_loc][1]*np.pi/180.]  
                    else:
                        ip_tab = np.repeat(system_param[pl_loc]['inclin_rad'],plot_options[key_plot]['norb'])
                        ip_range=[-1e10,1e10]
                    b_tab = np.abs(aRs_tab*np.cos(ip_tab))
                    if (pl_loc in plot_options[key_plot]['b_range_all']):b_range=plot_options[key_plot]['b_range_all'][pl_loc]
                    ...
                    for iorb,(lambdarad_loc,aRs_loc,ip_loc) in enumerate(zip(lambdarad_tab,aRs_tab,ip_tab)):
                        ...
                        _,_,_ = plot_orb_func(ax1,plot_options[key_plot]['stend_ph'][ipl],plot_options[key_plot]['npts_orbits'][ipl],pl_params_orb,pl_loc,plot_settings[key_plot]['col_orb_samp'][ipl],0.2,0.03,40.+2*ipl,1)
                        print(iorb)
                ...
                if plot_options[key_plot]['t_BJD'] is not None:
                    ...
                    x_pl_plot,y_pl_plot,_ = conv_Losframe_to_inclinedStarFrame(ang_orb,x_pl_sky,y_pl_sky,z_pl_sky)       
                else:
                    ...
                    w_pl=closest(y_orbit_view[w_visible],plot_options[key_plot]['yorb_pl'][ipl])
                ...
                if plot_options[key_plot]['pl_grid_overlay']:       
                    ...
                    lw_st_grid=1.
                    coord_pl_grid = {}      
                    d_sub,_,x_st_sky_grid_pl,y_st_sky_grid_pl,r_sub_pl2=occ_region_grid(RpRs,plot_options[key_plot]['n_plcell'][pl_loc])
                ...
                w_visible=np.where((z_orbit_view>0) & (x_orbit_view>=plot_options[key_plot]['xorb_dir'][ipl][0]) & (x_orbit_view<=plot_options[key_plot]['xorb_dir'][ipl][1]))[0]
                if len(w_visible)==0.:stop('No visible orbit in requested range')
                w_orien=closest(y_orbit_view[w_visible],plot_options[key_plot]['yorb_dir'][ipl])    
                ...
                ax1.quiver(x_orbit_view[w_visible][w_orien],y_orbit_view[w_visible][w_orien],0.2*np.sign(x_shaft),0.2*np.abs(y_shaft/x_shaft)*np.sign(y_shaft),color=plot_settings[key_plot]['col_orb'][ipl],
                ...
                if plot_options[key_plot]['conf_system']=='sky_ste':ang_normal =  system_param[pl_loc]['lambda_rad']   
                if plot_options[key_plot]['conf_system']=='sky_orb':ang_normal =  system_param[pl_loc]['lambda_rad']-lref   
                ...
                plt.plot(x_shaft,y_shaft,lw=3,color=plot_settings[key_plot]['col_orb'][ipl],zorder=30+2*n_pl)    #shaft             
                ...
                if cond_vis:
                    ...
                    ax1.quiver(orb_spin_xyzview[0],orb_spin_xyzview[1],xdir_quiv,ydir_quiv,color=plot_settings[key_plot]['col_orb'][ipl],
            ...
            if plot_options[key_plot]['plot_equ_vis'] or plot_options[key_plot]['plot_equ_hid']: 
                ...
                #Visible part  
                if plot_options[key_plot]['plot_equ_vis']:
                    if plot_options[key_plot]['conf_system']=='sky_orb':
                        w_vis_up=np.where( (z_eqst_view >= 0.) & (y_eqst_view > ysky_infl_vis) )[0]
                        w_vis_down=np.where( (z_eqst_view >= 0.) & (y_eqst_view <= ysky_infl_vis) )[0]
                        isort_down=np.argsort(x_eqst_view[w_vis_down])
                        plt.plot(x_eqst_view[w_vis_down][isort_down],y_eqst_view[w_vis_down][isort_down],zorder=25., color='black',lw=1.,alpha=1)  
                    if plot_options[key_plot]['conf_system']=='sky_ste':  
                        w_vis_up=np.where( z_eqst_view >= 0. )[0]
                    isort_up=np.argsort(x_eqst_view[w_vis_up])
                    plt.plot(x_eqst_view[w_vis_up][isort_up],y_eqst_view[w_vis_up][isort_up],zorder=25., color='black',lw=1.,alpha=1)   
                            
                #Hidden part
                if plot_options[key_plot]['plot_equ_hid']:
                    if plot_options[key_plot]['conf_system']=='sky_orb':
                ...
                #Add arrow in the direction of the stellar rotation
                w_vis = w_vis_up
                w_orien=closest(x_eqst_view[w_vis_up],0.)   
                if (w_orien==0) or (w_orien==len(w_vis_up)-1):
                    w_vis = w_vis_down
                    w_orien=closest(x_eqst_view[w_vis_down],0.) 
                x_tip=x_eqst_view[w_vis][w_orien]
                y_tip=y_eqst_view[w_vis][w_orien]
                x_shaft_eq = 40.*(x_eqst_view[w_vis][w_orien]-x_eqst_view[w_vis][w_orien-1]) 
                y_shaft_eq = 40.*(y_eqst_view[w_vis][w_orien]-y_eqst_view[w_vis][w_orien-1]) 
                ax1.quiver(x_tip,y_tip,0.2*np.sign(x_shaft_eq),0.2*np.abs(y_shaft_eq/x_shaft_eq)*np.sign(y_shaft_eq),color='black',
            ...
            if plot_options[key_plot]['plot_stspin']:
                ...
                if plot_options[key_plot]['plot_hidden_pole']:plt.plot(0.99*st_spin_x[w_unvis_far],0.99*st_spin_y[w_unvis_far],color='black',lw=lw_spin,linestyle='--',alpha=0.3)  
            ...
            if plot_options[key_plot]['plot_poles']:  
                ...
                #Plot North pole
                if (coord_Npole[2]>0.) or (plot_options[key_plot]['plot_hidden_pole']): 
                ...
                #Plot South pole
                if (coord_Spole[2]>0.) or (plot_options[key_plot]['plot_hidden_pole']):  
            ...
            if plot_options[key_plot]['disk_color']=='RV':
                val_disk=RVstel
                cmap = plt.get_cmap('bwr')
            elif plot_options[key_plot]['disk_color']=='LD':
                val_disk=ld_grid_star[:,iband] 
                cmap = plt.get_cmap('YlOrRd_r')
            elif plot_options[key_plot]['disk_color']=='GD':
            ...
            if plot_options[key_plot]['val_range'] is None:
                ...
                min_val=plot_options[key_plot]['val_range'][0]
                max_val=plot_options[key_plot]['val_range'][1]
            ...
            #Intensity shade
            if (plot_options[key_plot]['disk_color']=='RV') and (plot_options[key_plot]['shade_overlay']): 
            ...
            for i_in,x_pos,y_pos in zip(np.arange(len(val_disk))[cond_in_plot],coord_grid['x_st_sky'][cond_in_plot],coord_grid['y_st_sky'][cond_in_plot]):  
                edgecolor_stcell='none' if plot_options[key_plot]['st_grid_overlay'] else color_tab[i_in]
            ...
            if (plot_options[key_plot]['n_equi'] is not None) and (plot_options[key_plot]['conf_system']=='sky_ste'): 
                ...
                x_equiRV=np.zeros([plot_options[key_plot]['n_equi'],nrow_cells])*np.nan
                y_equiRV=np.zeros([plot_options[key_plot]['n_equi'],nrow_cells])*np.nan
                ...
                #Plot equiRV
                for i_equi in range(plot_options[key_plot]['n_equi']): 
            ...
            if plot_options[key_plot]['st_grid_overlay']:       
                col_grid = 'black'
                lw_st_grid=0.5
                ls_grid = ':'
            ...
            if plot_options[key_plot]['plot_colbar']:
                x2=0.87  #0.90							
                cbar_pos=fig.add_axes([plot_options[key_plot]['margins'][2],plot_options[key_plot]['margins'][1],x2-plot_options[key_plot]['margins'][2],plot_options[key_plot]['margins'][3]-plot_options[key_plot]['margins'][1]])   
                if plot_options[key_plot]['disk_color']=='RV':
                ...
                cbar.ax.tick_params(labelsize=plot_options[key_plot]['font_size']) 
                cbar.set_label(colbar_title, rotation=270,labelpad=25+2,size=plot_options[key_plot]['font_size'])	
        
            #-----------------------
            #Main axis 
            xmajor_int,xminor_int,xmajor_form = autom_x_tick_prop(plot_options[key_plot]['x_range'][1]-plot_options[key_plot]['x_range'][0])
            ymajor_int,yminor_int,ymajor_form = autom_y_tick_prop(plot_options[key_plot]['y_range'][1]-plot_options[key_plot]['y_range'][0])
            custom_axis(plt,ax=ax1,x_range=plot_options[key_plot]['x_range'],y_range=plot_options[key_plot]['y_range'],
                        xmajor_int=xmajor_int,xminor_int=xminor_int,ymajor_int=ymajor_int,yminor_int=yminor_int,
                        # xmajor_int=0.5,xminor_int=0.1,ymajor_int=0.5,yminor_int=0.1,
    ...
    if (plot_dic['binned_DI_Intr']!=''):
        ...
        plot_options[key_plot]['dim_plot_DI']='phase' 

        #Choose bin dimension for intrinsic profiles
        plot_options[key_plot]['dim_plot_intr']='r_proj' 
    ...
    if any('spec' in s for s in data_dic['Atm']['type'].values()) and (plot_dic['sp_atm']!=''):
        ...
        #Plot        
        sub_plot_spec(plot_options,'atm',plot_dic[key_plot])    
    ...
    if ('CCF' in data_dic['Atm']['type'].values()) and ((plot_dic['CCFatm']!='') or (plot_dic['CCFatm_res']!='')):
        ...
        #Plot
        sub_plot_CCF(plot_options,key_plot,plot_dic[key_plot])
    ...
    if any('spec' in s for s in data_dic['Atm']['type'].values()) and (plot_dic['sp_Atmbin']!=''): 
        ...
        #Plot        
        sub_plot_spec(plot_options,'Atmbin',plot_dic[key_plot])             
    ...
    if ('CCF' in data_dic['Atm']['type'].values()) and ((plot_dic['CCF_Atmbin']!='') or (plot_dic['CCF_Atmbin_res']!='')):
        ...
        ##############################################################################
        #Plot each CCF and its fit
        if (plot_dic['CCF_Atmbin']!=''):
            key_plot='CCF_Atmbin'

            print('-----------------------------------')
            print('+ Plot individual binned atmospheric CCFs for each visit')


        ##############################################################################
        #Plot residuals between the binned CCFs and their fit
        if (plot_dic['CCF_Atmbin_res']!='') and (gen_dic['ana_Atmbin']):
