...
void C_coadd_loc_cgauss_prof(double* rv_surf_star_grid, double* ctrst_grid, double* FWHM_grid, float skew, float kurt, double* skew_grid, double* kurt_grid, double* factor_grid, double* args_cen_bins, double* Fsurf_grid_spec, int args_ncen_bins, int Fsurf_grid_spec_shape_0, double* cgauss_grid) 
{
    ...
    for (int i = 0; i < Fsurf_grid_spec_shape_0; i++) {
        ...
        double C = ctrst_grid[i];
        double D = FWHM_grid[i];
        double E = skew_grid[i];
        double F = kurt_grid[i];
        double G = factor_grid[i];
        for (int j = 0; j < args_ncen_bins; j++) {

            // Apply skewness if present
            if (skew != 0.0) {
                G += E * (c[1] * (X * (args_cen_bins[j] - B) / D) + c[3] * (X * (args_cen_bins[j] - B) / D) * (X * (args_cen_bins[j] - B) / D) * (X * (args_cen_bins[j] - B) / D));
            }

            // Apply kurtosis if present
            if (kurt != 0.0) {
                G += F * (c[0] + c[2] * (X * (args_cen_bins[j] - B) / D) * (X * (args_cen_bins[j] - B) / D) + c[4] * (X * (args_cen_bins[j] - B) / D) * (X * (args_cen_bins[j] - B) / D) * (X * (args_cen_bins[j] - B) / D) * (X * (args_cen_bins[j] - B) / D));
            }
...
void C_coadd_loc_pgauss_prof(double* rv_surf_star_grid, double* ctrst_grid, double* FWHM_grid, double* c4_pol_grid, double* c6_pol_grid, double* dRV_joint_grid, double* args_cen_bins, double* Fsurf_grid_spec, int args_ncen_bins, int Fsurf_grid_spec_shape_0, double* pgauss_grid) 
{
    ...
    // Gaussian with baseline set to continuum value
        for (int j = 0; j < args_ncen_bins; j++) {
            ...
            if (args_cen_bins[j] >=  B[j] - G[j] &  args_cen_bins[j] <= B[j] + G[j]){
                pgauss_grid[i * args_ncen_bins + j] *= (E[j] * G[j] * G[j] * G[j] * G[j]) + (2.0 * F[j] * G[j] * G[j] * G[j] * G[j] * G[j] * G[j]) - (G[j] * G[j] * (args_cen_bins[j] - B[j]) * (args_cen_bins[j] - B[j]) * (2.0 * E[j] + 3.0 * F[j] * G[j] * G[j])) + (E[j] * (args_cen_bins[j] - B[j]) * (args_cen_bins[j] - B[j]) * (args_cen_bins[j] - B[j]) * (args_cen_bins[j] - B[j])) + (F[j] * (args_cen_bins[j] - B[j]) * (args_cen_bins[j] - B[j]) * (args_cen_bins[j] - B[j]) * (args_cen_bins[j] - B[j]) * (args_cen_bins[j] - B[j]) * (args_cen_bins[j] - B[j]))
            }
        }
    }
}
...
void C_coadd_loc_voigt_prof(double* rv_surf_star_grid, double* ctrst_grid, double* FWHM_grid, double* a_damp_grid, double* args_cen_bins, double* Fsurf_grid_spec, int args_ncen_bins, int Fsurf_grid_spec_shape_0, double* voigt_grid) 
{
    ...
    // Gaussian with baseline set to continuum value
        for (int j = 0; j < args_ncen_bins; j++) {
            voigt_grid[i * args_ncen_bins + j] = A * (1.0 - (C/Faddeeva_erf_re(E * I)) * Faddeeva_erf_re((X * (args_cen_bins[j] - B) / D) + E * I))
        }
    }
}
