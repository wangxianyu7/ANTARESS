...
def ln_prob_func_lmfit(p_step, x_val, fixed_args=None):
    ...
    # ln_prior = ln_prior_func(p_step,fixed_args)
    # ln_prob=-np.inf if np.isnan(ln_lkhood) else ln_prior + ln_lkhood
    # chi = np.sqrt(-2.*ln_prob/len(x_val))
    # chi_tab = np.repeat(chi,len(x_val))
...
'''
def fit_minimization(ln_prob_func_lmfit, p_use, xtofit, ytofit, covtofit, f_use,method='leastsq', maxfev=None, xtol=1e-7, ftol=1e-7,verbose=False,fixed_args=None,show_correl=False):
    ...
    if verbose:print('   duration : '+str((time.time()-st0)/60.)+' mn')
...
def fit_merit(p_final_in,fixed_args,fit_dic,verbose):
    ...
    #End counter
    fit_dic['fit_dur'] = time.time()-fit_dic['st0']
