...
from utils import stop,closest,np_where1D,npint,np_interp,closest_arr,init_parallel_func,planck,np_poly,is_odd,dataload_npz,datasave_npz,MAIN_multithread
...
from ANTARESS_routines.ANTARESS_orbit import Mean_anom_TR_calc,Kepler_func
...
def init_prop(data_dic,mock_dic,gen_dic,system_param,theo_dic,plot_dic,glob_fit_dic,detrend_prof_dic):
    ...
    if gen_dic['intr_data']:
        ...
        for key in ['map_Intr_prof','all_intr_data','sp_intr','Intr_prof']:plot_dic[key]=''
    ...
    for pl_loc in list(set(gen_dic['studied_pl']+gen_dic['kepl_pl'])):
        ...
        if (pl_loc in gen_dic['studied_pl']) and ('TLength' not in PlParam_loc):
            contact_phases=def_contacts(data_dic['DI']['system_prop']['achrom'][pl_loc][0],PlParam_loc,plot_dic['stend_ph'],star_params)
    ...
    if gen_dic['loc_data_corr']:
        ...
        if (not os_system.path.exists(gen_dic['save_data_dir']+'Loc_estimates/'+data_dic['Intr']['opt_loc_data_corr']['corr_mode']+'/')):os_system.makedirs(gen_dic['save_data_dir']+'Loc_estimates/'+data_dic['Intr']['opt_loc_data_corr']['corr_mode']+'/')  
    if (gen_dic['pl_atm']):
...
Sub-function to force a line FWHM to an input value 
    ...
    - this operation must be performed on velocity tables symmetrical with respect to the CCF center, ie for CCFs that have been aligned on the null velocity 
      this means the RV correction must be performed first, to determine and fix the correct systemic velocity
'''
