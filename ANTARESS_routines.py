...
from utils import stop,closest,np_where1D,npint,np_interp,closest_arr,init_parallel_func,planck,np_poly,is_odd,dataload_npz
from scipy.signal import savgol_filter
from astropy.io import fits #to read .fits
from itertools import product as it_product
from constant_data import G_usi,Mjup,Msun,c_light,AU_1,Rsun,Rjup
...
def model_par_names():
    ...
    return {
        ...
        'cont':'P$_\mathrm{cont}$','offset':'F$_\mathrm{off}$','slope':'P$_\mathrm{1cont}$',
...
Initializations
'''
def init_prop(data_dic,mock_dic,gen_dic,system_param,theo_dic,plot_dic,glob_fit_dic,PropAtm_fit_dic):
    ...
    sp_corr_list = ['corr_tell','corr_FbalOrd','corr_Fbal','corr_Ftemp','corr_cosm','mask_permpeak','corr_wig','corr_fring','trim_spec','glob_mast','gain_weight','det_gain']
    if (not gen_dic['specINtype']):
        for key in sp_corr_list:gen_dic[key]=False
    else:
        #Activate spectral gain calculation
        gen_dic['det_gain'] = True
    ...
    for mode in ['','multivis']:
        ...
        for data_type in ['DI','Intr','Atm']:
            gen_dic['bin'+mode] |= ( gen_dic[data_type+'bin'+mode] | gen_dic['fit_'+data_type+'bin'+mode])
            gen_dic['calc_bin'+mode] |= ( gen_dic['calc_'+data_type+'bin'+mode] | gen_dic['calc_fit_'+data_type+'bin'+mode])
    ...
    if gen_dic['intr_data']:
        ...
        for key in ['map_Intr_prof','all_intr_data','sp_intr','CCFintr']:plot_dic[key]=''
    ...
    if ('CCF' not in data_dic['DI']['type'].values()):
        plot_dic['prop_raw']==''
        gen_dic['corr_line_prof']=False
    if (not gen_dic['res_data']):
        for key in ['map_res_prof','sp_loc','CCFloc']:plot_dic[key]=''    
    if ('CCF' not in data_dic['Res']['type'].values()):gen_dic['fit_IntrProf']=False
    if ('CCF' not in data_dic['Atm']['type'].values()):gen_dic['fit_atm_all']=False
    ...
    if gen_dic['fit_IntrProf'] or gen_dic['fit_loc_prop'] or gen_dic['fit_atm_all'] or gen_dic['fit_atm_prop'] or gen_dic['fit_ResProf'] : gen_dic['multi_inst']=True
    ...
    if gen_dic['mask_permpeak'] or gen_dic['def_DImasks'] or gen_dic['def_Intrmasks']:
        for inst in data_dic['instrum_list'] :
    ...
    if 'LD_u2' not in data_dic['DI']['system_prop']['achrom']:data_dic['DI']['system_prop']['achrom']['LD_u2'] = 0.
    ...
    if gen_dic['theoPlOcc'] or (gen_dic['theo_spots']) or ((gen_dic['fit_DI'] or gen_dic['fit_DIbin'] or gen_dic['fit_DIbinmultivis']) and (('custom' in data_dic['DI']['model'].values()) or ('RT_ani_macro' in data_dic['DI']['model'].values()))) or gen_dic['mock_data'] \
        or gen_dic['fit_ResProf'] or gen_dic['correct_spots'] or gen_dic['fit_IntrProf']:
        ...
        if gen_dic['mock_data']:
            ...
            theo_dic['sme_grid']['wave'] = np.linspace(theo_dic['st_atm']['wav_min'],theo_dic['st_atm']['wav_max'],theo_dic['st_atm']['n_wav'])   
            theo_dic['sme_grid']['edge_bins'] = def_edge_tab(theo_dic['sme_grid']['wave'][None,:][None,:])[0,0]
    ...
    if gen_dic['specINtype']:
        if gen_dic['det_gain'] and (not os_system.path.exists(gen_dic['save_data_dir']+'Processed_data/Gains/')):os_system.makedirs(gen_dic['save_data_dir']+'Processed_data/Gains/')  
    ...
    if (gen_dic['fit_loc_prop']):
        if glob_fit_dic['PropLoc']['data']=='orig':suff='Orig/'
        elif glob_fit_dic['PropLoc']['data']=='bin':suff='Binned/'
        if (not os_system.path.exists(gen_dic['save_data_dir']+'Joined_fits/Intr_prop/'+suff)):os_system.makedirs(gen_dic['save_data_dir']+'Joined_fits/Intr_prop/'+suff)        
    if (gen_dic['fit_atm_prop']):
        if PropAtm_fit_dic['data']=='orig':suff='Orig/'
        elif PropAtm_fit_dic['data']=='bin':suff='Binned/'
        if (not os_system.path.exists(gen_dic['save_data_dir']+'Atm_data_prop/Fit/'+suff)):os_system.makedirs(gen_dic['save_data_dir']+'Atm_data_prop/Fit/'+suff)        
    ...
    if (gen_dic['fit_atm_all']) and (not os_system.path.exists(gen_dic['save_data_dir']+'Atm_data_prop/Fit/')):os_system.makedirs(gen_dic['save_data_dir']+'Atm_data_prop/Fit/') 
...
def model_star(mode,grid_dic,grid_type,system_prop_in,nsub_Dstar,star_params):
    ...
    if mode=='grid':grid_dic['mu'] = grid_dic['mu_grid_star_achrom']
...
def calc_st_sky(coord_grid,star_params):
    ...
    return cond_in_stphot,nsub_star
...
def init_data_instru(mock_dic,inst,gen_dic,data_dic,theo_dic,data_prop,coord_dic,system_param,plot_dic):
    ...
    if not gen_dic['mock_data']:
        if (not gen_dic['flag_err_inst'][inst]) and gen_dic['det_gain']:stop('Error table must be available to estimate gain')
    ...
    if (gen_dic['calc_proc_data']):
        ...
        if gen_dic['mock_data']: 
            ...
            data_inst['idx_ord_ref']=deepcopy(range(gen_dic['norders_instru'][inst]))      #to keep track of the original orders
        ...
        #Gain settings
        if (inst not in gen_dic['gain_nooutedge']):gen_dic['gain_nooutedge'][inst] = [0.,0.]
        ...
        for vis in vis_list:
            ...
            #Observational data        
                ...
                if inst in ['SOPHIE']:
                    ...
                    vis_path+= {
                        'CCF':'*CCF_TELL_CORR_',
                        'spec1D':'*S1D_TELL_CORR_',
                        'spec2D':'*S2D_TELL_CORR_',
                        }[data_inst['type']] 
                elif inst in ['EXPRES']:
                    vis_path+= {
                        'spec2D':'*',
            ...
            if (vis in gen_dic['unused_visits'][inst]):   
                ...
                if inst in ['HARPN','HARPS','CORALIE','SOPHIE','ESPRESSO','ESPRESSO_MR','NIRPS_HA','NIRPS_HE']:
                    for key in ['RVdrift','RVpip','FWHMpip','Ctrstpip']:DI_data_inst[vis][key] =  np.zeros(n_in_visit,dtype=float)*np.nan  
                    for key in ['eRVpip','eFWHMpip','eCtrstpip']:DI_data_inst[vis][key] =  np.zeros(n_in_visit,dtype=float)  
                ...
                for isub_exp,iexp in enumerate(range(n_in_visit)):
                    ...
                    if isub_exp==0:
                        ...
                        if gen_dic['mock_data']: 
                            ...
                            fixed_args['cen_bins']={inst:{vis:{0:mock_dic['DI_table']['x_start']+dcen_bins*(0.5+np.arange(data_inst[vis]['nspec']))}}}
                            fixed_args['edge_bins']={inst:{vis:{0:def_edge_tab(fixed_args['cen_bins'][inst][vis][0][None,:][None,:])[0,0]}}}
                            fixed_args['ncen_bins']={inst:{vis:data_inst[vis]['nspec']}}
                            fixed_args['dcen_bins']={inst:{vis:{0:fixed_args['edge_bins'][inst][vis][0][1::] - fixed_args['edge_bins'][inst][vis][0][0:-1]       }}}
                            fixed_args['dim_exp']={inst:{vis:[data_inst['nord'],fixed_args['ncen_bins'][inst][vis]]}}
           
                            #Activation of spectral conversion and resampling 
                            cond_conv_st_prof_tab(gen_dic,fixed_args,data_inst[vis]['type'])
                                                         
                            #Line profile model table
                            model_st_prof_tab(inst,vis,0,fixed_args,gen_dic,1)
                            
                            #Reference point for instrumental convolution
                            ref_inst_convol(fixed_args,fixed_args['cen_bins'][inst][vis][0])
                    ...
                    if gen_dic['mock_data']: 
                        ...
                        args_exp = def_st_prof_tab(inst,vis,0,fixed_args)

                        #Add jitter to the intrinsic profile properties (simulating stellar activity)
                        if (fixed_args['mode']=='glob_mod') and (inst in mock_dic['drift_intr']) and (vis in mock_dic['drift_intr'][vis]) and (len(mock_dic['drift_intr'][inst][vis]>0)):
                        ...
                        if (fixed_args['mode']=='Intrbin'):FWHM_inst = None
                            ...
                            if (inst in mock_dic['drift_post']) and (vis in mock_dic['drift_post'][vis]) and ('resol' in mock_dic['drift_post'][inst][vis]):
                                FWHM_inst = fixed_args['ref_conv']/mock_dic['drift_post'][inst][vis]['resol'][iexp]
                            else:FWHM_inst = return_FWHM_inst(inst,fixed_args['ref_conv'])

                        #Convolution, conversion and resampling 
                        DI_prof_exp = conv_st_prof_tab(inst,vis,0,fixed_args,args_exp,DI_prof_exp,FWHM_inst)

                        #Define number of photons received during exposure
                        #   - the model represents the density of photons per unit of time
                        #   - the continuum is proportional to the mock continuum level
                        DI_prof_exp_Ntrue = DI_prof_exp*coord_dic[inst][vis]['t_dur'][iexp]   
                        ...
                        if ('spec' in data_inst[vis]['type']):data_dic_temp['cen_bins'][iexp,0]=fixed_args['cen_bins'][inst][vis][0]*spec_dopshift(-rv_mock)
                        else:data_dic_temp['cen_bins'][iexp,0] = fixed_args['cen_bins'][inst][vis][0] + rv_mock        
                   
                        #Defining flux and noise table     
                        #    - see def_weights_bin(), the measured flux can be defined as:
                        # F_meas(t,w) = g(band) Nmeas(t,w)
                        #      where Nmeas(t,w) is drawn from a Poisson distribution with number of events Ntrue(t,w)
                        #      the estimate of the error is then
                        # EF_meas(t,w) = sqrt(g(band) F_meas(t,w))
                        #      which is a biased estimate of the true error but corresponds to what is returned by DRS
                        #    - the S/N of the mock profiles is F_meas(t,w)/EF_meas(t,w) = sqrt(F_meas(t,w)/g(band)) = sqrt(Nmeas(t,w)) = sqrt(C*texp*cont)
                        #      errors in the continuum of the normalized profiles is 1/sqrt(C*texp*cont) 
                        #      beware that this error does not necessarily match the flux dispersion
                        if (inst in mock_dic['gain']) and (vis in mock_dic['gain'][inst]):
                            DI_prof_exp_Nmeas = np.array(list(map(np.random.poisson, DI_prof_exp_Ntrue,  data_inst[vis]['nspec']*[1]))).flatten()
                            DI_prof_exp_Fmeas = mock_dic['gain'][inst][vis]*DI_prof_exp_Nmeas
                            DI_err_exp_Emeas = np.sqrt(mock_dic['gain'][inst][vis]*DI_prof_exp_Fmeas)
                        else:
                            DI_prof_exp_Fmeas = DI_prof_exp_Ntrue
                    ...
                    #-----------------------------------
                        ...
                        #Retrieve spectral data
                            ...
                            #2D spectra
                                ...
                                if inst in ['HARPS','ESPRESSO','ESPRESSO_MR','HARPN','NIRPS_HA','NIRPS_HE']:
                                    ...
                                    if (vis_path_skysub_exp is not None):stop('No sky-corrected data available')
                                    stop('Check if flux is per pixel like ESPRESSO')
                                    #Bin centers
                                    #    - dimension norder x nbins
                        ...
                        #Set undefined errors to squareroot of flux as first-order approximation
                    ...
                    if not gen_dic['mock_data']: 
                        ...
                        if ('RVpip' in DI_data_inst[vis]):
                            if inst in ['CORALIE']:
                                pre_txt='HIERARCH ESO DRS CCF'      
                                DI_data_inst[vis]['RVpip'][iexp]=hdr[pre_txt+' RVC'] #Baryc RV (drift corrected) (km/s)
                                DI_data_inst[vis]['eRVpip'][iexp]=hdr['HIERARCH ESO DRS DVRMS']/1e3                           
                            elif inst=='SOPHIE':
                                pre_txt='HIERARCH OHP DRS CCF'               
                                DI_data_inst[vis]['RVpip'][iexp]=hdr[pre_txt+' RV']  #Baryc RV (drift corrected ?? ) (km/s) 
                                DI_data_inst[vis]['eRVpip'][iexp]=hdr[pre_txt+' ERR']
                            elif inst in ['HARPS','ESPRESSO','ESPRESSO_MR','HARPN','NIRPS_HA','NIRPS_HE']:
                                reduc_txt=facil_inst+' QC '
                                pre_txt='HIERARCH '+reduc_txt+'CCF'        
                                DI_data_inst[vis]['RVpip'][iexp]=hdr[pre_txt+' RV'] #Baryc RV (drift corrected) (km/s)   
                                DI_data_inst[vis]['eRVpip'][iexp]=hdr[pre_txt+' RV ERROR']
                            DI_data_inst[vis]['FWHMpip'][iexp]=hdr[pre_txt+' FWHM'] 
                            DI_data_inst[vis]['Ctrstpip'][iexp]=hdr[pre_txt+' CONTRAST'] 
                            if inst in ['HARPS','SOPHIE','ESPRESSO','ESPRESSO_MR','HARPN','NIRPS_HA','NIRPS_HE']:
                                DI_data_inst[vis]['eFWHMpip'][iexp]=hdr[pre_txt+' FWHM ERROR'] 
                ...
                #    - for spectra, the common table is set to the table of the first exposure, defined in the input rest frame
                #      it will be used indifferently to resample spectra defined in the input or the stellar rest frame
                #    - for CCF we use the velocity table centered in the star rest frame, as operations using this table will be performed in the star rest frame, and the 
                ...
                if ('CCF' in data_inst['type']):                     
                    data_com = {'cen_bins':data_dic_temp['cen_bins'][0]-data_dic['DI']['sysvel'][inst][vis], 
                                'edge_bins':data_dic_temp['edge_bins'][0]-data_dic['DI']['sysvel'][inst][vis]}                
                elif ('spec' in data_inst['type']):
                    data_com = {'cen_bins':data_dic_temp['cen_bins'][0], 
                                'edge_bins':data_dic_temp['edge_bins'][0]}
                data_com['dim_exp']=data_inst[vis]['dim_exp']
                data_com['nspec']=data_inst[vis]['nspec']
...
'''
Estimate of detector gain 
    - see def_weights_spatiotemp_bin()
    - the gain is used as weight or to scale back profiles approximatively from flux to count units during CCF calculations (this allows artifically increasing errors when combining spectral regions with different SNRs)
      spectra must however always be kept in flux units, as the stellar spectrum shifts over time in the detector rest frame
      ...
      shifted stellar spectra with the same profile would thus have a different color balance after being converted in count units, preventing in particular the correct calculation of binned spectra 
    - weights are used for temporal binning, and are relevant only if the weight of a given pixel change over time, the gain is thus not defined for CCFs
    - a median gain profile over all visits of an instrument is calculated in spectral mode 
      we first fit a model to the gain for each exposure, so that it can be extrapolated over a larger, common range for all visits 
      then we calculate the median of these extrapolated model, interpolate it as a function, and use it to define the common gain profile over the specific table of each exposure
    - overall the estimated gains are stable between exposures but low count levels ten to yield larger gains - hence the independent calculation of gain per exposure, before taking the median over the visit
      this may come from additional noise sources: if E = Ewhite + Ered = sqrt(gdet_true)*sqrt(F) + Er then gdet_true = (E-Er)^2/F < E^2/F = gdet_meas (since Er < 2*E), so that we underestimate the actual gain. 
      in this case our assumptions to estimate gdet do not hold anymore, but for the purpose of weighing the exposures and scaling to equivalent count levels we still use the measured gain
      since g_meas = (Ew + Er)^2/F this factor accounts for the true gain but also for additional noise sources        
    - spectra are typically provided in the solar barycentric rest frame, and are thus not defined here in the detector rest frame, so that the gain profiles in different epochs may be shifted by the Earth barycentric RV difference   
      we nonetheless use a single gain profile, constant in time and common to all processed exposures of an instrument, so that the relative color balance between spectra is not modified when converting them back to counts 
      for the same reason the gain must be applied uniformely (ie, in the same rest frame) to spectra in different exposures, and their master, so that it does not affect their combinations - even if the original count-to-flux
 conversion is applied in the input rest frame
'''
def calc_det_gain(gen_dic,data_dic,inst,plot_dic,coord_dic):
    print('   > Estimating detector gain')
    data_inst = data_dic[inst] 
        
    #Calculating data
    if gen_dic['calc_det_gain']:
        print('         Calculating data')        
        gain_inputs_dic = {} 
        minmax_def = {}
        ...
        for vis in data_dic[inst]['visit_list']:
            ...
            #Estimate of detector flux gain
            ...
            data_vis['gain_data_paths'] = gen_dic['save_data_dir']+'Processed_data/Gains/'+inst+'_'+vis+'_'
            if data_vis['mock'] or (not gen_dic[inst][vis]['flag_err']):
                ...
                for iord in range(data_dic[inst]['nord']):                    
                    for iexp_glob,iexp_in_group in enumerate(iexp_gain_groups):
                        ...
                        for iexp in iexp_in_group:
                            ...
                            #Defined bins
                            #    - we further exclude ranges that were found to display abnormal gain estimates
                        ...
                        if np.sum(idx_def_group)>0:
                            bin_bd,raw_loc_dic = sub_def_bins(gen_dic['gain_binw'],idx_def_group,low_wav_group,high_wav_group,high_wav_group-low_wav_group,wav_group,flux_group,var1D_loc=var_group)
                ...
                for key in ['blue','mid','red']:
                    if (gen_dic['gain_deg'][key]<2) or (gen_dic['gain_deg'][key]>4):stop('Degrees must be between 2 and 4')
                for ideg in range(gen_dic['gain_deg']['mid']+1,5):p_start['b'+str(ideg)].vary = False                             
                for ideg in range(gen_dic['gain_deg']['blue']+1,5):p_start['a'+str(ideg)].vary = False                                   
                for ideg in range(gen_dic['gain_deg']['red']+1,5):p_start['c'+str(ideg)].vary = False   
                nfree_gainfit =  gen_dic['gain_deg']['blue']+gen_dic['gain_deg']['red']+1+gen_dic['gain_deg']['mid']-2                                     

                fixed_args={
                    'use_cov':False,
                    'deg_low':gen_dic['gain_deg']['blue'],
                    'deg_mid':gen_dic['gain_deg']['mid'],
                    'deg_high':gen_dic['gain_deg']['red'],
                    'constant':None                                                
                    }                                           
        ...
        #    - we assume the spectral tables are always more resolved than the typical variations of the gain profiles
        #    - the gain profile is defined in the input rest frame, over a regular table in log oversampled compared to the instrument resolution (dlnw_inst = dw/w = 1/R)
        mean_gdet_func = {} 
        gain_grid_dlnw = 0.5/return_resolv(inst)
        for iord in range(data_dic[inst]['nord']): 

            #Table of definition of mean gain profile over each order
            ...
            #Median gain over all exposures in the visit
            #    - interp1d is more stable at the edges than CubicSpline, and capture better the gain variations than polynomials
            #    - the small variations in measured gain between orders may result in two slices having different profiles
            ...
            #    - we limit the gain below the chosen global outlier threshold
            #    - we do not extrapolate beyond the range of definition of the median gain profile to avoid spurious behaviour
            med_gdet_ord = 1e3*med_gdet_allvis/gen_dic[inst]['n_visits'] 
            med_gdet_ord[med_gdet_ord>gen_dic['gain_thresh'][inst]['global']] = gen_dic['gain_thresh'][inst]['global']
            med_gdet_ord[med_gdet_ord<0.]=0.
            mean_gdet_func[iord] = interp1d(cen_bins_ord,med_gdet_ord,bounds_error=False,fill_value=(med_gdet_ord[0],med_gdet_ord[-1]))  

        #Store mean gain function
        np.savez_compressed(gen_dic['save_data_dir']+'Processed_data/Gains/'+inst+'_mean_gdet',data = {'func':mean_gdet_func},allow_pickle=True)  

        #Define gain tables for each exposure
        #    - the profile is the same, but defined over the table of the exposure
        #    - the path is made specific to a visit and a type of profile so that the gain function can still be called in the multi-visit routines for any type of profile,
        ...
        for vis in data_inst['visit_list']: 
            data_inst[vis]['gain_data_paths'] = gen_dic['save_data_dir']+'Processed_data/Gains/'+inst+'_'+vis+'_'
    ...
    for iexp_glob,iexp_gain_group in zip(iexp_glob_groups,iexp_gain_groups):
        ...
        for iord in range(nord):
            ...
            if gdet_val_all[iexp_glob,iord] is not None:
                ...
                cond_fit = (bin_ord_dic['gdet']>0.) & (bin_ord_dic['gdet']<gain_thresh['global']/1e3)
                ...
                fixed_args.update({'w_lowedge':low_edge+gain_edges['blue']*drange,
                                   'w_highedge':high_edge-gain_edges['red']*drange,
                                   'wref':0.5*(low_edge+high_edge)})  
                
                #Fit binned gain profile and define complete gain profile
                ...
                if (np.sum(cond_fit)>nfree_gainfit):
                    ...
                    #    - sigma-clipping applied to the inner parts of the order to prevent excluding edges where gain can vary sharply
                    #    - negative values are removed
                    #    - weights are used to prevent point with large gains (associated with low fluxes) biasing the simple polynomial fit
                    cond_check = deepcopy(cond_fit) 
                    if inst in gain_nooutedge:cond_check = cond_fit & (bin_ord_dic['cen_bins']>=bin_ord_dic['cen_bins'][0]+gain_nooutedge[inst][0]) & (bin_ord_dic['cen_bins']<=bin_ord_dic['cen_bins'][-1]-gain_nooutedge[inst][1]) 
                    if (np.sum(cond_check)>nfree_gainfit):
                        var_fit = bin_ord_dic['gdet'][cond_check]
                        _,merit,_ = fit_minimization(ln_prob_func_lmfit,p_start,bin_ord_dic['cen_bins'][cond_check],bin_ord_dic['gdet'][cond_check],np.array([var_fit]),gain_piecewise_func,verbose=False,fixed_args=fixed_args)  
                        res_gdet = bin_ord_dic['gdet'][cond_check] - merit['fit']
                        cond_fit[cond_check] = np.abs(res_gdet)<=gain_thresh['outliers']*np.std(res_gdet)
                        ...
                        if (np.sum(cond_fit)>nfree_gainfit):
                            ...
                            _,merit,p_best = fit_minimization(ln_prob_func_lmfit,p_start,bin_ord_dic['cen_bins'][cond_fit],bin_ord_dic['gdet'][cond_fit],np.array([var_fit]),gain_piecewise_func,verbose=False,fixed_args=fixed_args)                                              
...
'''
Joined polynomials to model gain functions
'''
def gain_piecewise_func(param_in,wav_in,args=None):
...
'''
def CCF_from_spec(mode,inst,vis,data_dic,gen_dic):
    data_vis=data_dic[inst][vis]
    gen_vis=gen_dic[inst][vis]
    data_type_gen=gen_dic['data_type_gen'][mode]
    dir_save = {}
    if mode=='DI':
        ...
        iexp_list= range(data_vis['n_in_visit'])
        dir_save['Res'] = gen_dic['save_data_dir']+'Res_data/CCFfromSpec/'+inst+'_'+vis+'_'
        dir_save['Intr'] = gen_dic['save_data_dir']+'Intr_data/CCFfromSpec/'+inst+'_'+vis+'_'
    ...
    if gen_dic['calc_'+data_type_gen+'_CCF']:
        ...
        for iexp_sub,iexp in enumerate(iexp_list):
            ...
            if data_type_gen in ['Intr','Atm']:
                data_ref = np.load(data_vis['mast_'+gen+'_data_paths'][iexp_eff]+'.npz',allow_pickle=True)['data'].item() 
                if iexp_sub==0:
        ...
        for iexp_sub,iexp in enumerate(iexp_list):
            ...
            if (data_type_gen=='Intr'):
                if (iexp in gen_vis['idx_in']):
                    ...
                    if ('Intr' in data_dic['Atm']['no_plrange']) and (iexp in data_dic['Atm'][inst][vis]['iexp_no_plrange']):
                        cond_in_pl = ~(np.ones(data_vis['nvel'],dtype=bool) & excl_plrange(cond_def_exp[0],data_dic['Atm'][inst][vis]['exclu_rangestar'],iexp,edge_bins[0],'CCF'))
            ...
            if data_type_gen in ['Intr','Atm']:
                np.savez_compressed(dir_mast[gen][iexp_eff],data={'edge_bins':edge_bins,'flux':CCF_ref[iexp_sub],'cov':cov_ref},allow_pickle=True)
...
def ResIntr_CCF_from_spec(inst,vis,data_dic,gen_dic):
    ...
    CCF_from_spec('Intr',inst,vis,data_dic,gen_dic)
    ...
    for i_in,iexp in zip(gen_vis['idx_exp2in'],range(data_vis['n_in_visit'])): 
        ...
        if (gen=='Res') and ('Res_CCF' in data_dic['Atm']['no_plrange']) and (iexp in data_dic['Atm'][inst][vis]['iexp_no_plrange']):           
            cond_def_cont_all[iexp] &= excl_plrange(cond_def_cont_all[iexp],data_dic['Atm'][inst][vis]['exclu_rangestar'],iexp,data_exp['edge_bins'][0],'CCF')
    
    #Continuum common to intrinsic profiles
    cond_cont_com  = np.all(cond_def_cont_all[gen_vis['idx_in']],axis=0)
    ...
    cont_intr = np.ones(data_vis['n_in_tr'])
    wcont_intr = np.ones(data_vis['n_in_tr'])
    for i_in,iexp in zip(gen_vis['idx_exp2in'],range(data_vis['n_in_visit'])): 
        if i_in ==-1:
            gen = 'Res'
            iexp_eff = iexp
        ...
        if gen == 'Intr':
            cont_intr[i_in] = np.mean(data_exp['flux'][0][cond_cont_com])
            wcont_intr[i_in] = 1./np.sum( data_exp['cov'][0][0,cond_cont_com] )
        
    #Mean continuum over all intrinsic CCFs
    ...
    data_dic['Intr']['mean_cont']=np.sum(cont_intr*wcont_intr)/np.sum(wcont_intr)            

    #Continuum correction
    #    - when calculated from spectra, intrinsic CCFs can show deviations from the common continuum level they should have 
    #      here we set manually their continuum to the mean continuum of all intrinsic CCFs before correction
    if data_dic['Intr']['cont_norm']:
        print('         Correcting CCF continuum')
        for i_in in range(data_vis['n_in_tr']):
            data_exp = np.load(data_vis['proc_Intr_data_paths']+str(i_in)+'.npz',allow_pickle=True)['data'].item()
            
            #Correction factor
            corr_exp = data_dic['Intr']['mean_cont']/cont_intr[i_in]

            #Overwrite exposure data
            data_exp['flux'][0],data_exp['cov'][0] = bind.mul_array(data_exp['flux'][0],data_exp['cov'][0],np.repeat(corr_exp,data_vis['nspec']))
            np.savez_compressed(data_vis['proc_Intr_data_paths']+str(i_in),data=data_exp,allow_pickle=True)           
...
'''
def new_compute_CCF(edge_wav,flux,cov,resamp_mode,edge_velccf,wght_mask,wav_mask,nthreads,gain = None):
    ...
    if (nthreads>1) and (nthreads<=len(wght_mask)):
        ...
        common_args=(n_RV,edge_wav,flux,cov,gain,resamp_mode)
        chunkable_args=[edge_mask_lines,wght_mask]
        fluxCCF,covCCF_line,nd_covCCF_line=parallel_new_compute_CCF(pool_proc,sub_new_compute_CCF,nthreads,len(wght_mask),chunkable_args,common_args)                           
        pool_proc.close()
        pool_proc.join()
        
    #Regular routine
    else:
        fluxCCF,covCCF_line,nd_covCCF_line=sub_new_compute_CCF(edge_mask_lines,wght_mask,n_RV,edge_wav,flux,cov,gain,resamp_mode)
    ...
    return fluxCCF,covCCF


def sub_new_compute_CCF(edge_mask,wght_mask,n_RV,edge_wav,flux,cov,gain,resamp_mode):
    ...
    for isub,(edge_mask_line,wght_mask_line) in enumerate(zip(edge_mask,wght_mask)):
        ...
        #    - if a gain profile is provided as input we take its mean value in the local line range and use it to scale the input spectrum
    ...
    return fluxCCF,covCCF_line,nd_covCCF_line
...
def init_visit(data_prop,data_dic,vis,coord_dic,inst,system_param,gen_dic):
    ...
    if gen_dic['res_data']:
        if data_dic['Res']['extract_in'] and ('spec' in data_dic['Res']['type'][inst]) and (not data_vis['comm_sp_tab']):Res_data_vis['idx_to_extract'] = deepcopy(gen_dic[inst][vis]['idx_in'])
        else:Res_data_vis['idx_to_extract'] =  np.arange(data_vis['n_in_visit'],dtype=int) 
    ...
    if (data_dic['Atm']['exc_plrange']):
        ...
        plrange_star={}
        plAtm_vis['exclu_range']={'CCF':{},'spec':{}}
        plAtm_vis['exclu_rangestar']={'CCF':{},'spec':{}}
        for pl_loc in data_vis['transit_pl']:
            ...
            #    - if input data is in spectral mode, ranges are defined in both spectral and RV space
            #      if input data is in CCF mode, ranges are only defined in RV   
            plAtm_vis['exclu_rangestar']['CCF'][pl_loc] = plrange_star[pl_loc]
            plAtm_vis['exclu_range']['CCF'][pl_loc] =  plrange_star[pl_loc] + coord_vis['RV_star_solCDM'][None,:]
                
            if ('spec' in data_dic[inst]['type']): 
                plAtm_vis['exclu_rangestar']['spec'][pl_loc] = np.zeros([len(data_dic['Atm']['CCF_mask_wav']),2,data_vis['n_in_visit']])*np.nan
                plAtm_vis['exclu_rangestar']['spec'][pl_loc][:,:,iexp_no_plrange] = data_dic['Atm']['CCF_mask_wav'][:,None,None]*spec_dopshift(-plrange_star[pl_loc][:,iexp_no_plrange])  

                plAtm_vis['exclu_range']['spec'][pl_loc] = np.zeros([len(data_dic['Atm']['CCF_mask_wav']),2,data_vis['n_in_visit']])*np.nan
                plAtm_vis['exclu_range']['spec'][pl_loc][:,:,iexp_no_plrange] = data_dic['Atm']['CCF_mask_wav'][:,None,None]*spec_dopshift(- (plrange_star[pl_loc][:,iexp_no_plrange]+coord_vis['RV_star_solCDM'][iexp_no_plrange]))  
...
def update_data_inst(data_dic,inst,gen_dic):
    ...
    if gen_dic['CCF_from_sp']:
        ...
        data_inst['mean_gdet'] = False 
    data_com_ref = np.load(data_inst[data_inst['com_vis']]['proc_com_data_paths']+'.npz',allow_pickle=True)['data'].item()   
...
def excl_plrange(cond_def,range_star_in,iexp,edge_bins,data_type):
    ...
    if data_type=='CCF':
        ...
        for pl_loc in range_star:
            cond_kept[ ((edge_bins[0:-1]>=range_star[pl_loc][0,iexp]) & (edge_bins[1:]<=range_star[pl_loc][1,iexp]))] = False 

    elif 'spec' in data_type:
        range_star = range_star_in['spec']
        
        #Defined bins in spectrum
        #    - we do not check pixels already undefined to gain time
        ...
        for pl_loc in range_star:
            ...
            if np.sum(cond_keepL)>0:
                ...
                idx_first_overpix =np.searchsorted(edge_bins[idx_def_exp_pl],range_star[pl_loc][cond_keepL,0],side='right')-1
                idx_last_overpix =np.searchsorted(edge_bins[idx_def_exp_pl+1],range_star[pl_loc][cond_keepL,1],side='left')
                ...
                for idx_first_overpix_loc,idx_last_overpix_loc in zip(idx_first_overpix,idx_last_overpix):
                    idx_undef = np.arange(idx_first_overpix_loc,idx_last_overpix_loc+1)
                    cond_kept[idx_def_exp_pl[idx_undef]] = False
...
'''
def corr_line_prof_gen(corr_prop_in,var,corr_in,mode):
...
def corr_line_prof(corr_line_prof_dic,data_dic,coord_dic,inst,vis,CCF_dic,data_prop,gen_dic,plot_dic):
    cond_trend = (corr_line_prof_dic['corr_trend'] and (inst in corr_line_prof_dic['prop']) and (vis in corr_line_prof_dic['prop'][inst]))
    cond_PC = (corr_line_prof_dic['corr_PC'] and (inst in corr_line_prof_dic['PC_model']) and (vis in corr_line_prof_dic['PC_model'][inst]))
    ...
    if cond_trend or cond_PC or cond_custom:
        ...
        if gen_dic['calc_corr_line_prof']:
            print('         Calculating data')
            ...
            if cond_trend:
                ...
                corr_prop = corr_line_prof_dic['prop'][inst][vis]
                ...
                if np.any([corr_loc=='FWHM' for corr_loc in prop_corr]):corr_FWHM=True
                ...
                for iexp in range(data_vis['n_in_visit']):
                    ...
                    if corr_ctrst:            
                        ...
                        if ('DI_CCF' in data_dic['Atm']['no_plrange']) and (iexp in data_dic['Atm'][inst][vis]['iexp_no_plrange']):           
                            cond_def_cont_all[iexp] &= excl_plrange(data_exp['cond_def'][0],data_dic['Atm'][inst][vis]['exclu_range'],iexp,data_exp['edge_bins'][0],'CCF')
    
                        #------------------------
    
                        #With phase   
                        if 'ctrst_phase' in corr_list:glob_corr_ctrst[iexp] = corr_line_prof_gen(corr_prop['ctrst_phase'],cen_ph_exp,glob_corr_ctrst[iexp],'modul')
                        
                        #With SNR           
                        if 'ctrst_snr' in corr_list:glob_corr_ctrst[iexp] = corr_line_prof_gen(corr_prop['ctrst_snr'],SNR_exp,glob_corr_ctrst[iexp],'modul')
                        elif 'ctrst_snrQ' in corr_list:glob_corr_ctrst[iexp] = corr_line_prof_gen(corr_prop['ctrst_snrQ'],SNRQ_exp,glob_corr_ctrst[iexp],'modul')
    
                        #With airmass and indexes
                        for key in env_prop:
                            if 'ctrst_'+key in corr_list:glob_corr_ctrst[iexp] = corr_line_prof_gen(corr_prop['ctrst_'+key],env_prop[key],glob_corr_ctrst[iexp],'modul')
                    ...
                    if corr_FWHM:                   
                        ...
                        #With phase   
                        if 'FWHM_phase' in corr_list:glob_corr_FWHM[iexp] = corr_line_prof_gen(corr_prop['FWHM_phase'],cen_ph_exp,glob_corr_FWHM[iexp],'modul')
                        
                        #With SNR           
                        if 'FWHM_snr' in corr_list:glob_corr_FWHM[iexp] = corr_line_prof_gen(corr_prop['FWHM_snr'],SNR_exp,glob_corr_FWHM[iexp],'modul')
                        elif 'FWHM_snrQ' in corr_list:glob_corr_FWHM[iexp] = corr_line_prof_gen(corr_prop['FWHM_snrQ'],SNRQ_exp,glob_corr_FWHM[iexp],'modul')
                        
                        #With airmass and indexes
                        for key in env_prop:
                            if 'FWHM_'+key in corr_list:glob_corr_FWHM[iexp] = corr_line_prof_gen(corr_prop['FWHM_'+key],env_prop[key],glob_corr_FWHM[iexp],'modul')
                    ...
                    if corr_RV:    
                        ...
                        if 'RV_phase' in corr_list:
                            glob_corr_RV[iexp] = corr_line_prof_gen(corr_prop['RV_phase'],cen_ph_exp,glob_corr_RV[iexp],'add')
                            
                        #With SNR
                        if 'RV_snr' in corr_list:
                            glob_corr_RV[iexp] = corr_line_prof_gen(corr_prop['RV_snr'],SNR_exp,glob_corr_RV[iexp],'add')
    
                        elif 'RV_snrQ' in corr_list:
                            glob_corr_RV[iexp] = corr_line_prof_gen(corr_prop['RV_snrQ'],SNRQ_exp,glob_corr_RV[iexp],'add')
    
                        #With airmass and indexes
                        for key in env_prop:
                            if 'RV_'+key in corr_list:glob_corr_RV[iexp] = corr_line_prof_gen(corr_prop['RV_'+key],env_prop[key],glob_corr_RV[iexp],'add')
    
                #Normalizing around the mean
                glob_corr_ctrst/=np.mean(glob_corr_ctrst)                    
                glob_corr_FWHM/=np.mean(glob_corr_FWHM)                     
               
                #Continuum common to all input CCFs
                if corr_ctrst: 
            ...
            if cond_PC:
                ...
                #PCA results
                pca_results = np.load(corr_line_prof_dic['PC_model'][inst][vis]['all'],allow_pickle=True)['data'].item() 

                #Upload flux scaling 
                data_scaling = (np.load(gen_dic['save_data_dir']+'Scaled_data/'+inst+'_'+vis+'_add'+'.npz',allow_pickle=True)['data'].item())

                #Joint intrinsic fit
                if 'in' in corr_line_prof_dic['PC_model'][inst][vis]:
                    jointfit_results = np.load(corr_line_prof_dic['PC_model'][inst][vis]['in'],allow_pickle=True)['data'].item() 
                ...
                if (inst in corr_line_prof_dic['idx_PC']) and (vis in corr_line_prof_dic['idx_PC'][inst]):idx_pc = corr_line_prof_dic['idx_PC'][inst][vis]
                else:idx_pc = range(pca_results['n_pc'])
                
            #----------------------------------------------------------------------------------------               

            #Correct each exposure  
            proc_DI_data_paths_new = gen_dic['save_data_dir']+'corr_line_prof/'+inst+'_'+vis+'_'                          
            for iexp in range(data_vis['n_in_visit']):
                ...
                if ('spec' in data_dic[inst][vis]['type']):
                    ...
                    #Radial velocity table associated with chosen transition
                    bins_edge = c_light*( (data_exp['edge_bins'][0]/corr_line_prof_dic['line_trans']) - 1.) 
                    bins_cen = c_light*( (data_exp['cen_bins'][0]/corr_line_prof_dic['line_trans']) - 1.)  
    
                else:
                    bins_edge = data_exp['edge_bins'][0]
                    bins_cen = data_exp['cen_bins'][0]             
                ...
                if cond_trend:
                    ...
                    if corr_ctrst:                 
                        glob_corr=np.repeat(glob_corr_ctrst[iexp],data_vis['nspec'])
                        data_exp['flux'][0],data_exp['cov'][0] = corr_line_prof_ctrst(data_exp['flux'][0],glob_corr,cond_cont_com,in_cov=data_exp['cov'][0])
                                       
                    #Correcting for RV variation    
                    if corr_RV: 
                        bins_cen,bins_edge,data_exp['flux'][0],data_exp['cov'][0] = corr_line_prof_RV(data_exp['flux'][0],bins_cen,bins_edge,glob_corr_RV[iexp],gen_dic,in_cov=data_exp['cov'][0],comm_sp_tab=data_vis['comm_sp_tab'])
                ...
                #Custom correction
                if cond_custom:
                    bins_cen -= cust_corr_RV[iexp]
                    bins_edge -= cust_corr_RV[iexp]
                ...
                if cond_PC: 
                    ...
                    if (iexp in pca_results['idx_corr']) or ((jointfit_results is not None) and (i_in in jointfit_results['idx_in_fit'][inst][vis])):
                        ...
                        if ('spec' in data_dic[inst][vis]['type']):
                            bins_edge_PCA = c_light*( (pca_results['edge_bins'][0]/corr_line_prof_dic['line_trans']) - 1.)  
                        else:
                            bins_edge_PCA = pca_results['edge_bins'][0]                            
                        ...
                        if ((jointfit_results is not None) and (i_in in jointfit_results['idx_in_fit'][inst][vis])):
                            ...
                            LC_theo_exp = 1. - data_scaling['loc_flux_scaling'][iexp,0](bins_edge_PCA)
                        ...
                        #Scale to the level of disk-integrated flux
                        pc_mod_exp*=data_scaling['glob_flux_scaling'][iexp]
  
                        #Temporary exposure table centered in star rest frame
                        edge_bins_shift = bins_edge - coord_dic[inst][vis]['RV_star_solCDM'][iexp] 
                        ...
                        #Correction
                        data_exp['flux'][0]+=pc_mod_exp
    
                #-----------------------------------------------------
                #Saving corrected data
                #----------------------------------------------------- 
    
                #Switch spectrum into wavelength space
                if ('spec' in data_dic[inst][vis]['type']):
                    data_exp['edge_bins'] = np.array([corr_line_prof_dic['line_trans']*(1. + (bins_edge/c_light))])
                    data_exp['cen_bins']  = np.array([corr_line_prof_dic['line_trans']*(1. + (bins_cen/c_light))])                      
                else:
                    data_exp['edge_bins'] = np.array([bins_edge])
                    data_exp['cen_bins']  = np.array([bins_cen])  
    
                #Updating defined bins
                data_exp['cond_def'][0] = ~np.isnan(data_exp['flux'][0])
            ...
            #Updating path to processed data and checking it has been calculated
            data_vis['proc_DI_data_paths'] = gen_dic['save_data_dir']+'corr_line_prof/'+inst+'_'+vis+'_' 
...
Sub-function to correct line profile contrast
    - line profiles are temporarily set to a null continuum so that their contrast can be 'stretched'
      corr_line_prof=((in_CCF-cont_CCF)/corr_val)+cont_CCF 
'''
def corr_line_prof_ctrst(in_CCF,corr_val,cond_cont_com,in_cov=None):

    #CCF continuum flux
    #    - defined in the same way as when fitting CCFs, to be consistent with the definition of the contrast
    cont_CCF=np.mean(in_CCF[cond_cont_com])
                
    #Correcting for contrast variation
    #    - the covariance is only modified by the 'corr_val' division
    if in_cov is not None:
        flux_temp = in_CCF-cont_CCF
        flux_temp,corr_cov = bind.mul_array(flux_temp,in_cov,1./corr_val)
        corr_line_prof = flux_temp+cont_CCF
    else:
        corr_line_prof=((deepcopy(in_CCF)-cont_CCF)/corr_val)+cont_CCF          
   
    return corr_line_prof,corr_cov


'''
Sub-function to shift a line profile in RV
...
'''
def corr_line_prof_RV(in_CCF,bins_cen,bins_edge,RV_corr,gen_dic,in_cov=None,comm_sp_tab=False):
    if comm_sp_tab:
      if in_cov is not None:corr_line_prof,corr_cov= bind.resampling(bins_edge, bins_edge-RV_corr,in_CCF,cov=in_cov, kind=gen_dic['resamp_mode'])
      else:corr_line_prof= bind.resampling(bins_edge, bins_edge-RV_corr,in_CCF,kind=gen_dic['resamp_mode'])
    else:
        corr_line_prof = in_CCF 
        corr_cov = in_cov
        bins_cen -= RV_corr
        bins_edge -= RV_corr                    
    return bins_cen,bins_edge,corr_line_prof,corr_cov
...
def corr_line_prof_FWHM(in_CCF,bins_edge,FWHM_corr,RV_star_solCDM,gen_dic,in_cov=None,comm_sp_tab=False):
    ...
    edge_bins_shift = bins_edge - RV_star_solCDM 
...
def align_profiles(data_type,data_dic,inst,vis,gen_dic,coord_dic):
    ...
    if gen_dic['calc_align_'+data_type]:
        ...
        for isub,iexp in enumerate(prop_dic[inst][vis]['idx_def']):    
            ...
            #    - the gain profile is common to all exposures of a processed instrument, and is originally sampled over the table of each exposure in the detector rest frame
            #    - the gain profile is then used as weight in temporal binning, or to scale back profiles from flux to count units
    ...
    prop_dic[inst][vis]['idx_def'] = (np.load(data_vis['proc_'+data_type+'_data_paths']+'_add.npz',allow_pickle=True)['data'].item())['idx_aligned']
...
def align_data(data_exp,rout_mode,nord,dim_exp_resamp,resamp_mode,cen_bins_resamp, edge_bins_resamp,rv_shift_cen,rv_shift_edge = None , nocov = False):
    ...
    if ('spec' in rout_mode):
        ...
        #Achromatic shift
        if (rv_shift_edge is None):dop_shift = spec_dopshift(rv_shift_cen)                        
...
Scale data to the correct flux level
    ...
    - we first correct for L(t) by dividing the profiles by globF(v,t) = TFcorr(v,t)/med(tk, TFcorr(v,tk) ) or TFcorr(v,t)/(Fsc*int(w in full_band, dw )) 
 ...
 LC(band,t) = int(w over band, Fin(w,t)*dw )/int(w over band, Fstar(w,vis)*dw )
      ...
      the theoretical light curves fitted to the measured one assume a constant, uniform stellar flux and limb-darkening law, and a constant average radius
...
def rescale_data(data_inst,inst,vis,data_dic,coord_dic,exp_dur_d,gen_dic,plot_dic,system_param,theo_dic):
    ...
    if (gen_dic['calc_flux_sc']):
        ...
        if (plot_dic['input_LC']!='') or (plot_dic['prop_loc']!=''):
        ...
        loc_flux_scaling = np.zeros(data_vis['dim_sp'],dtype=object) 
        ...
        for iexp in range(data_vis['n_in_visit']): 
            ...
            if (system_prop['nw']==1):  
                for iord in range(data_inst['nord']):
                    loc_flux_scaling[iexp,iord] = np.poly1d([1.-LC_flux_band_all[iexp,0]])
            else:
                for iord in range(data_inst['nord']):
                    loc_flux_scaling[iexp,iord] = interp1d(system_prop['w'],1.-LC_flux_band_all[iexp],fill_value=(1.-LC_flux_band_all[iexp,0],1.-LC_flux_band_all[iexp,-1]), bounds_error=False)
    ...
    #Updating path to processed data and checking it has been calculated
        ...
        data_vis['proc_DI_data_paths']=gen_dic['save_data_dir']+'Scaled_data/'+inst+'_'+vis+'_'    
        check_data({'path':data_vis['proc_DI_data_paths']+str(0)})   
...
"""
def calc_binned_prof(idx_to_bin,nord,dim_exp,nspec,data_to_bin_in,data_dic,inst,n_in_bin,cen_bins_exp,edge_bins_exp,dx_ov_in=None):

    #Initialize global weight table
    ...
    glob_weight_spec = np.zeros([n_in_bin]+dim_exp,dtype=float)  
    dx_ov = np.ones(n_in_bin) if dx_ov_in is None else deepcopy(dx_ov_in) 
    data_to_bin = deepcopy(data_to_bin_in)          
    for isub,iexp in enumerate(idx_to_bin): 
        
        #Set as undefined pixels with undefined weights
        #    - weights can be undefined if some contributors were undefined
        data_to_bin[iexp]['cond_def'][np.isnan(data_to_bin[iexp]['weight'])]  =False 
        
        #Contributions to global weights
        #    - remains null for undefined bins (eg due to planetary range masking), so that they do not contribute to the bin 
        ...
        cond_def_exp = data_to_bin[iexp]['cond_def']
        glob_weight_spec[isub,cond_def_exp] = dx_ov[isub]*data_to_bin[iexp]['weight'][cond_def_exp]

        #Set to 0 undefined pixels so that they do not mess up co-additions
        data_to_bin[iexp]['flux'][~cond_def_exp] = 0.
    ...
    glob_weight_tot=np.sum(glob_weight_spec,axis=0)
    cond_def_ov = (glob_weight_tot>0.)
    glob_weight_spec[:,cond_def_ov]/=glob_weight_tot[cond_def_ov]
    ...
    for iord in range(nord):
        ...
        for isub,iexp in enumerate(idx_to_bin):   
            ...
            #            ov is the fraction covered by exposure i in the new exposure along the bin dimension
            ...
            if (True in data_to_bin[iexp]['cond_def'][iord]):                
                ...
                flux_temp,cov_temp = bind.mul_array(data_to_bin[iexp]['flux'][iord],data_to_bin[iexp]['cov'][iord],glob_weight_spec[isub,iord])
                ...
                data_bin['cond_def'][iord] |= data_to_bin[iexp]['cond_def'][iord]
            ...
            for isub_bin,(ibin,xbin_low_loc,xbin_high_loc) in enumerate(zip(idx_bin_proc,x_bd_low_proc,x_bd_high_proc)):   
                ...
                if n_overpix>0:
                    ...
                    if np.sum(cond_defbin_compdim)>0:    
                        ...
                        # F(pix) = g* N(pix) / dpix = N(dx)/dx
                        #      where g is the gain between flux and number of photons
                        #    - the flux in a bin is the total number of photons from the overlapping fractions of all pixels, divided by the effective overlapping surface (see below)
                        # F(bin) = g*sum( overlapping pixels, N(pixel overlap) )/sum( overlapping pixels, dx(pixel overlap) )
                        #       since F(pixel) = g*N(pixel overlap)/dx(pixel overlap) we obtain
...
Routine to extract local residual stellar profiles
 ...
 + the surface density flux can be written as fi(w,v) = Ii(w)*LDi(w) 
...
def extract_res_profiles(gen_dic,data_dic,inst,vis,data_prop,coord_dic):
    ...
    if (not gen_dic['align_DI']):print('WARNING: disk-integrated profiles must be aligned')
    data_inst = data_dic[inst]
    data_vis = data_inst[vis]
    ...
    if (gen_dic['calc_res_data']):
        ...
        if gen_dic['flux_sc']:scaled_data_paths = data_dic[inst][vis_bin]['scaled_data_paths']
        ...
        for iexp_off in idx_to_bin_unik:
            ...
            data_exp_off = np.load(data_inst[vis_bin]['proc_DI_data_paths']+str(iexp_glob)+'.npz',allow_pickle=True)['data'].item() 
            ...
            if gen_dic['gain_weight'] and data_vis['mean_gdet']:data_to_bin_gen[iexp_off]['mean_gdet'] = dataload_npz(data_inst[vis_bin]['mean_gdet_DI_data_paths'][iexp_glob])['mean_gdet'] 
            ...
            data_ref = np.load(data_dic[inst][vis_bin]['mast_DI_data_paths'][iexp_glob]+'.npz',allow_pickle=True)['data'].item() 
            ...
            if ((mode=='') and data_vis['comm_sp_tab']) or ((mode=='multivis') and data_inst['comm_sp_tab']):
                ...
                data_to_bin_gen[iexp_off]['weight'] = def_weights_spatiotemp_bin(range(data_inst['nord']),scaled_data_paths,inst,vis_bin,gen_dic['corr_Fbal'],gen_dic['corr_FbalOrd'],gen_dic['save_data_dir'],gen_dic['type'],data_inst['nord'],iexp_glob,'DI',data_vis['type'],data_vis['dim_exp'],data_to_bin_gen[iexp_off]['tell'],data_to_bin_gen[iexp_off]['mean_gdet'],data_to_bin_gen[iexp_off]['cen_bins'],flux_ref_exp,None,bdband_flux_sc = gen_dic['flux_sc'])
        ...
        for isub,iexp in enumerate(data_dic['Res'][inst][vis]['idx_to_extract']):        
            ...
            if (isub==0) or ((mode=='') and (not data_vis['comm_sp_tab'])) or ((mode=='multivis') and (not data_inst['comm_sp_tab'])):                
                data_to_bin={}
                for iexp_off in idx_to_bin_all[0]:
                    ...
                    if ((mode=='') and (not data_vis['comm_sp_tab'])) or ((mode=='multivis') and (not data_inst['comm_sp_tab']) and (vis_bin!=data_inst['com_vis'])):
                        data_to_bin[iexp_off]={}
                        
                        #Resampling exposure profile
                        data_to_bin[iexp_off]['flux']=np.zeros(data_vis['dim_exp'],dtype=float)*np.nan
                        data_to_bin[iexp_off]['cov']=np.zeros(data_inst['nord'],dtype=object) 
                        tell_exp=np.ones(data_vis['dim_exp'],dtype=float) if data_vis['tell_sp'] else None
                        mean_gdet_exp=np.ones(data_vis['dim_exp'],dtype=float) if gen_dic['gain_weight'] and data_vis['mean_gdet'] else None
                        for iord in range(data_inst['nord']): 
                            data_to_bin[iexp_off]['flux'][iord],data_to_bin[iexp_off]['cov'][iord] = bind.resampling(data_exp['edge_bins'][iord], data_to_bin_gen[iexp_off]['edge_bins'][iord], data_to_bin_gen[iexp_off]['flux'][iord] , cov = data_to_bin_gen[iexp_off]['cov'][iord], kind=gen_dic['resamp_mode'])                                                        
                            if data_vis['tell_sp']:tell_exp[iord] = bind.resampling(data_exp['edge_bins'][iord], data_to_bin_gen[iexp_off]['edge_bins'][iord], data_to_bin_gen[iexp_off]['tell'][iord] , kind=gen_dic['resamp_mode']) 
                            if gen_dic['gain_weight'] and data_vis['mean_gdet']:mean_gdet_exp[iord] = bind.resampling(data_exp['edge_bins'][iord], data_to_bin_gen[iexp_off]['edge_bins'][iord],data_to_bin_gen[iexp_off]['mean_gdet'][iord], kind=gen_dic['resamp_mode'])   
                            data_to_bin[iexp_off]['cond_def'] = ~np.isnan(data_to_bin[iexp_off]['flux'])   

                        #Weight definition 
                        flux_ref_exp = np.ones(data_vis['dim_exp'],dtype=float)
                        data_to_bin[iexp_off]['weight'] = def_weights_spatiotemp_bin(range(data_inst['nord']),scaled_data_paths,inst,vis_bin,gen_dic['corr_Fbal'],gen_dic['corr_FbalOrd'],gen_dic['save_data_dir'],gen_dic['type'],data_inst['nord'],iexp_glob,'DI',data_vis['type'],data_vis['dim_exp'],tell_exp,mean_gdet_exp,data_exp['cen_bins'],flux_ref_exp,None,bdband_flux_sc = gen_dic['flux_sc'])
                    ...
                    if ('DI_Mast' in data_dic['Atm']['no_plrange']) and (iexp_glob in data_dic['Atm'][inst][vis_bin]['iexp_no_plrange']):
                        for iord in range(data_inst['nord']):                        
                            data_to_bin[iexp_off]['cond_def'][iord] &= excl_plrange(data_to_bin[iexp_off]['cond_def'][iord],data_dic['Atm'][inst][vis_bin]['exclu_rangestar'],iexp_off,data_exp['edge_bins'][iord],data_vis['type'])

                #Calculate master on current exposure table
                data_mast = calc_binned_prof(idx_to_bin_all[0],data_dic[inst]['nord'],data_vis['dim_exp'],data_vis['nspec'],data_to_bin,data_dic,inst,n_in_bin_all[0],data_exp['cen_bins'],data_exp['edge_bins'],dx_ov_in = dx_ov_all[0])
    ...
    #Path to weighing master and gain profile
    ...
    #    - gain paths are updated even if they are not used as weights, to be used in flux/count scalings
...
Routine to process individual intrinsic stellar profiles
  ...
  + when CCFs are calculated from residual spectra the continuum is unknown a priori, as it depends on the reference spectrum to which each DI spectrum corresponds to, times the spectral flux scaling during transit
    ...
    when converting them into CCFloc, we also compute the equivalent CCFscal of (1 - LC_theo(band,t))
    CCFloc are then converted into CCFintr in this routine by dividing their continuum using CCFscal 
    this is an approximation, since the spectral scaling cannot be isolated from F_intr(w,t,vis) when computing CCFloc, but it is the same approximation we do when scaling DI CCFs with a white light transit
    we take this approach rather than first calculate Fintr and then its CCF because we need CCFloc to later derive the atmospheric CCFs
...
def extract_intr_profiles(data_dic,gen_dic,inst,vis,star_params,coord_dic,theo_dic,plot_dic):
    print('   > Calculating intrinsic stellar profiles')  
    data_vis=data_dic[inst][vis]
    gen_vis = gen_dic[inst][vis]
    ...
    data_vis['proc_Intr_data_paths']=proc_gen_data_paths_new+'_in'  
    ...
    for i_in,iexp in enumerate(gen_vis['idx_in']):
        ...
        if data_vis['mean_gdet']:data_vis['mean_gdet_Intr_data_paths'][i_in] = data_vis['mean_gdet_Intr_data_paths'][iexp]
    ...
    if (gen_dic['calc_intr_data']):
        ...
        for i_in,iexp in enumerate(gen_vis['idx_in']):
            ...
            if intr_rv_corr:
                ...
                if data_vis['tell_sp']:data_exp['tell'] = dataload_npz(data_vis['tell_Res_data_paths']+str(iexp))['tell'] 
                if data_vis['mean_gdet']:data_exp['mean_gdet'] = dataload_npz(data_vis['mean_gdet_Res_data_paths']+str(iexp))['mean_gdet'] 
            ...
            if ('Intr' in data_dic['Atm']['no_plrange']) and (not gen_dic['Intr_CCF']) and (iexp in plAtm_vis['iexp_no_plrange']):
                ...
                for iord in range(data_dic[inst]['nord']):                        
                    cond_out_pl[iord] &= excl_plrange(data_exp['cond_def'][iord],plAtm_vis['exclu_rangestar'],iexp,data_exp['edge_bins'][iord],data_vis['type'])
            ...
            np.savez_compressed(proc_gen_data_paths_new+'_in'+str(i_in),data=data_exp,allow_pickle=True)

    else:
        data_paths={i_in:proc_gen_data_paths_new+'_in'+str(i_in) for i_in in data_dic['Intr'][inst][vis]['idx_def']}
        check_data(data_paths)  

    #Continuum of intrinsic CCFs
    #    - see ResIntr_CCF_from_spec for details
    if data_vis['type']=='CCF':
        cond_def_cont_all  = np.zeros([data_vis['n_in_tr'],data_vis['nspec']],dtype=bool)
        for i_in in range(data_vis['n_in_tr']):
            data_exp = dataload_npz(data_vis['proc_Intr_data_paths']+str(i_in))
            if len(data_dic['Intr']['cont_range'][inst])==0:cond_def_cont_all[i_in,:] = True
            else:
                for bd_int in data_dic['Intr']['cont_range'][inst]:cond_def_cont_all[i_in] |= (data_exp['edge_bins'][0,0:-1]>=bd_int[0]) & (data_exp['edge_bins'][0,1:]<=bd_int[1])        
                cond_def_cont_all[i_in] &= data_exp['cond_def'][0]
        cond_cont_com  = np.all(cond_def_cont_all,axis=0)
        if np.sum(cond_cont_com)==0.:stop('No pixels in common continuum')  
        cont_intr = np.ones(data_vis['n_in_tr'])
        wcont_intr = np.ones(data_vis['n_in_tr'])
...
Routine to apply PCA
 ...
 Pert(w,t,v) ~ sum(k, Ipert(w,t,v)*LDk(band)*Sk) = Ipert(w,t,v)*sum(i, LDk(band)*Sk) = Ipert(w,t,v)*Sstar_LD(band)
    ...
    - we apply the correction to the raw disk-integrated profiles F(w,t,v)*Cref(band,v)*globF(v,t) in corr_line_prof(), so that the correction model is: 
        ...
        if (gen_dic['calc_pca_ana']):
            ...
            for isub,iexp in enumerate(idx_pca):
                ...
                if ('PCA_corr' in data_dic['Atm']['no_plrange']) and (iexp in data_dic['Atm'][inst][vis]['iexp_no_plrange']):
                    cond_in_pl = ~(np.ones(data_vis['nvel'],dtype=bool) & excl_plrange(data_exp['cond_def'][0],data_dic['Atm'][inst][vis]['exclu_rangestar'],iexp,data_exp['edge_bins'][0],'CCF'))
            ...
            for isub,iexp in enumerate(idx_corr): 
                ...
                result, merit ,p_best = fit_minimization(ln_prob_func_lmfit,p_start,data_exp['cen_bins'][0,cond_fit_range],flux_fit,cov_fit,pc_model,verbose=True,fixed_args=fixed_args)
...
'''
Routine to fit profiles
    - disk-integrated stellar profiles, intrinsic stellar profiles or atmospheric profiles
...
'''
def fit_prof(vis_mode,data_type,data_dic,gen_dic,inst,vis,coord_dic,theo_dic,plot_dic,star_param):
    ...
    if ('DI' in data_type) or ('Intr' in data_type):
        ...
        print('   > Fitting'+txt_print+gen_dic['type_name'][data_type_gen]+' stellar profiles')                 
    elif ('Atm' in data_type):   
        data_type_gen='Atm' 
        print('   > Fitting'+txt_print+gen_dic['type_name'][data_type_gen]+data_dic['Atm']['pl_atm_sign']+' profiles') 
    ...
    if cond_calc:
        ...
        #Generic fit options
        fit_properties={'type':data_mode,'nthreads':gen_dic['fit_prof_nthreads'],'rv_osamp_line_mod':gen_dic['rv_osamp_line_mod'],'use_cov':gen_dic['use_cov'],'varpar_priors':{},'jitter':False,'inst_list':[inst],'inst_vis_list':{inst:[vis_det]},
                        'save_dir':save_path+'_'+prop_dic['fit_mod']+'/','conv_model' : prop_dic['conv_model']}
        ...
        if (('DI' in data_type) and (inst in prop_dic['occ_range'])) or (data_type=='Introrig'):
            if len(data_inst[vis]['transit_pl'])>1:stop('Adapt model to multiple planets')
            else:
                ref_pl = data_inst[vis]['transit_pl'][0]
                surf_rv_mod = (np.load(gen_dic['save_data_dir']+'Introrig_prop/PlOcc_Prop_'+inst+'_'+vis+'.npz',allow_pickle=True)['data'].item())['achrom'][ref_pl]['rv'][0]
        ...
        if 'DI' in data_type:
            ...
            #Definition of ranges covered by occulted stellar lines
            #    - we center requested range around surface RVs in original rest frame
            if (inst in prop_dic['occ_range']):
                if data_type=='DIorig':RV_star_solCDM = coord_dic[inst][vis]['RV_star_solCDM']
                elif data_type=='DIbin':
                    if not (gen_dic['align_DI']):RV_star_solCDM = data_bin['RV_star_solCDM']
                    else:RV_star_solCDM = np.repeat(0.,fit_dic['n_exp'])
                occ_exclu_range = np.zeros([2,fit_dic['n_exp']],dtype=float)*np.nan
                occ_range = np.array(prop_dic['occ_range'][inst])[:,None] + (surf_rv_mod+RV_star_solCDM[idx_exp2in])                                
            ...
            if (prop_dic['model'][inst]=='custom'):
                ...
                if fit_properties['mode']=='Intrbin':
                    ...
                    data_Intrbin = np.load(gen_dic['save_data_dir']+'Intrbin_data/'+gen_dic['add_txt_path'][data_type_gen]+inst+'_'+vis_Intrbin+'_'+fit_properties['dim_bin']+'_add.npz',allow_pickle=True)['data'].item()
        ...
        #Intrinsic profiles
            ...
            #Original profiles
            if data_type=='Introrig':fit_dic['n_exp'] = data_inst[vis]['n_in_tr']
        ...
        #Atmospheric profiles
            ...
            #Binned profiles
            elif data_type=='Atmbin':iexp_def = range(fit_dic['n_exp'])
        ...
        for isub,iexp in enumerate(iexp_def):
            ...
            #Upload profile               
            if bin_mode=='':     data_fit_loc = np.load(data_inst[vis]['proc_'+data_type_gen+'_data_paths']+str(iexp)+'.npz',allow_pickle=True)['data'].item() 
            ...
            if (data_type=='DIorig') and ('DI_CCF' in data_dic['Atm']['no_plrange']) and (iexp in data_dic['Atm'][inst][vis]['iexp_no_plrange']):   
                cond_kept = excl_plrange(data_fit[isub]['cond_def'],data_dic['Atm'][inst][vis]['exclu_range'+rest_fr],iexp,data_fit[isub]['edge_bins'],data_mode)
        ...
        fit_dic['cond_detected'] = np.repeat(False,fit_dic['n_exp'])
        for isub,iexp in enumerate(iexp_def):            
            ...
            #Switch spectrum into RV space
            #    - facilitates the definition of models and convolution
            #    - only possible if the fit is focused on a single line
            if ('spec' in data_mode) and (prop_dic['mod_def'][inst]['mode']=='glob_mod'):

                #Radial velocity table associated with chosen transition
                #    - resampling only if variable bin size (RV table must be regular for model computation)
                rv_edge_in = c_light*( (data_fit[isub]['edge_bins']/prop_dic['line_trans']) - 1.)
                drv_tab = rv_edge_in[1:]-rv_edge_in[0:-1]
                mean_drv = np.mean(drv_tab)
                if np.max(np.abs(drv_tab-mean_drv))>1e-6:              
                    n_rv = int(np.ceil((rv_edge_in[-1] - rv_edge_in[0])/mean_drv))
                    bins_edge = rv_edge_in[0] + ((rv_edge_in[-1] - rv_edge_in[0])/n_rv) *np.arange(n_rv+1) 
                    data_fit[isub]['flux'],cov_exp = bind.resampling(bins_edge, rv_edge_in, data_fit[isub]['flux'], cov = cov_exp, kind=gen_dic['resamp_mode'])                                                          
 
                else:
                    bins_edge = rv_edge_in 
                bins_cen = 0.5*(bins_edge[0:-1]+bins_edge[1::])

                #Corresponding spectral table, if relevant
                #    - used to resample the model intensity grid
                fit_properties['cen_bins_sp'] = prop_dic['line_trans']*(1.+(bins_cen/c_light))

            else:
                bins_edge = data_fit[isub]['edge_bins']
                bins_cen = data_fit[isub]['cen_bins']    
            
            #Perform fit to individual profiles
            fit_properties.update({'iexp':iexp,'flux_cont':flux_cont,'DI_grid':True})
            fit_dic[iexp]=fit_prof_func(inst,data_dic,vis_det,prop_dic,gen_dic,prop_dic['verbose'],fit_dic['cond_def_fit_all'][isub],fit_dic['cond_def_cont_all'][isub] ,data_type_gen,bins_edge,bins_cen,data_fit[isub]['flux'],cov_exp,
                                        idx_force_det,theo_dic,star_param,fit_properties,prop_dic['line_fit_priors'],prop_dic['model'][inst],prop_dic['mod_prop'])

            #Switch back model into wavelength space
            if ('spec' in data_mode) and (prop_dic['mod_def'][inst]['mode']=='glob_mod'):
                for key in ['cen_bins','edge_bins','vel_HR']:fit_dic[iexp][key] = prop_dic['line_trans']*(1.+(fit_dic[iexp][key]/c_light))
            ...
            if data_type=='DIorig':
                if ('RVpip' in prop_dic[inst][vis]):
                    fit_dic[iexp]['RVpipres']=prop_dic[inst][vis]['RVpip'][iexp]-coord_dic[inst][vis]['RV_star_solCDM'][iexp]
                    fit_dic[iexp]['err_RVpipres']=prop_dic[inst][vis]['eRVpip'][iexp]  
                fit_dic[iexp]['RVres']=fit_dic[iexp]['rv']-coord_dic[inst][vis]['RV_star_solCDM'][iexp]
                fit_dic[iexp]['err_RVres']=fit_dic[iexp]['err_rv']
                if prop_dic['model'][inst]=='dgauss':
                    fit_dic[iexp]['RV_lobe_res']=fit_dic[iexp]['RV_lobe']-coord_dic[inst][vis]['RV_star_solCDM'][iexp]
            ...
            #    - we report the errors on the local velocities
                ...
                fit_dic[iexp]['RVres']=fit_dic[iexp]['rv']-surf_rv_mod[iexp] 
                fit_dic[iexp]['RVmod']=surf_rv_mod[iexp] 
            
            #Calculating residuals from orbital RVs (km/s)
            elif data_type=='Atmorig': fit_dic[iexp]['RVres']=fit_dic[iexp]['rv']-coord_dic[inst][vis]['rv_pl'][iexp_orig]
...
def init_bin_rout(data_type,bin_prop,idx_in_bin,dim_bin,coord_dic,inst,vis_to_bin,data_dic,gen_dic):
    ...
    for vis in vis_to_bin:
        ...
        if dim_bin in ['phase']:     
            ...
            x_cen_all = coord_vis['cen_ph'][idx_orig]         #central coordinates of all exposures, required for def_local_profiles()
            ...
            x_cen_all = transit_prop_nom[dim_bin][0,:]      #central coordinates of all exposures, required for def_local_profiles()  
...
def process_bin_prof(mode,data_type_gen,gen_dic,inst,vis_in,data_dic,coord_dic,data_prop,system_param,theo_dic,plot_dic,masterDI=False):
    ...
    if (calc_check):
        ...
        if mode=='multivis':
            ...
            #Bin properties
            bin_prop = prop_dic['prop_bin'][inst]['binned']
        ...
        #    - common table and dimensions are specific to this visit
            ...
            nspec_com = data_inst['nspec']
            vis_to_bin=[vis_in]
            bin_prop = prop_dic['prop_bin'][inst][vis_in]
        ...
        data_to_bin={}
        if (data_type_gen=='DI') and (not masterDI) and not (gen_dic['align_DI']):data_glob_new['RV_star_solCDM'] = np.zeros(n_bin,dtype=float)
        ...
        for iexp_off in idx_to_bin_unik:
            ...
            if gen_dic['gain_weight'] and data_inst[vis_bin]['mean_gdet']:data_exp['mean_gdet'] = dataload_npz(data_inst[vis_bin]['mean_gdet_'+data_type_gen+'_data_paths'][iexp])['mean_gdet'] 
            ...
            if masterDI:
                ...
                cov_ref_exp = None            
            else:data_ref = dataload_npz(data_dic[inst][vis_bin]['mast_'+data_type_gen+'_data_paths'][iexp])
            if ((mode=='') and (not data_inst[vis_bin]['comm_sp_tab'])) or ((mode=='multivis') and (not data_inst['comm_sp_tab']) and (vis_bin!=data_inst['com_vis'])):
                ...
                mean_gdet_exp=np.ones(dim_exp_com,dtype=float) if gen_dic['gain_weight'] and data_inst[vis_bin]['mean_gdet'] else None                
                for iord in range(data_inst['nord']): 
                    ...
                    if gen_dic['gain_weight'] and data_inst[vis_bin]['mean_gdet'] :mean_gdet_exp[iord] = bind.resampling(data_com['edge_bins'][iord], data_exp['edge_bins'][iord], data_exp['mean_gdet'][iord] , kind=gen_dic['resamp_mode'])                    
                data_to_bin[iexp_off]['cond_def'] = ~np.isnan(data_to_bin[iexp_off]['flux'])  
            ...
            if (data_type_gen=='DI') and ('DI_Mast' in data_dic['Atm']['no_plrange']) and (iexp_glob in data_dic['Atm'][inst][vis_bin]['iexp_no_plrange']):
                for iord in range(data_inst['nord']):                        
                    data_to_bin[iexp_off]['cond_def'][iord] &= excl_plrange(data_to_bin[iexp_off]['cond_def'][iord],data_dic['Atm'][inst][vis_bin]['exclu_rangestar'],iexp_glob,data_com['edge_bins'][iord],data_mode)
            ...
            data_to_bin[iexp_off]['weight'] = def_weights_spatiotemp_bin(range(data_inst['nord']),scaled_data_paths,inst,vis_bin,gen_dic['corr_Fbal'],gen_dic['corr_FbalOrd'],gen_dic['save_data_dir'],gen_dic['type'],data_inst['nord'],iexp_glob,data_type,data_mode,dim_exp_com,tell_exp,mean_gdet_exp,data_com['cen_bins'],flux_ref_exp,cov_ref_exp,flux_est_loc_exp=flux_est_loc_exp,cov_est_loc_exp = cov_est_loc_exp, SpSstar_spec = SpSstar_spec,bdband_flux_sc = gen_dic['flux_sc'])                          
        ...
        for i_new,(idx_to_bin,n_in_bin,dx_ov) in enumerate(zip(idx_to_bin_all,n_in_bin_all,dx_ov_all)):
            ...
            data_exp_new = calc_binned_prof(idx_to_bin,data_dic[inst]['nord'],dim_exp_com,nspec_com,data_to_bin,data_dic,inst,n_in_bin,data_com['cen_bins'],data_com['edge_bins'],dx_ov_in = dx_ov)
        ...
        #---------------------------------------------------------------------------
        #Calculating theoretical properties of planet-occulted regions
        #    - only possible if data binned over phase
        #    - new coordinates are relative to the planet chosen as reference for the binned coordinates 
        #---------------------------------------------------------------------------
        data_glob_new.update({'st_bindim':new_x_low,'end_bindim':new_x_high,'cen_bindim':new_x_cen,'n_exp':n_bin,'dim_all':[n_bin]+dim_exp_com,'dim_exp':dim_exp_com,'nspec':nspec_com})
...
Function to define weights when binning profiles temporally/spatially
    - weights should only be defined using the inverse squared error if the weighted values are comparable
'''
def def_weights_spatiotemp_bin(iord_orig_list,scaled_data_paths,inst,vis,gen_corr_Fbal,gen_corr_Fbal_ord,save_data_dir,gen_type,nord,iexp_glob,data_type,data_mode,dim_exp,tell_exp,mean_gdet,cen_bins,flux_ref_exp,cov_ref_exp,flux_est_loc_exp=None,cov_est_loc_exp=None,SpSstar_spec=None,bdband_flux_sc=False,glob_flux_sc=None,corr_Fbal = True):

    #Weight definition
    #    - reminder on fluxes and photon counts
    ...
    #      the flux density, per unit of time and wavelength, can be written as F = g*N where g is a gain factor from photons to energies and N the photon density per unit of time and wavelength       
    #      the corresponding error on the flux density is E = E[ g N ] = g*E(N) = g*sqrt(N), assuming no other noise sources
    #      if two independent measurements are made, eg related as N2 = a*N1, then E(N2) = sqrt(a*N1) = sqr(a)*E(N1) and the same with flux errors
    #      if the same measurement is manipulated, eg with N' = a*N1, then the rules of error propagation apply and E(N') = a*E(N1)
    #    - the true flux measured in a given exposure, and its error, can be expressed as:
    # F_true(t,w) = g(band) Ntrue(t,w)  
    # E[ F_true(t,w) ] = g(band) sqrt( Ntrue(t,w) )
    # EF_true(t,w) = g(band) sqrt( Ntrue(t,w) ) 
    #              = g(band) sqrt( F_true(t,w) / g(band) )
    #              = sqrt( g(band) F_true(t,w))
    #      where g represents unit and instrumental conversion, assumed constant over time 
    #      our estimate of the true measured flux is F_meas(t,w) = g(band) Nmeas(t,w), where Nmeas(t,w) follows a Poisson distribution with number of events Ntrue(t,w)
    #      if enough photons are measured the Poisson distribution on Nmeas(t,w) and thus F_meas(t,w) can be approximated by a normal distribution with mean F_true(t,w) and standard deviation EF_true(t,w) 
    ...
    # + low-frequency components are assumed constant over an order and are not aligned / resampled (eg detector gain, global flux scaling, flux balance and light curve scaling)
    ...
    # Fsc(w,t,v) = LC_theo(band,t,v)*Fcorr(w,t,v)/globF(t,v)    
    ...
    #      the measured spectrum is the one read from the input files of the instruments DRS, which links the counts and flux spectrum as:
    # F_meas(w,t,v) = gdet(band,v)*N_meas(w,t,v) 
    #      where gdet(band,v) = g(band)*Cdet(band,v) 
    #            g converts counts in flux and is assumed constant for a given instrument
    #            Cdet are the detector corrections for the flat field and blaze, assumed constant for a visit and with low-frequency variations
    #      thus
    # Fsc(w,t,v) = LC_theo(band,t,v)*gdet(band,v)*N_meas(w,t,v)*Ccorr(w,t,v)/globF(t,v) 
    # or
    # Nsc(w,t,v) = LC_theo(band,t,v)*N_meas(w,t,v)*Ccorr(w,t,v)/globF(t,v) 
    #    - as explained above we want to use as weights the errors on the true measured flux, ie : 
    # EFsc_true(w,t,v) = E( LC_theo(band,t,v)*gdet(band,v)*N_true(w,t,v)*Ccorr(w,t,v)/globF(t,v) )
    #                  = LC_theo(band,t,v)*gdet(band,v)*Ccorr(w,t,v)*E( N_true(w,t,v)/globF(t,v) )
    #                  = LC_theo(band,t,v)*gdet(band,v)*Ccorr(w,t,v)*sqrt( N_true(w,t,v) )/globF(t,v)
    #      since the master spectrum is measured with a high SNR we can assume MFstar(w,v) ~ MFstar_true(w,v)     
    # Fsc_true(w,t,v) = LC_theo(band,t,v)*MFstar_true(w,v) 
    #                 ~ LC_theo(band,t,v)*MFstar(w,v)      
    # gdet(band,v)*Nsc_true(w,t,v) = LC_theo(band,t,v)*MFstar(w,v)    
    # gdet(band,v)*LC_theo(band,t,v)*N_true(w,t,v)*Ccorr(w,t,v)/globF(t,v) = LC_theo(band,t,v)*MFstar(w,v)  
    # N_true(w,t,v) = MFstar(w,v)*globF(t,v)*dt/(Ccorr(w,t,v)*gdet(band,v))  
    #     and the corresponding error on the scaled flux writes as: 
    # EFsc_true(w,t,v) = LC_theo(band,t,v)*gdet(band,v)*Ccorr(w,t,v)*sqrt( MFstar(w,v)*globF(t,v)/(Ccorr(w,t,v)*gdet(band,v))  )/globF(t,v)
    #                  = LC_theo(band,t,v)*sqrt(gdet(band,v)*Ccorr(w,t,v)*MFstar(w,v)*dt/globF(t,v)  )   
    #      we note that the spectra Fsc_true(w,t,v)/LC_theo(band,t,v) have the same flux density profile as the master spectrum of the unocculted star MFstar_true(w,v), but the latter 
    # is measured during dt_ref over a much larger number of photons than an exposure spectrum. We should thus rescale the master spectrum by dt/dt_ref to get a number of photons comparable to that of the exposure, 
    # however we can ignore dt_ref, since it will be common to all exposures and be removed when normalizing the weight profiles over all exposures
    #      we also note that we neglected the differences between the Fsc_true and MFstar profiles that are due to the occulted local stellar lines and planetary atmospheric lines
    #    - the flux gain and detector correction are not necessarily available for all instruments
    #      we thus use the input flux and error tables to estimate gdet(band,v) 
    #      the flux density in a given pixel verifies:
    # F_true(w,t,v) = gdet(w,v)*N_true(w,t,v)
    # EF_true(w,t,v) = gdet(w,v)*sqrt(N_true(w,t,v))
    #      summing the flux densities over a large band yields 
    # TF_meas(t,v) ~ TF_true(t,v) = sum(w, F_true(w,t,v) ) = gdet(band,v)*sum(w over band, N_true(w,t,v) ) 
    #      thus
    # ETF_meas(t,v) ~ E[ TF_true(t,v) ]
    #               = E[ gdet(band,v)*sum(w over band, N_true(w,t,v) ) ]
    #               = gdet(band,v)*E[ sum(w over band, N_true(w,t,v) ) ]
    #               = gdet(band,v)*sqrt( sum(w over band, E[ N_true(w,t,v) )]^2 )
    #               = gdet(band,v)*sqrt( sum(w over band, N_true(w,t,v) ) )  
    #      and thus 
    # gdet(band,v) = TF_meas(t,v)/sum(w over band, N_true(w,t,v) )
    # gdet(band,v) = TF_meas(t,v)*gdet(band,v)^2/ETF_meas(t,v)^2
    # gdet(band,v) = ETF_meas(t,v)^2 / TF_meas(t,v)
    #      we estimate gdet over each order for S2D and the full spectral range for S1D
    #    - final weights are defined as:    
    # weight(w,t,v) = 1/EFsc_true(w,t,v)^2  
    #               = 1/(LC_theo(band,t,v)^2*gdet(band,v)*Ccorr(w,t,v)*MFstar(w,v)/globF(t,v))  
    #    - spectra must be defined on the same spectral table to be averaged in time/space
    #      in the star rest frame, spectral features from the disk-integrated spectrum remain fixed over time , ie that a given pixel see no variations in flux from MFstar(w,v) in a given visit and the contribution of the master spectrum can be ignored
    #    - even if the gain remains constant in time and does not contribute to the weighing of disk-integrated spectra, it must always be set to a value consistent with error definitions in the pipeline,
    # so that EFsc_true is comparable with other error tables in the weighing of intrinsic and planetary profiles
    #      gain profiles are thus always estimated by default by the pipeline, either from the input error tables when available or from the imposed error table for consistency    
    #--------------------------------------------------------     
    ...
    # > detector gain: uploaded from 'gdet_inputs' for every type of data
    #   for original 2D or 1D spectra, it returns the estimated spectral gain profiles for each exposure 
    #   for original CCFs or after conversion into CCFs or from 2D/1D it returns a global gain
    #   if original 2D or 1D spectra were converted back into count-equivalent values and are still in their original format, gain profiles are rescaled by the mean gain profile over the visit
    ...
    def calc_EFsc2(flux_sc_ord,flux_ref_ord,spec_corr_ord):
        return (flux_sc_ord**2.)*flux_ref_ord*spec_corr_ord/glob_flux_sc
    ...
    if data_type=='DI': 
        ...
        # Fintr_estimated(w,t,v) = (Fstar(w,v) - F(w,t,v))/(1 - LC_theo(band,t))
        #                        = (Fstar(w,v) - (Fstar(w,t,v) - Fintr_meas(w,t,v)))/(1 - LC_theo(band,t))
        #                        = (dFstar(w,t,v) + Fintr_meas(w,t,v)))/(1 - LC_theo(band,t))
        #      if Fstar(w,v) is a good estimate of Fstar(w,t,v) we will retrieve the correct intrinsic profile, but its uncertainties will be affected by the errors on Fstar(w,t,v) - which will dominate the weighing when binning the retrieved 
        ...
        #      local stellar profiles Fstar_loc_true can be defined in different ways (see def_local_profiles()). 
        #      we neglect their errors when models are used (mode_loc_data_corr = 'glob_mod', 'indiv_mod', 'rec_prof', 'theo') 
...
def def_masks(vis_mode,gen_dic,data_type_gen,inst,vis,data_dic,plot_dic,system_param,data_prop):
    ...
    if (data_type_gen in ['DI','Intr']):
        ...
        if (plot_dic[data_type_gen+'mask_spectra']!=''):
            dic_sav['cont_func_dic'] = cont_func_dic[0]            
        ...
        #Create data columns
        c1 = pyfits.Column(name='lambda', array=DRS_waves, format='1D')
        c2 = pyfits.Column(name='contrast', array=mask_weights, format='1E')
        t = pyfits.BinTableHDU.from_columns([c1, c2])
        ...
        p = pyfits.PrimaryHDU(header=hdr)
        
        #Saving
        hdulist = pyfits.HDUList([p,t])
    ...
    if (plot_dic[data_type_gen+'mask_spectra']!='') | (plot_dic[data_type_gen+'mask_ld_lw']!='') | (plot_dic[data_type_gen+'mask_RVdev_fit']!=''):
        np.savez_compressed(save_data_paths+'Plot_info',data=dic_sav,allow_pickle=True)
...
'''
def fit_prof_func(inst,data_dic,vis,fit_prop_dic,gen_dic,verbose,cond_def_fit,cond_def_cont,CCF_type,  
                  rv_edge_bins,rv_cen_bins,flux_loc,cov_loc,idx_force_det,theo_dic,star_params,fit_properties,priors_CCF,model_choice,model_prop): 
    
    #Arguments to be passed to the fit function
    fixed_args = deepcopy(fit_properties)

    #Fit tables 
    #    - to avoid issues with gaps and resampling/convolution, the model is calculated on the full continuous velocity table, and then limited to fitted pixels
    #    - fitted profiles can be trimmed to avoid computing models over a too-wide table
    if np.sum(cond_def_fit)==0.:stop('No bin in fitted range')
    rv_cen_bins_fit=rv_cen_bins[cond_def_fit]
    flux_loc_fit=flux_loc[cond_def_fit]
    cov_loc_fit=cov_loc[:,cond_def_fit] 
    fixed_args['ncen_bins'] = len(rv_cen_bins)
    fixed_args['cen_bins']=rv_cen_bins
    ...
    fixed_args['cond_def_fit']=cond_def_fit   
    fixed_args['x_val']=rv_cen_bins_fit
    fixed_args['y_val']=flux_loc_fit
    fixed_args['cov_val'] = cov_loc_fit
    output_prop_dic={}
    
    #Oversampling of model 
    if fixed_args['rv_osamp_line_mod'] is not None:

        #Oversampled table
        min_x = fixed_args['edge_bins'][0]
        max_x = fixed_args['edge_bins'][-1]
        nx_HR=np.ceil(round((max_x-min_x)/fixed_args['rv_osamp_line_mod']))
        dx_HR=(max_x-min_x)/nx_HR
        fixed_args['edge_bins_HR'] = min_x + dx_HR*np.arange(nx_HR+1)        
        fixed_args['cen_bins_HR'] = 0.5*(fixed_args['edge_bins_HR'][1::]+fixed_args['edge_bins_HR'][0:-1])
    
    #Model is convolved with instrumental response before comparison with observations
    if fixed_args['conv_model']:fixed_args['FWHM_inst'] = return_FWHM_inst(inst,c_light)
    ...
    if (CCF_type=='Intr'):
        ...
        if ('FWHM' in model_prop):
            cond_peak=(rv_cen_bins_fit>-0.5*model_prop['FWHM'][inst][vis]['guess']) & (rv_cen_bins_fit<0.5*model_prop['FWHM'][inst][vis]['guess'])
    ...
    #Estimate from measured CCF peak
        ...
        if CCF_peak==0.:RV_guess_tab[0]=np.mean(rv_cen_bins_fit)
        else:RV_guess_tab[0]=rv_cen_bins_fit[flux_loc_fit==CCF_peak][0]
    ...
    #-----------------------------------
    if (CCF_type in ['DI','Intr']): 
        idx_low=np.where(rv_cen_bins_fit < RV_guess_tab[0])[0]
        if len(idx_low)==0:vlow_FWHM=rv_cen_bins_fit[0]
        else:
            ilow_FWHM=closest(flux_loc_fit[idx_low],0.5*(CCF_peak+fixed_args['flux_cont']))
            vlow_FWHM=rv_cen_bins_fit[idx_low][ilow_FWHM]
        idx_high=np.where(rv_cen_bins_fit >= RV_guess_tab[0])[0]
        if len(idx_high)==0:vhigh_FWHM=rv_cen_bins_fit[-1]
        else:
            ihigh_FWHM=closest(flux_loc_fit[idx_high],0.5*(CCF_peak+fixed_args['flux_cont']))
            vhigh_FWHM=rv_cen_bins_fit[idx_high][ihigh_FWHM]     
    ...
    if CCF_type in ['DI','Intr']:fixed_args['amp_sign']=-1.
    elif CCF_type=='Atm':fixed_args['amp_sign']=1.
    ...
    if (model_choice in ['gauss','voigt']):
        ...
        p_start.add_many(('ctrst',   ctrst_guess,   True,   0.,     1. ,        None),
                         ('rv',      RV_guess_tab[0],      True,   RV_guess_tab[1], RV_guess_tab[2],     None),               
                         ('FWHM',    FWHM_guess,    True,   0.,     FWHM_max,   None),
                         ('skewA',   0.0,           False,   -1e3,1e3,None),
                         ('kurtA',   0.0,           False,   -100.,100.,None),
                         ('slope',   0.0,           False,   None,None,None),
                         ('cont',    fixed_args['flux_cont'], False,   None,None,None))        
    ...
    #    - polynomial is made continuous in value and derivative, at a velocity value symmetric with respect to the centroid RV
        ...
        p_start.add_many( ( 'ctrst',        ctrst_guess,    True,   0.,     1. ,        None),
                          ( 'rv',           RV_guess_tab[0],       True,   RV_guess_tab[1], RV_guess_tab[2],     None),   
                          ( 'FWHM',         FWHM_guess,     True,   0.,     FWHM_max,   None),
                          ( 'cont',         fixed_args['flux_cont'],  False,  0.,None,  None),
                          ( 'dRV_joint',    dRV_joint,      True,   0.,None,  None),
    ...
    #    - positions, widths and contrast are linked 
        ...
        p_start.add_many(( 'ctrst',         ctrst_guess,        True,   0.,     1. ,        None),
                         ...
                         ( 'cont',          fixed_args['flux_cont'],      False,  0.,None,  None),
                         ( 'slope',         0.0,                False,  None,None,  None),
                         ( 'rv_l2c',        rv_l2c,             True ,  None ,  None,  None), 
    ...
    elif (model_choice=='custom') and (CCF_type=='DI'):
        
        #Grid initialization
        fixed_args,p_start = init_custom_DI_par(fixed_args,gen_dic,data_dic['DI']['system_prop'] ,theo_dic,star_params,p_start,RV_guess_tab,True)
    ...
    p_start = par_formatting(p_start,model_prop,priors_CCF,fit_dic,fixed_args,inst,vis)
    ...
    #Fit by emcmc 
        ...
        #Reuse MCMC
            ...
            walker_chains=np.load(fit_dic['save_dir']+'/raw_chains_walk'+np.str(fit_prop_dic['mcmc_set']['nwalkers'][inst][vis])+'_steps'+np.str(fit_prop_dic['mcmc_set']['nsteps'][inst][vis])+fit_dic['run_name']+'.npz')['walker_chains']  
    ...
    output_prop_dic['rv_fit']=fixed_args['x_val']
    output_prop_dic['CCF_fit']=fixed_args['y_val']
    output_prop_dic['covCCF_fit']=fixed_args['cov_val']
 
    #Best-fit model to the full profile at the observed resolution
    output_prop_dic['edge_bins'] = rv_edge_bins
    output_prop_dic['cen_bins'] = rv_cen_bins
    fixed_args['cond_def_fit']=np.repeat(True,fixed_args['ncen_bins'])    
    output_prop_dic['cond_def'] = np.ones(len(rv_cen_bins),dtype=bool)
    ...
    if (model_choice=='dgauss'):
        output_prop_dic['gauss_core'],output_prop_dic['gauss_lobe']=fixed_args['fit_func_gen'](p_final,rv_cen_bins)[1:3] 
        if fixed_args['FWHM_inst'] is not None:
            output_prop_dic['gauss_core'] = convol_prof(output_prop_dic['gauss_core'],rv_cen_bins,fixed_args['FWHM_inst'])  
            output_prop_dic['gauss_lobe'] = convol_prof(output_prop_dic['gauss_lobe'],rv_cen_bins,fixed_args['FWHM_inst']) 

    #Gaussian + polynomial model: output the two components
    elif (model_choice=='gauss_poly'):
        output_prop_dic['gauss_core'],output_prop_dic['poly_lobe']=fixed_args['fit_func_gen'](p_final,rv_cen_bins)[1:3] 
        if fixed_args['FWHM_inst'] is not None:
            output_prop_dic['gauss_core'] = convol_prof(output_prop_dic['gauss_core'],rv_cen_bins,fixed_args['FWHM_inst'])  
            output_prop_dic['poly_lobe'] = convol_prof(output_prop_dic['poly_lobe'],rv_cen_bins,fixed_args['FWHM_inst']) 
            
    #Custom model: output the line without continuum
    elif (model_choice=='custom'):
        output_prop_dic['core'],output_prop_dic['core_norm']=fixed_args['fit_func_gen'](p_final,rv_cen_bins,args=fixed_args)[1:3]
        if fixed_args['FWHM_inst'] is not None:
            output_prop_dic['core'] = convol_prof(output_prop_dic['core'],rv_cen_bins,fixed_args['FWHM_inst'])  
            output_prop_dic['core_norm'] = convol_prof(output_prop_dic['core_norm'],rv_cen_bins,fixed_args['FWHM_inst']) 

    #Best-fit model to the full line profile at high resolution, including instrumental convolution if requested
    fixed_args['ncen_bins'] =  int((gen_dic['RVend_HR_mod']-gen_dic['RVstart_HR_mod'])/gen_dic['dRV_HR_mod']) 
    fixed_args['cen_bins'] = gen_dic['RVstart_HR_mod'] + gen_dic['dRV_HR_mod']*np.arange( fixed_args['ncen_bins'] )
    output_prop_dic['vel_HR']=fixed_args['cen_bins']  
    fixed_args['dcen_bins'] = fixed_args['edge_bins'][1::]-fixed_args['edge_bins'][0:-1] 
    fixed_args['cond_def_fit']=np.repeat(True,fixed_args['ncen_bins']) 
    if (model_choice=='custom'):
        if ('spec' in data_dic[inst][vis]['type']) and (fixed_args['mode']=='glob_mod') and (data_dic['DI']['line_trans'] is not None):
            fixed_args['cen_bins_sp'] = data_dic['DI']['line_trans']*(1.+(fixed_args['velccf_HR']/c_light))
        theo_intr2loc(theo_dic,data_dic['DI']['system_prop'],fixed_args,fixed_args['ncen_bins'],theo_dic['nsub_star'])        
    output_prop_dic['mod_HR']=fixed_args['fit_func'](p_final,None,args=fixed_args)
        ...
        if (CCF_type in ['DI','Intr']):
            ...
            if (model_choice in ['dgauss','custom']):
                if any([par_name in fit_prop_dic['deriv_prop'] for par_name in ['true_ctrst','true_FWHM','true_amp']]):
                    if fit_dic['fit_mod'] in ['chi2','']:
                        p_final['true_ctrst'],p_final['true_FWHM'],p_final['true_amp'] = cust_mod_true_prop(p_final,output_prop_dic['vel_HR'],fixed_args)  
                        if fit_dic['fit_mod']=='chi2': 
                            sig_loc=np.nan
                            fit_dic['sig_parfinal_err']['1s']= np.hstack((fit_dic['sig_parfinal_err']['1s'],[[sig_loc],[sig_loc]])) 
                            fit_dic['sig_parfinal_err']['1s']= np.hstack((fit_dic['sig_parfinal_err']['1s'],[[sig_loc],[sig_loc]])) 
                        ...
                        p_final_loc=deepcopy(p_final)
                        fixed_args['vel_HR'] = output_prop_dic['vel_HR']
    ...
    #    - Area criterion 
        ...
        if ('crit_amp' in output_prop_dic) and ('crit_area' in output_prop_dic):  
            ...
            output_prop_dic['detected'] = False
            ...
            if ('_ord' in par):
                ...
                for inst_loc in inst_list:
                    ...
                    for vis_loc in vis_list:
                        ...
                        if gen_root_par in ['ctrst','FWHM','amp_l2c','rv_l2c','FWHM_l2c','a_damp','slope']:
                            ...
                            fixed_args['linevar_par'][inst_loc][vis_loc]+=[gen_root_par]                     
...
def gen_over_func(param_in,func_nam,args):
    ...
    #No oversampling
    if args['rv_osamp_line_mod'] is None:
        mod_prof = func_nam(param,args['cen_bins'],args)[0]
    
    #Oversampling each bin  
    else:    

        #HR profile 
        mod_prof_HR = func_nam(param,args['cen_bins_HR'],args)[0]
            
        #Resampling on final table
        mod_prof = bind.resampling(args['edge_bins'],args['edge_bins_HR'],mod_prof_HR, kind=args['resamp_mode'])  
    
    #Add offset if required
    if ('offset' in param):mod_prof+=param['offset']
        
    #Instrumental convolution 
    if args['FWHM_inst'] is not None:
        mod_prof = convol_prof(mod_prof,args['cen_bins'],args['FWHM_inst'])                 

    #Limiting table to fitted pixels
    mod_prof = mod_prof[args['cond_def_fit']]
...
def convol_prof(prof_in,cen_bins,FWHM):
    ...
    hnkern=np.int(np.ceil(2.125*FWHM/dbins)+1)
...
def gauss_herm_lin(param,x,args=None):
    ...
    #Continuum        
    cont_pol = param['cont']*( 1 + param['slope']*x)
...
def proc_cust_mod_true_prop(merged_chain_proc,args,param_loc):
    ...
    for istep in range(nsamp): 
        ...
        true_ctrst_step,true_FWHM_step,true_amp_step=cust_mod_true_prop(param_proc,args['vel_HR'],args)  
...
def calc_loc_line_prof(icell,rv_shift,Fsurf_cell_spec,flux_loc_cell,mu_cell,args,param):
    ...
    #    - see align_data for details 
        ...
        if ('spec' in args['type']):edge_bins_rest = args['edge_bins_intr']*np.sqrt(1. - (rv_shift/c_light) )/np.sqrt(1. + (rv_shift/c_light) ) 
...
def def_st_prof_tab(inst,vis,isub,args):
    ...
    for key in ['edge_bins','cen_bins','dcen_bins']:
        args_exp[key] = args[key+suff][inst][vis][isub]
    for key in ['ncen_bins','dim_exp']:
        args_exp[key] = args[key+suff][inst][vis]
...
Activation of spectral conversion and resampling 
'''
def cond_conv_st_prof_tab(gen_dic,fixed_args,data_type):
    
    #Spectral oversampling
    if gen_dic['rv_osamp_line_mod'] is not None:fixed_args['resamp'] = True
    ...
    if ('spec' in data_type) and (fixed_args['mode']=='glob_mod'):
...
'''
Definition of spectral table for model line profile
'''
def model_st_prof_tab(inst,vis,isub,fixed_args,gen_dic,nexp):
    
    #Resampled model table
    #    - defined in RV space if relevant for spectral data
    if fixed_args['resamp']:
        rv_resamp = deepcopy(gen_dic['rv_osamp_line_mod'])
        if fixed_args['spec2rv']:
            edge_bins_RV = c_light*((fixed_args['edge_bins'][inst][vis][isub]/fixed_args['line_trans']) - 1.) 
            if rv_resamp is None:rv_resamp = np.mean(edge_bins_RV[1::]-edge_bins_RV[0:-1]    )
            min_x = edge_bins_RV[0]
            max_x = edge_bins_RV[-1]    
        else:
            min_x = fixed_args['edge_bins'][inst][vis][isub][0]
            max_x = fixed_args['edge_bins'][inst][vis][isub][-1]            
        ...
        if 'ncen_bins_HR' not in fixed_args:
            fixed_args['ncen_bins_HR']={inst:{vis:int(np.ceil(round((max_x-min_x)/rv_resamp)))}}
        dx_HR=(max_x-min_x)/fixed_args['ncen_bins_HR'][inst][vis]
        fixed_args['edge_bins_HR'][inst][vis][isub] = min_x + dx_HR*np.arange(fixed_args['ncen_bins_HR'][inst][vis]+1)        
        fixed_args['cen_bins_HR'][inst][vis][isub] = 0.5*(fixed_args['edge_bins_HR'][inst][vis][isub][1::]+fixed_args['edge_bins_HR'][inst][vis][isub][0:-1])   
        fixed_args['dcen_bins_HR'][inst][vis][isub] = fixed_args['edge_bins_HR'][inst][vis][isub][1::]-fixed_args['edge_bins_HR'][inst][vis][isub][0:-1]                      
        if 'dim_exp_HR' not in fixed_args:
            fixed_args['dim_exp_HR']={inst:{vis:fixed_args['dim_exp'][inst][vis]}} 
            fixed_args['dim_exp_HR'][inst][vis][1] = fixed_args['ncen_bins_HR'][inst][vis]
...
Reference point for instrumental convolution
'''
def ref_inst_convol(fixed_args,cen_bins):
    if (fixed_args['mode']=='glob_mod') or fixed_args['spec2rv']:fixed_args['ref_conv'] = c_light
...
def conv_st_prof_tab(inst,vis,isub,args,args_exp,line_mod_in,FWHM_inst):
    ...
    if args['spec2rv']:args_exp['edge_bins'] = args['line_trans']*(1.+(args_exp['edge_bins']/c_light))

    #Resample model on observed table if oversampling
    if args['resamp']:line_mod_out = bind.resampling(args['edge_bins'][inst][vis][isub],args_exp['edge_bins'],line_mod_out, kind=args['resamp_mode'])       
...
'''
def init_custom_DI_par(fixed_args,gen_dic,system_prop,theo_dic,star_params,params,RV_guess_tab):  

    #Variable stellar grid
    #    - all stellar properties are initialized to default stellar values
    #      those defined as variable properties through the settings will be overwritten 
    ...
    if fixed_args['DI_grid']:
        params.add_many('LD_u1',            system_prop['achrom']['LD_u1'][0],                False ,   None,None,  None)
        params.add_many('LD_u2',            system_prop['achrom']['LD_u2'][0],                False ,   None,None,  None) 
        params.add_many('slope',            0.,                                               False,    None,None,  None)
        for ideg in range(4):params.add_many('c'+str(ideg)+'_pol',          0.,              False,    None,None,None)  
...
'''
def init_custom_DI_prof(fixed_args,gen_dic,system_prop,theo_dic,star_params,params):                  
    fixed_args['resamp_mode'] = gen_dic['resamp_mode'] 
    fixed_args['system_prop'] = deepcopy(system_prop)
    fixed_args['nsub_star'] = theo_dic['nsub_star']     
 
    #Theoretical intrinsic profile
    ...
    elif fixed_args['mode']=='glob_mod':  
        fixed_args['cov_loc_star']=False 
        fixed_args['func_prof'] = dispatch_func_prof(fixed_args['func_prof_name'])

    #---------------------------------

    #Fixed stellar grid
    if (not fixed_args['fit_star_grid']):
        init_st_loc_prof(fixed_args,theo_dic,params)       

    #Variable stellar grid
    #    - only achromatic intensity variations are fitted
    else:
        fixed_args['nsub_Dstar'] = theo_dic['nsub_Dstar']
...
'''
Functions to initialize intrinsic stellar profiles
'''
def init_st_loc_prof(args,grid_dic,param):

    #Store stellar grid
    for key in ['x_st_sky','y_st_sky','z_st_sky','x_st','y_st','z_st','mu']:args[key]=grid_dic[key]
   
    #Attribute intrinsic profile to each cell 
    init_st_intr_prof(args,grid_dic,param)

    #Scaling intrinsic profiles -> local profiles
    #    - broadband intensity profiles must be defined over the input table in spectral mode, if relevant
    theo_intr2loc(grid_dic,args['system_prop'],args,args['ncen_bins'],grid_dic['nsub_star'])     
...
def init_st_intr_prof(args,grid_dic,param):
    ...
    #Analytic intrinsic profile
    #    - calculating for each stellar cell the properties defining analytical profiles
    if args['mode']=='glob_mod': 
        args['flux_intr_grid'] = np.zeros(grid_dic['nsub_star'])*np.nan   
        linevar_coord_grid = calc_linevar_coord_grid(args['coord_line'],grid_dic)
        ...
        for par_loc in args['linevar_par'][args['inst']][args['vis']]:     
            ...
            args['input_cell_all'][par_loc] = calc_polymodu(args['pol_mode'],args['coeff_line'][par_loc],linevar_coord_grid) 
   
    #Theoretical intrinsic profiles
    #    - interpolating profiles defined over mu grid over full mu grid
    elif args['mode']=='theo':
        args['flux_intr_grid'] = args['sme_grid']['flux_intr_grid'](grid_dic['mu'])
...
def custom_DI_prof(param,x,args=None):
    ...
    #--------------------------------------------------------------------------------
    #Recalculating stellar grid
    #--------------------------------------------------------------------------------
    if args['fit_star_grid']:
        
        #Update potentially variable stellar properties
        args['system_prop']['achrom'].update({'LD_u1':[param['LD_u1']],'LD_u2':[param['LD_u2']]}) 
        args['star_params'].update({
            'f_GD':param['f_GD'],'RpoleReq':1.-param['f_GD'],'beta_GD':param['beta_GD'],'Tpole':param['Tpole'],
            'istar_rad':np.arccos(param['cos_istar']),'om_eq' : param['veq']/args['star_params']['Rstar_km']})

        #Generate stellar grid
        grid_dic={}
        model_star('grid',grid_dic,[args['type']],args['system_prop'],args['nsub_Dstar'],args['star_params'])

        #Initialize local stellar profiles
        init_st_loc_prof(args,grid_dic,param)
        
    #--------------------------------------------------------------------------------
    #Retrieving properties required for the calculation of local line profiles
    #--------------------------------------------------------------------------------
    ...
    rv_shift_grid = calc_RVrot(args['x_st_sky'],args['y_st'],np.arccos(param['cos_istar']),param) + param['rv']
    ...
    if np.max(cb_band)>0.:rv_shift_grid += np_poly(cb_band)(args['mu']).flatten()

    #--------------------------------------------------------------------------------        
    #Coadding local line profiles over stellar disk
    #--------------------------------------------------------------------------------
    icell_list = np.arange(args['nsub_star'])
    if args['nthreads']>1:flux_DI_sum=para_coadd_loc_line_prof(coadd_loc_line_prof,args['nthreads'],args['nsub_star'],[rv_shift_grid,icell_list,args['Fsurf_grid_spec'],args['flux_intr_grid'],args['mu']],(param,args,))                           
    else:                 flux_DI_sum=coadd_loc_line_prof(rv_shift_grid,icell_list,args['Fsurf_grid_spec'],args['flux_intr_grid'],args['mu'],param,args)
    DI_flux_norm = np.sum(flux_DI_sum,axis=0)

    #Scaling disk-integrated profile to requested continuum
    #    - returned by the function normalized to unity  
    DI_flux_cont = param['cont']*DI_flux_norm

    #Polynomial continuum level
    #    - P(x) = cont*(1 + a1*rv + a2*rv^2 + a3*rv^3 ... )
    DI_flux_mod=DI_flux_cont*(1. + param['c1_pol']*args['cen_bins'] + param['c2_pol']*args['cen_bins']**2.  + param['c3_pol']*args['cen_bins']**3. + param['c4_pol']*args['cen_bins']**4. ) 
...
'''
def sub_calc_plocc_prop(key_chrom,args,par_list_in,transit_pl,system_param,theo_dic,system_prop_in,param,coord_pl_in,iexp_list,out_ranges=False,Ftot_star=False):
    system_prop = deepcopy(system_prop_in)
    n_exp = len(iexp_list)
    
    #Line profile initialization
    if ('line_prof' in par_list_in):
        par_list_in+=['rv','mu','SpSstar']
        par_list_in+=args['linevar_par'][args['inst']][args['vis']]
        ...
        if key_chrom==['chrom']:
            ...
            #    - requires calculation of achromatic properties                
                ...
                if (args['mode']=='glob_mod'):stop('Analytical model not suited for wide spectral bands') 
    ...
    if 'line_prof' in par_list_in:
        for subkey_chrom in key_chrom:surf_prop_dic[subkey_chrom]['line_prof']=np.zeros([args['ncen_bins'],n_exp])
    ...
    if ('rv' in par_list_in):par_list+=['rv']
    ...
    for ipl,pl_loc in enumerate(transit_pl):
        ...
        if np.sum(np.abs(coord_pl_in[pl_loc]['ecl'][iexp_list])!=1.)>0:
            ...
            #Planet-dependant properties
            for subkey_chrom in key_chrom:
                surf_prop_dic[subkey_chrom][pl_loc]={}
                for par_loc in par_list:surf_prop_dic[subkey_chrom][pl_loc][par_loc]=np.zeros([system_prop[subkey_chrom]['nw'],n_exp])*np.nan        
        ...
        if cond_iexp_proc[i_in]:        
            ...
            for iosamp in range(n_osamp_exp):
                ...
                for pl_loc in transit_pl_exp:   
                    ...
                    for subkey_chrom in key_chrom:
                        for iband in range(system_prop[subkey_chrom]['nw']):
                            Focc_star[subkey_chrom][iband],cond_occ = calc_occ_region_prop(line_occ_HP[subkey_chrom][iband],cond_occ,iband,args,system_prop[subkey_chrom],iosamp,pl_loc,pl_proc[subkey_chrom][iband],theo_dic['Ssub_Sstar_pl'][pl_loc],x_st_sky_max,y_st_sky_max,system_prop[subkey_chrom]['cond_in_RpRs'][pl_loc][iband],par_list,param,theo_dic['Istar_norm_'+subkey_chrom],\
                                                                                  x_oversamp_pl,y_oversamp_pl,lambda_rad_pl,par_star,sum_prop_dic[subkey_chrom][pl_loc],coord_reg_dic[subkey_chrom][pl_loc],range_dic[subkey_chrom][pl_loc],range_par_list,Focc_star[subkey_chrom][iband],line_occ_HP[subkey_chrom][iband],star_params,cb_band_dic[subkey_chrom][iband],theo_dic)
                ...
                if cond_occ and ('line_prof' in par_list_in) and  (theo_dic['precision']=='medium'):
                    ...
                    surf_prop_dic[key_chrom[-1]]['line_prof'][:,i_in]+=intr2plocc_prof(args,transit_pl_exp,coord_reg_dic,idx_w,system_prop,key_chrom,param)
            ...
            if ('line_prof' in par_list_in):
                ...
                if (theo_dic['precision']=='low'): 
                    ...
                    surf_prop_dic[key_chrom[-1]]['line_prof'][:,i_in]=intr2plocc_prof(args,transit_pl_exp,surf_prop_dic,idx_w,system_prop,key_chrom,param)
                ...
                #    - the weighing by stellar intensity is naturally included when applying flux scaling 
                    ...
                    if (theo_dic['precision']=='high') and args['conv2intr']:
                        surf_prop_dic[key_chrom[-1]]['line_prof'][:,i_in] = np.sum(args['dcen_bins']*surf_prop_dic[key_chrom[-1]]['line_prof'][:,i_in])/np.sum(args['dcen_bins'])
...
'''
Calculation of planet-occulted line profile from theoretical intrinsic profiles
    - profiles can be theoretical (imported/calculated with an atmospheric model) or calculated analytically (in RV or wavelength space, but over a single line) 
...
'''
def intr2plocc_prof(args,transit_pl,coord_dic,idx_w,system_prop,key_chrom,param):
    data_av_pl={'cen_bins':[args['cen_bins']],'edge_bins':[args['edge_bins']]}
    ...
    for pl_loc in transit_pl:  
        ...
        if ~np.isnan(coord_dic[chrom_calc][pl_loc]['Ftot'][idx_w[chrom_calc]]):
            ...
            #Theoretical or measured profile
                ...
                data_av_pl=align_data(data_av_pl,args['type'],args['nord'],args['dim_exp'],args['resamp_mode'],data_av_pl['cen_bins'],data_av_pl['edge_bins'],-surf_shifts,rv_shift_edge = surf_shifts_edge, nocov = True)
            ...
            if coord_dic[chrom_calc][pl_loc]['rv_broad'][idx_w[chrom_calc]][0]>0.:
                data_av_pl['flux'][0] = convol_prof(data_av_pl['flux'][0],args['cen_bins'],coord_dic[chrom_calc][pl_loc]['rv_broad'][idx_w[chrom_calc]][0])  
...
def calc_occ_region_prop(line_occ_HP_band,cond_occ,iband,args,system_prop,idx,pl_loc,pl_proc_band,Ssub_Sstar,x_st_sky_max,y_st_sky_max,cond_in_RpRs,par_list,param,Istar_norm_band,x_pos_pl,y_pos_pl,lambda_rad_pl,par_star,sum_prop_dic_pl,\
                         coord_reg_dic_pl,range_reg_pl,range_par_list,Focc_star_band,line_occ_HP,star_params,cb_band,theo_dic):
    ...
    cond_in_stphot,n_pl_occ = calc_st_sky(coord_grid,star_params)
    ...
    if n_pl_occ>0:
        ...
        if len(pl_proc_band)>0:
            cond_pl_occ_corr = np.repeat(True,np.sum(cond_in_stphot))
        ...
        #Flux occulted from all planets
...
+ we add the values, even if each 'i_in' index is only updated once, so that the subroutine can be used directly with oversampling
...
def sum_region_prop(line_occ_HP_band,iband,args,system_prop,par_list,Fsurf_grid_band,coord_grid,Ssub_Sstar,cb_band,range_par_list,range_reg_pl,sum_prop_dic_pl,coord_reg_dic_pl,par_star,lambda_rad_pl_loc,theo_dic,param):
    ...
    for par_loc in par_list:
        ...
        if par_loc=='SpSstar':
            ...
            #Full RV (km/s)
            #    - accounting for an additional offset to model jitter
            elif par_loc=='rv':
            ...
            elif (line_occ_HP_band!='') and (par_loc in args['linevar_par'][args['inst']][args['vis']]):  
                coord_line_grid = calc_linevar_coord_grid(args['coord_line'],coord_grid)
                coord_grid[par_loc] = calc_polymodu(args['pol_mode'],args['coeff_line'][par_loc],coord_line_grid) 
            ...
            if par_loc=='xp_abs':coord_grid[par_loc+'_sum'] = np.abs(np.sum(coord_grid[par_loc]*Fsurf_grid_band))
            else:coord_grid[par_loc+'_sum'] = np.sum(coord_grid[par_loc]*Fsurf_grid_band)
...
#    - 'absolute' : coeff_pol[0]*x^n + coeff_pol[1]*x^(n-1) .. + coeff_pol[n]*x^0
#      'modulated' : (coeff_pol[0]*x^n + coeff_pol[1]*x^(n-1) .. + 1)*coeff_pol[n]
...
def calc_linevar_coord_grid(dim,grid):
    if (dim=='mu'):linevar_coord_grid = grid['mu']
    elif (dim=='xp_abs'):linevar_coord_grid = grid['xp_abs']
    elif (dim=='r_proj'):linevar_coord_grid = np.sqrt(grid['r_proj'])
    elif (dim=='y_st'):linevar_coord_grid = grid['y_st']
...
'''
Global call to functions defining local profiles associated with each observed exposure
'''
def def_local_profiles(inst,vis,gen_dic,data_dic,data_prop,coord_dic,system_param,theo_dic):

    print('   > Building estimates for local stellar profiles') 
    text_print={
        'DIbin':    '         > Using DI master',
        'Intrbin':  '         > Using binned intrinsic profiles',
        'glob_mod': '         > Using global line model',
        'indiv_mod':'         > Using individual line models',
        'rec_prof': '         > Using reconstruction',
        'theo':     '         > Using theoretical profiles',
    ...
    if (gen_dic['calc_loc_data_corr']):
        ...
        if data_dic['Intr']['mode_loc_data_corr'] in ['DIbin','Intrbin']:
            loc_prof_meas(data_dic['Intr']['mode_loc_data_corr'],inst,vis,data_dic['Intr'],gen_dic,data_dic,data_prop,data_dic['Atm'],coord_dic)
            
        #Using global CCF profile model
        elif data_dic['Intr']['mode_loc_data_corr']=='glob_mod': 
            loc_prof_globmod(inst,vis,gen_dic,data_dic,data_prop,system_param,theo_dic,coord_dic)
            
        #Using individual CCF profile models
        elif data_dic['Intr']['mode_loc_data_corr']=='indiv_mod': 
            loc_prof_indivCCFmod(inst,vis,gen_dic,data_dic)
        ...
        #Defining undefined pixels via a polynomial fit to defined pixels in complementary exposures, or via a 2D interpolation over complementary exposures and a narrow spectral band
            ...
            loc_prof_rec(inst,vis,gen_dic,data_dic,coord_dic)
            
        #Using theoretical profiles
        elif data_dic['Intr']['mode_loc_data_corr']=='theo': 
            loc_prof_theo()
    ...
    #Checking that local data has been calculated for all exposures
        ...
        data_paths={i_in:gen_dic['save_data_dir']+'Loc_estimates/'+data_dic['Intr']['mode_loc_data_corr']+'/'+inst+'_'+vis+'_in'+str(i_in) for i_in in idx_est_loc}
        check_data(data_paths)
...
def loc_prof_meas(corr_mode,inst,vis,gen_dic,data_dic,data_prop,coord_dic):
    ...
    for isub,i_in in enumerate(idx_aligned):    
        ...
        #Upload local stellar profile for current exposure to get its spectral tables
        data_loc_exp = np.load(data_vis['proc_Res_data_paths']+str(gen_dic[inst][vis]['idx_in2exp'][i_in])+'.npz',allow_pickle=True)['data'].item()    
        ...
        for iexp_off in idx_to_bin_all[i_bin]:    
            ...
            if corr_mode=='DIbin':
                ...
                if ('DI_Mast' in data_dic['Atm']['no_plrange']) and (iexp_bin in data_dic['Atm'][inst][vis_bin]['iexp_no_plrange']):
                    for iord in range(data_inst['nord']):
                        data_exp_bin['flux'][iord][excl_plrange(data_exp_bin['cond_def'][iord],data_dic['Atm'][inst][vis_bin]['exclu_rangestar'],iexp_bin,data_loc_exp['edge_bins'][iord],data_vis['type'])] = np.nan
            ...
            # will not create any correlations 
                ...
                data_exp_bin = dataload_npz(gen_dic['save_data_dir']+'Aligned_Intr_data/'+inst+'_'+vis_bin+'_in'+str(iexp_bin))
            ...
            #Weight profile
            data_to_bin[iexp_off]['weight'] = def_weights_spatiotemp_bin(range(data_inst['nord']),scaled_data_paths,inst,vis_bin,gen_dic['corr_Fbal'],gen_dic['corr_FbalOrd'],gen_dic['save_data_dir'],gen_dic['type'],data_inst['nord'],iexp_bin_glob,in_type,data_vis['type'],data_vis['dim_exp'],data_to_bin[iexp_off]['tell'],data_to_bin[iexp_off]['mean_gdet'],data_to_bin[iexp_off]['cen_bins'],data_ref_align['flux'],data_ref_align['cov'],bdband_flux_sc = gen_dic['flux_sc'])            
            
        #Calculating binned profile
        data_est_loc = calc_binned_prof(idx_to_bin_all[i_bin],data_dic[inst]['nord'],data_vis['dim_exp'],data_vis['nspec'],data_to_bin,data_dic,inst,n_in_bin_all[i_bin],data_loc_exp['cen_bins'],data_loc_exp['edge_bins'],dx_ov_in = dx_ov_all[i_bin])
        
        #Rescaling master-out to the level of the local profile
    ...
    np.savez_compressed(gen_dic['save_data_dir']+'Loc_estimates/'+data_dic['Intr']['mode_loc_data_corr']+'/'+inst+'_'+vis+'_add',data=data_add,allow_pickle=True)
...
'''
Sub-function for local profiles using global line profile model
...
'''
def loc_prof_globmod(inst,vis,gen_dic,data_dic,data_prop,system_param,theo_dic,coord_dic):
    data_vis = data_dic[inst][vis]
    opt_dic = data_dic['Intr']['opt_loc_data_corr']['glob_mod']
    if (data_dic['Intr']['align_mode']=='meas'):stop('This mode cannot be used with measured RVs')

    #Retrieving selected model properties
    if ('IntrProf_prop_path' not in opt_dic):opt_dic['IntrProf_prop_path']={inst:{vis:gen_dic['save_data_dir']+'/Joined_fits/Intr_prof/Fit_results.npz'}}
    data_prop = np.load(opt_dic['IntrProf_prop_path'][inst][vis],allow_pickle=True)['data'].item()  
    params=data_prop['p_final'] 
    fixed_args={}
    for key in ['func_prof_name','coeff_ord2name','coeff_ord2name','pol_mode']:fixed_args[key] = data_prop[key]
    fixed_args.update({   
        'mac_mode':theo_dic['mac_mode'], 
        'fit_star_grid': False,
        'type':data_dic[inst][vis]['type'],
        'nord':data_dic[inst]['nord'],
        'nthreads': data_dic['Intr']['opt_loc_data_corr']['nthreads'],
        'resamp_mode' : gen_dic['resamp_mode'], 
        'conv2intr':False,
        'inst':inst,
        'vis':vis,  
    ...
    #Spectral oversampling and RV mode for analytical models of spectral profiles
    if (gen_dic['rv_osamp_line_mod'] is not None) or ('spec' in data_dic[inst][vis]['type']):fixed_args['resamp'] = True
    else:fixed_args['resamp'] = False
    
    #Reference point for instrumental convolution
    fixed_args['ref_conv'] = c_light  

    #Processing in-transit exposures
    for isub,i_in in enumerate(range(data_dic[inst][vis]['n_in_tr'])):
      
        #Upload spectral tables of local stellar profile for current exposure
        data_loc_exp = np.load(data_vis['proc_Res_data_paths']+str(gen_dic[inst][vis]['idx_in2exp'][i_in])+'.npz',allow_pickle=True)['data'].item()    

        #Spectral table for model calculation
        fixed_args['cen_bins'] = {inst:{vis:{0:data_loc_exp['cen_bins'][0]}}}
        fixed_args['edge_bins']={inst:{vis:{0:data_loc_exp['edge_bins'][0]}}}
        fixed_args['ncen_bins']={inst:{vis:data_dic[inst][vis]['nspec']}}
        fixed_args['dcen_bins']={inst:{vis:{0:fixed_args['edge_bins'][inst][vis][0][1::] - fixed_args['edge_bins'][inst][vis][0][0:-1]       }}}
        fixed_args['dim_exp']={inst:{vis:data_dic[inst][vis]['dim_exp']}}

        #General options
        cond_conv_st_prof_tab(gen_dic,fixed_args,data_dic[inst][vis]['type']) 
        model_st_prof_tab(inst,vis,0,fixed_args,gen_dic,1)
        ref_inst_convol(fixed_args,fixed_args['cen_bins'][inst][vis][0])
        args_exp = def_st_prof_tab(inst,vis,0,fixed_args)      
        ...
        flux_loc = conv_st_prof_tab(inst,vis,0,fixed_args,args_exp,sp_line_model,return_FWHM_inst(inst,fixed_args['ref_conv']))

        #Saving estimate of local profile for current exposure                   
        np.savez_compressed(gen_dic['save_data_dir']+'Loc_estimates/'+data_dic['Intr']['mode_loc_data_corr']+'/'+inst+'_'+vis+'_in'+str(i_in),
                            data={'cen_bins':data_loc_exp['cen_bins'],'edge_bins':data_loc_exp['edge_bins'],'cond_def':np.ones(data_dic[inst][vis]['dim_exp'],dtype=bool),'flux' : np.array([flux_loc])},allow_pickle=True)

    #Saving complementary data
    np.savez_compressed(gen_dic['save_data_dir']+'Loc_estimates/'+data_dic['Intr']['mode_loc_data_corr']+'/'+inst+'_'+vis+'_add',data={'idx_est_loc':range(data_dic[inst][vis]['n_in_tr'])},allow_pickle=True)
...
'''
def loc_prof_indivCCFmod(inst,vis,gen_dic,data_dic):
    data_vis = data_dic[inst][vis]
    ...
    #Saving complementary data
    np.savez_compressed(gen_dic['save_data_dir']+'Loc_estimates/'+data_dic['Intr']['mode_loc_data_corr']+'/'+inst+'_'+vis+'_add',data={'idx_est_loc':idx_aligned},allow_pickle=True)
...
def loc_prof_rec(inst,vis,gen_dic,data_dic,coord_dic):
    ...
    for isub_rec,(vis_rec,iexp_rec) in enumerate(zip(vis_orig,idx_orig_vis)):
        data_exp = np.load(gen_dic['save_data_dir']+'Aligned_Intr_data/'+inst+'_'+vis_rec+'_in'+str(iexp_rec)+'.npz',allow_pickle=True)['data'].item()    
    ...
    for isub,i_in in enumerate(idx_aligned):
        ...
        #Upload local stellar profile for current exposure
        data_loc_exp = np.load(data_vis['proc_Res_data_paths']+str(gen_dic[inst][vis]['idx_in2exp'][i_in])+'.npz',allow_pickle=True)['data'].item()    
...
'''
Sub-function for local profiles interpolating them from imported profiles
'''
def loc_prof_theo():

            
    stop('TBD')
    #  method 4) we upload a grid of synthetic local spectra f(w,vis), which we interpolate at the relevant position of the planet-occulted regions, and scale with Sthick(band,t)

    ##### pour le module sur les spectres stellaire locaux theoriques:
#     Then we assign a spectrum to
# each point of the grid, by quadratically interpolating on  and
# Doppler-shifting the model spectra created using the tool Spectroscopy
# Made Easy (SME, Piskunov & Valenti 2017), with ATLAS9
# stellar atmospheric models (assuming solar abundances
# and local thermodynamic equilibrium approximation) and the
# line list from the VALD database (Ryabchikova et al. 2015).
...
def extract_pl_profiles(data_dic,inst,vis,gen_dic):
    ...
    if (data_dic['Atm']['pl_atm_sign']=='Absorption') or ((data_dic['Atm']['pl_atm_sign']=='Emission') and data_dic['Intr']['cov_loc_star']):
        ...
        data_vis['LocEst_Atm_data_paths'] = {iexp:gen_dic['save_data_dir']+'Loc_estimates/'+data_dic['Intr']['mode_loc_data_corr']+'/'+inst+'_'+vis+'_in'+str(i_in) for iexp,i_in in zip(iexp_paths,idx_est_loc)}
    ...
    if (gen_dic['calc_pl_atm']):
        ...
        for iexp,i_in in zip(range(data_vis['n_in_visit']),gen_dic[inst][vis]['idx_exp2in']):
            ...
            if (i_in>-1) and (i_in in idx_est_loc):data_loc_star = np.load(gen_dic['save_data_dir']+'Loc_estimates/'+data_dic['Intr']['mode_loc_data_corr']+'/'+inst+'_'+vis+'_in'+str(i_in)+'.npz',allow_pickle=True)['data'].item()   
...
'''
def conv_2D_to_1D_spec(data_type,inst,vis,gen_dic,data_dic,prop_dic,coord_dic,data_prop,system_param,theo_dic):
    data_inst = data_dic[inst]
    data_vis = data_dic[inst][vis]
    calc_check = gen_dic['calc_spec_1D_'+data_type]
    data_type_gen=gen_dic['data_type_gen'][data_type]
    if data_type=='DI':iexp_conv = range(data_dic[inst][vis]['n_in_visit'])               
    ...
    if calc_check:
        print('         Calculating data')
        nthreads = gen_dic['nthreads_spec_1D_'+data_type]
        ...
        if gen_dic['gain_weight'] and data_vis['mean_gdet']:
            proc_weight=True
            mean_gdet_data_paths = data_vis['mean_gdet_'+data_type_gen+'_data_paths']
        else:
            proc_weight=False
            mean_gdet_data_paths = None
        DImast_weight_data_paths = data_vis['mast_'+data_type_gen+'_data_paths'] if gen_dic['DImast_weight'] else None     
        ...
        else:cond_aligned = False
        
        #Saving complementary data
        np.savez_compressed(proc_gen_data_paths_new+'add',data={'iexp_conv':iexp_conv,'cond_aligned':cond_aligned},allow_pickle=True)     

    else: 
        
        #Updating path to processed data and checking it has been calculated      
        data_paths={iexp:proc_gen_data_paths_new+str(iexp) for iexp in iexp_conv}
        check_data(data_paths) 
    ...
    for iexp in iexp_conv:
        data_vis['mast_'+data_type_gen+'_data_paths'][iexp] = proc_gen_data_paths_new+'ref_'+str(iexp)
        if data_vis['tell_sp']:data_vis['tell_'+data_type_gen+'_data_paths'][iexp] = proc_gen_data_paths_new+'_tell'+str(iexp)
        if data_type_gen=='Atm':data_vis['LocEst_Atm_data_paths'][iexp] = proc_gen_data_paths_new+'estloc_'+str(iexp)
    ...
    for isub,iexp in enumerate(iexp_conv):   
        ...
        if data_type_gen=='DI':iexp_glob=iexp
            ...
            if data_type_gen=='Intr':iexp_glob = idx_in2exp[iexp]                     
                ...
                data_est_loc=dataload_npz(LocEst_Atm_data_paths[iexp])
                flux_est_loc_exp = data_est_loc['flux']
                if cov_loc_star:cov_est_loc_exp = data_est_loc['cov']                      
                if data_type=='Emission':iexp_glob=iexp
                elif data_type=='Absorption':
                    SpSstar_spec = data_exp['SpSstar_spec']
                    iexp_glob = idx_in2exp[iexp]                      

        #Weight definition
        #    - since here we bin profiles defined on the same spectral table no resampling is required
        ...
        #      all profiles that are the same for overlapping orders are thus not used in the weighing
        #      they are however processed in the same way as the exposure if used later on in the pipeline 
        weight_exp = def_weights_spatiotemp_bin(range(nord),None,inst,vis,gen_corr_Fbal,gen_corr_Fbal_ord,save_data_dir,gen_type,nord,iexp_glob,data_type,data_mode,dim_exp,None,data_exp['mean_gdet'], data_exp['cen_bins'],np.ones(dim_exp),None,corr_Fbal=False)

        #Processing each order
        tot_weight = np.zeros(nspec_1D,dtype=float) 
        ...
        for iord in range(nord):
            ...
            #Multiply by order weight
            #    - we perform the operation before resampling because the weights are defined with the same structure as the profiles
            flux_temp,cov_temp = bind.mul_array( data_exp['flux'][iord] , data_exp['cov'][iord] , weight_exp[iord])

            #Resample on 1D table
            flux_ord,cov_ord = bind.resampling(edge_bins_1D, data_exp['edge_bins'][iord], flux_temp , cov = cov_temp, kind=resamp_mode)                        
            weight_ord = bind.resampling(edge_bins_1D, data_exp['edge_bins'][iord], weight_exp[iord], kind=resamp_mode)                        
            cond_def = (~np.isnan(flux_ord)) & (~np.isnan(weight_ord))

            #Set undefined pixels to 0 so that they do not contribute to the 1D spectrum
            flux_ord[~cond_def] = 0.            
            ...
            if SpSstar_spec is not None:
                SpSstar_spec_temp = bind.resampling(edge_bins_1D,data_exp['edge_bins'][iord],SpSstar_spec[iord](data_exp['cen_bins'][iord])*weight_exp[iord] ,kind=resamp_mode)
                SpSstar_spec_temp[~cond_def] = 0.
                SpSstar_spec_ord_contr+=SpSstar_spec_temp
            if flux_est_loc_exp is not None:  
                if cov_est_loc_exp is None:                    
                    flux_est_loc_temp = bind.resampling(edge_bins_1D, data_ref['edge_bins'][iord], flux_est_loc_exp[iord]*weight_exp[iord] , kind=resamp_mode)   
                    flux_est_loc_temp[~cond_def] = 0.   
                    flux_est_loc_ord_contr+=flux_est_loc_temp                  
                else:
                    flux_est_loc_temp,cov_est_loc_temp = bind.mul_array( flux_est_loc_exp[iord], cov_est_loc_exp[iord] , weight_exp[iord])
                    flux_est_loc_temp,cov_est_loc_temp = bind.resampling(edge_bins_1D, data_ref['edge_bins'][iord], flux_est_loc_temp , cov = cov_est_loc_temp, kind=resamp_mode)   
                    flux_est_loc_temp[~cond_def] = 0.   
                    flux_est_loc_ord_contr+=[flux_est_loc_temp]
                    cov_est_loc_ord_contr+=[cov_est_loc_temp]

            #Update total weight per pixel
            tot_weight[cond_def] += weight_ord[cond_def]
            
            #Defined bins in 1D spectrum
            #    - a bin is defined if at least one bin is defined in any of the contributing orders
            cond_def_1D |= cond_def

        #Co-addition of spectra from all orders
        flux_1D,cov_1D = bind.sum(flux_ord_contr,cov_ord_contr)

        #Normalization of defined pixels
        #    - the normalization factor is set to 1 for undefined pixels to avoid warnings, and their value is set to nan afterward
        norm_fact = np.ones(nspec_1D,dtype=float)
        norm_fact[cond_def_1D] = 1./tot_weight[cond_def_1D]
        flux_1D,cov_1D = bind.mul_array(flux_1D,cov_1D,norm_fact)

        #Reset undefined pixels to nan
        flux_1D[~cond_def_1D]=np.nan        
        ...
        if DImast_weight:
            ...
            np.savez_compressed(proc_gen_data_paths_new+'ref_'+str(iexp),data={'edge_bins':data_exp1D['edge_bins'],'flux':flux_ref_1D[None,:],'cov':[cov_1D]},allow_pickle=True)           
        if flux_est_loc_exp is not None:
            if cov_est_loc_exp is None:
                flux_est_loc_1D = flux_est_loc_ord_contr*norm_fact
                flux_est_loc_1D[~cond_def_1D]=np.nan 
                ...
                flux_est_loc_1D,cov_est_loc_1D = bind.mul_array(flux_est_loc_1D,cov_est_loc_1D,norm_fact)
                flux_est_loc_1D[~cond_def_1D]=np.nan   
                dic_sav_estloc = {'edge_bins':data_exp1D['edge_bins'],'flux':flux_est_loc_1D[None,:],'cov':[cov_est_loc_1D]}
            np.savez_compressed(proc_gen_data_paths_new+'estloc_'+str(iexp),data=dic_sav_estloc,allow_pickle=True)           
...
'''
Routine to analyze atmospheric profiles
'''
def analyze_prof(mode,data_type,data_dic,gen_dic,inst,vis,coord_dic,theo_dic,plot_dic):
    ana_dic={}
    
    stop('Merge with fit_prof')
    ...
    if data_type=='Atmorig':
        print('   > Analyzing atmospheric '+data_dic['Atm']['pl_atm_sign']+' profiles') 
        if data_dic['Atm']['pl_atm_sign']=='Absorption':n_exp = data_dic[inst][vis]['n_in_tr']
        elif data_dic['Atm']['pl_atm_sign']=='Emission':n_exp = data_dic[inst][vis]['n_in_visit']
        iexp_def = data_dic['Atm'][inst][vis]['idx_def']        
        
    elif data_type=='Atmbin': 
        print('   > Analyzing binned atmospheric '+data_dic['Atm']['pl_atm_sign']+' profiles') 
        data_bin = np.load(gen_dic['save_data_dir']+'Atmbin_data/'+data_dic['Atm']['pl_atm_sign']+'/'+inst+'_'+vis_save+'_'+data_dic['Atm']['dim_bin']+'_add.npz',allow_pickle=True)['data'].item()
        n_exp = data_bin['n_exp']
        iexp_def = range(n_exp)
        
    #Mean signal in requested ranges
    ana_dic['int_sign']=np.zeros(n_exp,dtype=float)
    ana_dic['e_int_sign']=np.zeros(n_exp,dtype=float)
    ana_dic['R_sign']=np.zeros(n_exp,dtype=float)
    ...
    for iexp in iexp_def:                
        ...
        #Upload latest processed profiles
        if data_type=='Atmorig':
            data_exp = np.load(data_dic[inst][vis]['proc_Atm_data_paths']+str(iexp)+'.npz',allow_pickle=True)['data'].item()    
        elif data_type=='Atmbin':
            data_exp = np.load(gen_dic['save_data_dir']+'Atmbin_data/'+data_dic['Atm']['pl_atm_sign']+'/'+inst+'_'+vis_save+'_'+data_dic['Atm']['dim_bin']+str(iexp)+'.npz',allow_pickle=True)['data'].item()    

        #Calculation of mean integrated signal
        cond_int_sign=False 
        for bd_int in data_dic['Atm']['int_sign_range']:cond_int_sign |= (data_exp['edge_bins'][0,0:-1]>=bd_int[0]) & (data_exp['edge_bins'][0,1::]<=bd_int[1])          
        dcen_range = data_exp['edge_bins'][0,1::][cond_int_sign]-data_exp['edge_bins'][0,0:-1][cond_int_sign]
        width_range = np.sum(dcen_range)
        ana_dic['int_sign'][iexp] = np.sum(data_exp['flux'][cond_int_sign]*dcen_range)/width_range
        ana_dic['e_int_sign'][iexp] = np.sqrt(  np.sum( (data_exp['cov'][0][0,cond_int_sign]*dcen_range)**2. ))/width_range
        ana_dic['R_sign'][iexp] = ana_dic['int_sign']/ana_dic['e_int_sign']

        stop('checker erreur avec JB')

    #Saving data          
    np.savez_compressed(gen_dic['save_data_dir']+data_type+'_prop/'+data_dic['Atm']['pl_atm_sign']+'/'+inst+'_'+vis_save,data=ana_dic,allow_pickle=True)
...
def get_planet_disk_prop(spots_prop, pl_loc, grid_dic,system_prop, x_pos_pl, y_pos_pl, star_params, LD_law, ld_coeff, gd_band, cb_band, param, coeff_ord2name, func_prof_name, Rp_Rs, var_par_list, pol_mode,args) :
    ...
    if True in cond_pl_occ:
        ...
        if func_prof_name == 'voigt' :
            ...
            region_prop['slope_pl']  = poly_prop_calc(param,coord_prop,coeff_ord2name['slope'],  pol_mode)
...
def compute_deviation_profile(args, param, inst, vis, iexp,gen_dic,theo_dic,data_dic,coord_dic,system_param) :
    ...
    #             input_exp['slope']  = tab_prop_sp ['slope_sp'] [itile]
