...
..
  Erik: actually it would be useful to add some information about the new blaze functionality, and some details about the options in this module
...
..
  Erik: could you make a figure of the calibration profile in a given order, with the estimate from input data in one subplot, and the blaze-derived one in another ?
...
..
  Erik: could you add a figure of the telluric properties
...
You can model the continuum of the model CCF with a polynomial instead of a flat line (with :green:`deg` between 1 and 4):: 
 ...
 gen_dic['tell_cont_range'] = {instrument:{visit:{molecule : deg}} 
...
.. Tip:: 
 ...
 This may become apparent through poorly-fitted telluric CCFs (look out for large variations in the blue residual profile of :numref:`Tell_CCF_comp`) and emission-like features around masked telluric lines in the corrected spectra.
 ...
 In the present example we mask all exposures, in the six orders (actually, duplicated ESPRESSO slices that cover the same spectral range) that typically contain the deep telluric lines to mask in ESPRESSO datasets. 
 ...
 ..
   Erik: not sure I understand the interest of this ?
   You can also plot it using the :green:`plot_dic[sp_raw']` and plot all exposures on top of each other, but I prefer using the rainbow colour scaling for exposures using the Global masters.

 ..
   Erik: it would be useful to show the global master plot in one of the masked orders, zoomed on the deep telluric lines, just before 'Then, go back to the ..'
...
This is the option that was preferred for the TOI-421 dataset given the medium-frequency variations that can be seen within each visit, and between both visits, in :numref:`Fbal_corr`. 
 
.. figure:: Fbal_corr.png
  :width: 800
  :name: Fbal_corr

  Flux balance variations in the 20221117 and 20231106 visits. Exposures are colour-coded over the rainbow scale with increasing time, from purple to red.
  Splines allow capturing subtle features, such as the ones visible around spectral order 112.
  The use of phantom bins avoids divergences in the blue part of the spectrum.

..
  Erik: can you show the correction of both visits in two subplots, and add the inter-visit plot here, as a third subplot ?
...
You can further inspect the plots activated with :green:`plot_dic['cosm_corr']='pdf'` (saved in :orange:`/Working_dir/Star/Planet_Plots/Spec_raw/Cosmics_Instrument_Visit/`).
Spectral detection profiles (ie, the ratio between the flux difference and error mentioned above) plotted for each exposure and order show cosmic-flagged pixels allow you to adjust the detection threshold.
The :orange:`Occurrences` plot, which shows the number of flagged cosmic in each order over all exposures, is a good way to check if you are overcorrecting your dataset (typically if more pixels are flagged as
